...
export class MapController extends BaseScriptComponent {
   ...
   * IMPORTANT: Cette fonction doit être appelée AVANT toute modification du zoom.
   * 
   * @param currentOffset - L'offset actuel de la grille (vec2)
   * @param zoomLevel - Le niveau de zoom actuel (AVANT modification)
   * @param currentOffsetForLocation - L'offsetForLocation actuel (AVANT modification)
   * @param currentMapLocation - La mapLocation actuelle (AVANT modification)
   * @param currentNorthwestAsset - Le northwestLocationAsset actuel (AVANT modification)
  ...
  private getCurrentViewCenterGeoPosition(
    currentOffset: vec2, 
    zoomLevel: number,
    currentOffsetForLocation: vec2,
    currentMapLocation: GeoPosition,
    currentNorthwestAsset: LocationAsset
  ): GeoPosition {
    // Le centre de la vue correspond à l'offset - mapFocusPosition (qui est normalement 0.5, 0.5)
    const centerOffset = currentOffset.sub(this.mapParameters.mapFocusPosition);
    
    // Calculer la position relative par rapport à l'offset de la position de référence
    const relativeOffset = centerOffset.sub(currentOffsetForLocation);
    
    // Convertir cet offset en coordonnées géographiques
    // en utilisant la même logique que fromLocalPositionToLongLat
    const pixelOffsetX = relativeOffset.x * TEXTURE_SIZE;
    const pixelOffsetY = relativeOffset.y * TEXTURE_SIZE;
    
    const mapImageOffset = this.mapModule.longLatToImageRatio(
      currentMapLocation.longitude,
      currentMapLocation.latitude,
      currentNorthwestAsset
    );
    
    const pixelX = mapImageOffset.x * TEXTURE_SIZE + pixelOffsetX;
    const pixelY = mapImageOffset.y * TEXTURE_SIZE - pixelOffsetY;
    ...
    geoPos.altitude = currentMapLocation.altitude || 0;
  ...
  handleZoomIn(): void {
    ...
    const oldOffsetForLocation = this.offsetForLocation.clone(); // Important: cloner pour éviter les références
    const oldMapLocation = GeoPosition.create();
    oldMapLocation.longitude = this.mapLocation.longitude;
    oldMapLocation.latitude = this.mapLocation.latitude;
    oldMapLocation.altitude = this.mapLocation.altitude;
    const oldNorthwestAsset = this.northwestLocationAsset;
    ...
    // 2. Calculer la position géographique au centre avec les anciennes valeurs
    const centerGeoPosition = this.getCurrentViewCenterGeoPosition(
      oldOffset,
      oldZoomLevel,
      oldOffsetForLocation,
      oldMapLocation,
      oldNorthwestAsset
    );
    ...
    // 4. Recalculer les assets de référence avec le nouveau zoom
    this.referencePositionLocationAsset = LocationAsset.getGeoAnchoredPosition(
      this.mapLocation.longitude,
      this.mapLocation.latitude
    ).location.adjacentTile(0, 0, this.mapParameters.zoomOffet);
    ...
    // 5. Mettre à jour offsetForLocation avec le nouveau zoom
    this.updateLocationOffset();
    
    log.i(`New offsetForLocation: (${this.offsetForLocation.x.toFixed(4)}, ${this.offsetForLocation.y.toFixed(4)})`);
    
    // 6. Calculer où centerGeoPosition doit être positionné avec le nouveau zoom
    const newCenterOffset = getOffsetForLocation(
      this.mapModule,
      this.referencePositionLocationAsset,
      centerGeoPosition.longitude,
      centerGeoPosition.latitude
    );
    
    log.i(`New center offset: (${newCenterOffset.x.toFixed(4)}, ${newCenterOffset.y.toFixed(4)})`);
    
    // 7. Appliquer le nouvel offset (en ajoutant mapFocusPosition)
    const finalOffset = newCenterOffset.add(this.mapParameters.mapFocusPosition);
    log.i(`Final offset to apply: (${finalOffset.x.toFixed(4)}, ${finalOffset.y.toFixed(4)})`);
    
    this.gridView.setOffset(finalOffset);
    
    // 8. Recréer le PinOffsetter avec les nouveaux paramètres
    this.pinOffsetter = PinOffsetter.makeMapLocationOffsetter(
      this.mapModule,
      this.referencePositionLocationAsset
    );
    
    // 9. Mettre à jour la grille et les pins
    this.gridView.handleUpdateConfig(this.config);
    this.gridView.layoutCells(true);
    
    // 10. Repositionner tous les pins avec le nouveau zoom
    this.pinSet.forEach((pin: MapPin) => {
      this.pinOffsetter.bindScreenTransformToLocation(
        pin.screenTransform,
        pin.location.longitude,
    ...
    // 11. Repositionner le pin utilisateur si présent
    if (this.userPin) {
      this.pinOffsetter.bindScreenTransformToLocation(
        this.userPin.screenTransform,
  ...
  handleZoomOut(): void {
    ...
    const oldZoomLevel = this.mapParameters.zoomLevel;
    const oldOffset = this.gridView.getOffset();
    const oldOffsetForLocation = this.offsetForLocation.clone();
    const oldMapLocation = GeoPosition.create();
    oldMapLocation.longitude = this.mapLocation.longitude;
    oldMapLocation.latitude = this.mapLocation.latitude;
    oldMapLocation.altitude = this.mapLocation.altitude;
    ...
    // 10. Repositionner tous les pins
    this.pinSet.forEach((pin: MapPin) => {
      this.pinOffsetter.bindScreenTransformToLocation(
        pin.screenTransform,
        pin.location.longitude,
