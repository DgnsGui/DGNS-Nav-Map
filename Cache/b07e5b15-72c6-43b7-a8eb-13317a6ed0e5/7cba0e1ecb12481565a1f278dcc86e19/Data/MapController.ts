...
export class MapController extends BaseScriptComponent {
  ...
  handleZoomIn(): void {
    ...
    const newOffset = getOffsetForLocation(
      this.mapModule,
      this.referencePositionLocationAsset,
      currentViewCenter.latitude,
      currentViewCenter.longitude
    );
  ...
  private getViewCenterLocation(): GeoPosition | null {
    if (!this.isInitialized) return null;
    
    const currentOffset = this.gridView.getOffset();
    
    // Convertir l'offset de la vue en position locale
    const centerLocalPos = new vec2(
      currentOffset.x - 0.5,
      currentOffset.y - 0.5
    );
