...
let MapController = (() => {
    ...
    var MapController = _classThis = class extends _classSuper {
        ...
        // CORRECTIF ZOOM: Préserver la position de la vue pendant le zoom
        handleZoomIn() {
            log.i("Zoom in - preserving view center");
            // IMPORTANT: Calculer les coordonnées AVANT de changer le zoom
            const currentViewCenter = this.getViewCenterLocation();
            if (!currentViewCenter) {
                log.e("Could not get current view center, aborting zoom");
                return;
            }
            log.i(`Current view center: lat=${currentViewCenter.latitude}, lon=${currentViewCenter.longitude}`);
            // Changer le zoom
            this.mapParameters.zoomLevel++;
            this.mapParameters.zoomOffet = (0, MapUtils_1.calculateZoomOffset)(this.mapParameters.zoomLevel);
            // Reconfigurer les assets de location avec le NOUVEAU zoom
            this.referencePositionLocationAsset = LocationAsset.getGeoAnchoredPosition(this.mapLocation.longitude, this.mapLocation.latitude).location.adjacentTile(0, 0, this.mapParameters.zoomOffet);
            this.northwestLocationAsset = LocationAsset.getGeoAnchoredPosition(MAX_LONGITUDE, MAX_LATITUDE).location.adjacentTile(0, 0, this.mapParameters.zoomOffet);
            this.updateLocationOffset();
            // Créer le nouveau pin offsetter avec le nouveau zoom
            this.pinOffsetter = PinOffsetter_1.PinOffsetter.makeMapLocationOffsetter(this.mapModule, this.referencePositionLocationAsset);
            // Calculer le nouvel offset pour garder le même centre géographique
            const newOffset = (0, MapUtils_1.getOffsetForLocation)(this.mapModule, this.referencePositionLocationAsset, currentViewCenter.latitude, currentViewCenter.longitude);
            log.i(`Setting new offset: x=${newOffset.x}, y=${newOffset.y}`);
            // Appliquer le nouvel offset (le centre de la vue reste au même endroit géographique)
            this.gridView.setOffset(newOffset.add(new vec2(0.5, 0.5)));
            // Mettre à jour la grille
            this.gridView.handleUpdateConfig(this.config);
            this.gridView.layoutCells(true);
            // Rebind tous les pins avec le nouveau système de coordonnées
            this.pinSet.forEach((pin) => {
                this.pinOffsetter.bindScreenTransformToLocation(pin.screenTransform, pin.location.longitude, pin.location.latitude);
            });
            // Rebind le user pin si présent
            if (this.userPin && this.userPin.screenTransform) {
        ...
        handleZoomOut() {
            log.i("Zoom out - preserving view center");
            // IMPORTANT: Calculer les coordonnées AVANT de changer le zoom
            const currentViewCenter = this.getViewCenterLocation();
            if (!currentViewCenter) {
                log.e("Could not get current view center, aborting zoom");
        ...
        setNewMapLocation(location) {
            ...
            this.pinOffsetter.layoutScreenTransforms(this.gridView);
            // CORRECTIF: Ne pas recentrer automatiquement si l'utilisateur a scrollé
            if (this.shouldFollowMapLocation && !this.viewScrolled) {
        ...
        centerMap() {
            ...
            this.tweenCancelFunction = (0, MapUtils_1.makeTween)((t) => {
                ...
                if (t === 1) {
                    // CORRECTIF: Réactiver le suivi ET réinitialiser le flag de scroll
                    this.shouldFollowMapLocation = true;
        ...
        // NOUVELLE MÉTHODE: Obtenir la position géographique du centre de la vue
        getViewCenterLocation() {
            if (!this.isInitialized)
                return null;
            const currentOffset = this.gridView.getOffset();
            // Convertir l'offset de la vue en position locale
            const centerLocalPos = new vec2(currentOffset.x - 0.5, currentOffset.y - 0.5);
            // Convertir en coordonnées géographiques
            return this.fromLocalPositionToLongLat(centerLocalPos, this.mapParameters.zoomLevel);
        }
