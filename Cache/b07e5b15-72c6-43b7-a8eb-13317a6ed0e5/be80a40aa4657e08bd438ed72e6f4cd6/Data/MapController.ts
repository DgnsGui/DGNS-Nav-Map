// Fix: Add declarations for Lens Studio environment types and globals to resolve TypeScript errors.
...
declare class ScreenTransform {
    rotation: quat;
    anchors: { getCenter(): vec2; setCenter(pos: vec2): void; };
    scale: vec3;
    localPointToWorldPoint(point: vec2): vec3;
}
...
declare class ImageComponent {
    ...
    mainPass: { baseTex: any };
}

declare class SceneObject {
    getChild(index: number): SceneObject;
    getComponent(name: string): any; // Can be ScreenTransform, ImageComponent, etc.
...
declare class LocationService {
    onNorthAlignedOrientationUpdate: { add(callback: (orientation: quat) => void): void };
    accuracy: any;
    getCurrentPosition(success: (pos: GeoPosition) => void, error: (msg: string) => void): void;
}
...
declare class LocationAsset {
    static getGeoAnchoredPosition(lon: number, lat: number): { location: { adjacentTile(a: number, b: number, c: number): LocationAsset }};
}

declare class MapTextureProvider {
    onFailed: { add(callback: () => void): void };
    onReady: { add(callback: () => void): void };
    control: any;
}

declare class MapModule {
    createMapTextureProvider(): { control: MapTextureProvider, [key: string]: any };
    longLatToImageRatio(lon: number, lat: number, asset: LocationAsset): vec2;
}
...
export class MapController extends BaseScriptComponent {
  ...
  /**
   * [CORRECTED] Zooms in, keeping the current view centered.
   * This function now operates purely in the grid's offset coordinate system
   * to avoid floating-point inaccuracies from geo-coordinate conversions.
   */
  handleZoomIn(): void {
    if (!this.isInitialized) {
    ...
    // 2. Calculate the vector from the map's reference location to the current view center.
    // This represents the user's scroll.
    const relativeOffset = oldOffset.sub(oldOffsetForLocation);
    ...
    // 4. Recalculate reference assets and the new reference offset with the new zoom.
    this.referencePositionLocationAsset = LocationAsset.getGeoAnchoredPosition(
      this.mapLocation.longitude,
      this.mapLocation.latitude
    ).location.adjacentTile(0, 0, this.mapParameters.zoomOffet);
    ...
    this.updateLocationOffset(); // This calculates the new 'offsetForLocation'
    
    // 5. Calculate the final offset. We take the new reference offset and add the
    // scaled relative offset. When zooming in, the distance doubles.
    const finalOffset = this.offsetForLocation.add(relativeOffset.uniformScale(2.0));
    
    this.gridView.setOffset(finalOffset);
    
    // 6. Recreate the PinOffsetter and update all visuals for the new zoom level.
    this.pinOffsetter = PinOffsetter.makeMapLocationOffsetter(
      this.mapModule,
      this.referencePositionLocationAsset
    );
  ...
  /**
   * [CORRECTED] Zooms out, keeping the current view centered.
   * This function uses the same stable offset-based logic as handleZoomIn.
   */
  handleZoomOut(): void {
    if (!this.isInitialized) {
      log.w("Cannot zoom: map not initialized");
    ...
    // 5. Calculate the final offset. When zooming out, the distance is halved.
    const finalOffset = this.offsetForLocation.add(relativeOffset.uniformScale(0.5));
