...
export class MapController extends BaseScriptComponent {
  ...
  // Déclarations complètes des événements
  ...
  removeMapPin(mapPin: MapPin) {
    if (this.pinSet.has(mapPin)) { this.pinSet.delete(mapPin); if (mapPin.placeInfo) this.pinnedPlaceSet.delete(mapPin.placeInfo.placeId); this.pinOffsetter.unbindScreenTransform(mapPin.screenTransform); mapPin.sceneObject.destroy(); this.onMapPinRemovedEvent.invoke(mapPin); }
  }

  removeMapPins() {
    this.pinSet.forEach((pin: MapPin) => { this.pinOffsetter.unbindScreenTransform(pin.screenTransform); pin.sceneObject.destroy(); });
    this.pinSet.clear();
  ...
  spawnUserPin(prefab: ObjectPrefab, location: GeoPosition, scale: vec2) {
    this.userPin = MapPin.makeMapPin(prefab, this.mapGridObject, this.mapPinsAnchor.layer, 3, location, undefined, true);
  ...
  private setNewUserPosition(location: GeoPosition): void {
    ...
    if (!old && location) this.onUserLocationSetEvent.invoke(location);
  }
  ...
  drawGeometryPoint(p: vec2, r: number) {/**/}
  drawGeometryLine(l: vec2[], t: number) {/**/}
  drawGeometryMultiline(m: any, t: number) {/**/}
  ...
  centerMap(): void {
    ...
    const target = getOffsetForLocation(this.mapModule, this.referencePositionLocationAsset, this.mapLocation.longitude, this.mapLocation.latitude).add(new vec2(0.5, 0.5));
  ...
  isMapCentered(): boolean {
    ...
    return this.gridView.getOffset().equal(getOffsetForLocation(this.mapModule, this.referencePositionLocationAsset, this.mapLocation.longitude, this.mapLocation.latitude).add(new vec2(0.5, 0.5)));
  }
