...
let MapController = (() => {
    ...
    var MapController = _classThis = class extends _classSuper {
        ...
        updateZoomLevel(newZoomLevel) {
            // 1. Sauvegarder l'offset actuel et calculer le centre de la vue
            const currentOffset = this.gridView.getOffset();
            // Le centre de la vue est à l'offset actuel (0.5, 0.5 représente le centre de l'écran)
            const viewCenterOffset = currentOffset.sub(new vec2(0.5, 0.5));
            // 2. Convertir cet offset en position géographique au zoom actuel
            const currentZoomLevel = this.mapParameters.zoomLevel;
            const centerGeoPosition = this.fromLocalPositionToLongLat(viewCenterOffset, currentZoomLevel);
            log.i(`Zoom center: lat=${centerGeoPosition.latitude}, lon=${centerGeoPosition.longitude}`);
            ...
            // Ajouter 0.5, 0.5 pour repositionner le centre au centre de l'écran
            const newOffset = newCenterOffset.add(new vec2(0.5, 0.5));
