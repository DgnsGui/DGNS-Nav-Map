// --- START OF FILE MapController.ts (FINAL with Syntax Fix) ---
...
export class MapController extends BaseScriptComponent {
  ...
  private createMapPin(location: GeoPosition, placeInfo: PlaceInfo = undefined): MapPin {
  ...
  private updateRotations() { const pinRotation = -this.getUserHeading(); if (this.mapParameters.showUserPin && this.userPin) this.updateUserPinRotation(pinRotation); if (this.mapParameters.setMapToCustomLocation) return; if (this.mapParameters.isMinimapAutoRotate && !this.viewScrolled && this.config.isMiniMap) { this.updateMapRotation(); this.updateMapPinRotations(pinRotation); } }
  private updateMapPinRotations(pinRotation: number) { if (this.mapParameters.mapPinsRotated) { const targetRot = quat.fromEulerAngles(0, 0, pinRotation); this.pinSet.forEach((pin: MapPin) => { pin.screenTransform.rotation = this.mapParameters.enableMapSmoothing ? quat.slerp(pin.screenTransform.rotation, targetRot, 4 * getDeltaTime()) : targetRot; }); } }
  private updateMapRotation() { const targetRot = quat.fromEulerAngles(0, 0, this.getUserHeading()); this.config.gridScreenTransform.rotation = this.mapParameters.enableMapSmoothing ? quat.slerp(this.config.gridScreenTransform.rotation, targetRot, 4 * getDeltaTime()) : targetRot; }
  private updateUserPinRotation(pinRotation: number) { if (this.userPin && this.userPin.screenTransform && this.mapParameters.userPinAlignedWithOrientation) { const targetRot = quat.fromEulerAngles(0, 0, pinRotation); this.userPin.screenTransform.rotation = this.mapParameters.enableMapSmoothing ? quat.slerp(this.userPin.screenTransform.rotation, targetRot, 4 * getDeltaTime()) : targetRot; } }
  ...
  private setPinLocation(pin: MapPin, adjustedAnchoredPosition: vec2) { const offset = this.gridView.getOffset().sub(this.offsetForLocation).sub(new vec2(0.5, 0.5)); const location: GeoPosition = this.fromLocalPositionToLongLat(new vec2(adjustedAnchoredPosition.x - offset.x, adjustedAnchoredPosition.y + offset.y), this.mapParameters.zoomLevel); pin.location = location; this.pinOffsetter.bindScreenTransformToLocation(pin.screenTransform, pin.location.longitude, pin.location.latitude); if (this.userLocation) pin.location.altitude = this.userLocation.altitude; this.onMapPinAddedEvent.invoke(pin); }
  private fromLocalPositionToLongLat(localPosition: vec2, zoomLevel: number): GeoPosition { const px = localPosition.x * TEXTURE_SIZE; const py = -localPosition.y * TEXTURE_SIZE; const mapOffset = this.mapModule.longLatToImageRatio(this.mapLocation.longitude, this.mapLocation.latitude, this.northwestLocationAsset); const pixelX = mapOffset.x * TEXTURE_SIZE + px; const pixelY = mapOffset.y * TEXTURE_SIZE + py; const mapSize = TEXTURE_SIZE << zoomLevel; const x = clip(pixelX, 0, mapSize - 1) / mapSize - 0.5; const y = 0.5 - clip(pixelY, 0, mapSize - 1) / mapSize; const lat = 90 - (360 * Math.atan(Math.exp(-y * 2 * Math.PI))) / Math.PI; const lon = 360 * x; const loc = GeoPosition.create(); loc.longitude = lon; loc.latitude = lat; return loc; }
  ...
  private createMapGrid() { const gridST = this.mapGridObject.getComponent("ScreenTransform"); this.gridView = MapGridView.makeGridView(this); this.config = MapConfig.makeConfig(this.mapPinsAnchor, this.mapScreenTransform, gridST, this.mapTilePrefab, this, this.mapParameters.enableScrolling, this.mapParameters.scrollingFriction, this.mapParameters.tileCount); this.initialMapLocation = GeoPosition.create(); this.initialMapLocation.longitude = this.mapLocation.longitude; this.initialMapLocation.latitude = this.mapLocation.latitude; this.onInitialLocationSetEvent.invoke(this.initialMapLocation); this.shouldFollowMapLocation = true; this.setUpZoom(); }
  ...
  showNearbyPlaces(category: string[]): void {
    this.placesProvider.getNearbyPlaces(this.mapLocation, NEARBY_PLACES_RANGE, category)
      .then((places) => {
        if (places.length === 0) {
