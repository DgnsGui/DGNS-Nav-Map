...
const NEARBY_PLACES_RANGE = 100; // Keep as default fallback
...
export class MapController extends BaseScriptComponent {
  ...
  // Search configuration - AJOUT POUR CORRIGER L'ERREUR
  private searchRadius: number = NEARBY_PLACES_RANGE; // Default search radius
  ...
  /**
   * Set the search radius for nearby places in meters
   */
  setSearchRadius(radius: number): void {
    this.searchRadius = radius;
    log.i(`Search radius set to ${radius} meters`);
  }
  ...
  /**
   * Show nearby places with specific categories and configurable radius
   */
  showNearbyPlaces(category: string[]): void {
    log.i(`Searching for nearby places with radius: ${this.searchRadius}m, categories: ${category.join(', ')}`);
    
    if (!this.placesProvider) {
      log.e("Places provider not available");
      this.onNearbyPlacesFailedEvent.invoke();
      return;
    }

    if (!this.mapLocation) {
      log.e("Map location not available");
      this.onNearbyPlacesFailedEvent.invoke();
      return;
    }
    
    this.placesProvider
      .getNearbyPlaces(this.mapLocation, this.searchRadius, category)
      .then((places) => {
        if (places.length === 0) {
          log.i("No nearby places found");
