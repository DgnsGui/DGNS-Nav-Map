...
let MapController = (() => {
    ...
    var MapController = _classThis = class extends _classSuper {
        ...
        offsetToGeoPosition(offset, zoomLevel) {
            // Convertir l'offset de la grille en position géographique
            // L'offset représente la position dans l'espace normalisé des tuiles
            const mapSize = TEXTURE_SIZE << zoomLevel;
            // Calculer la position en pixels dans l'espace des tuiles
            const x = (offset.x + 0.5) * mapSize;
            const y = (0.5 - offset.y) * mapSize;
            // Convertir en coordonnées normalisées [0, 1]
            const normalizedX = x / mapSize;
            const normalizedY = y / mapSize;
            // Convertir en longitude/latitude selon la projection Mercator
            const longitude = (normalizedX - 0.5) * 360;
            const latitude = 90 - (360 * Math.atan(Math.exp(-(normalizedY - 0.5) * 2 * Math.PI))) / Math.PI;
            const location = GeoPosition.create();
            location.longitude = longitude;
            location.latitude = latitude;
            return location;
        }
        geoPositionToOffset(location, zoomLevel, referenceAsset) {
            // Calculer l'offset pour une position géographique donnée à un niveau de zoom donné
            return (0, MapUtils_1.getOffsetForLocation)(this.mapModule, referenceAsset, location.longitude, location.latitude);
        }
        updateZoomLevel(newZoomLevel) {
            ...
            const currentReferenceAsset = this.referencePositionLocationAsset;
            log.i(`Current offset: (${currentOffset.x}, ${currentOffset.y})`);
            // 2. Calculer la position géographique du centre de la vue actuelle
            // Le centre de la vue correspond à currentOffset
            const centerGeoPosition = this.offsetToGeoPosition(currentOffset.sub(this.offsetForLocation), currentZoomLevel);
            log.i(`View center at: lat=${centerGeoPosition.latitude}, lon=${centerGeoPosition.longitude}`);
            ...
            this.referencePositionLocationAsset = LocationAsset.getGeoAnchoredPosition(this.mapLocation.longitude, this.mapLocation.latitude).location.adjacentTile(0, 0, this.mapParameters.zoomOffet);
            this.northwestLocationAsset = LocationAsset.getGeoAnchoredPosition(MAX_LONGITUDE, MAX_LATITUDE).location.adjacentTile(0, 0, this.mapParameters.zoomOffet);
            // 5. Mettre à jour l'offset pour la location de base
            this.updateLocationOffset();
            // 6. Recréer le pinOffsetter avec les nouveaux assets
            this.pinOffsetter = PinOffsetter_1.PinOffsetter.makeMapLocationOffsetter(this.mapModule, this.referencePositionLocationAsset);
            // 7. Calculer le nouvel offset pour le même centre géographique
            const newCenterOffset = this.geoPositionToOffset(centerGeoPosition, newZoomLevel, this.referencePositionLocationAsset);
            const newOffset = newCenterOffset.add(this.offsetForLocation);
            log.i(`New offset: (${newOffset.x}, ${newOffset.y})`);
            ...
            log.i(`Zoom ${newZoomLevel > currentZoomLevel ? 'IN' : 'OUT'} completed at level ${this.mapParameters.zoomLevel}`);
        }
        handleZoomIn() {
