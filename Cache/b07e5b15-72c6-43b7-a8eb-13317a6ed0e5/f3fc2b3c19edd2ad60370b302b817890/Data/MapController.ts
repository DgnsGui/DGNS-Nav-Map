...
export class MapController extends BaseScriptComponent {
  ...
  private offsetToGeoPosition(offset: vec2, zoomLevel: number): GeoPosition {
  ...
  private geoPositionToOffset(location: GeoPosition, zoomLevel: number, referenceAsset: LocationAsset): vec2 {
    // Calculer l'offset pour une position géographique donnée à un niveau de zoom donné
    return getOffsetForLocation(
      this.mapModule,
      referenceAsset,
      location.longitude,
      location.latitude
  ...
  private updateZoomLevel(newZoomLevel: number): void {
    ...
    const centerGeoPosition = this.offsetToGeoPosition(
      currentOffset.sub(this.offsetForLocation),
      currentZoomLevel
    );
    ...
    const newCenterOffset = this.geoPositionToOffset(
      centerGeoPosition,
      newZoomLevel,
      this.referencePositionLocationAsset
    );
