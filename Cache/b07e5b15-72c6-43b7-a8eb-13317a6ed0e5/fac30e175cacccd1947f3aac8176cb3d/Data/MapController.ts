...
export class MapController extends BaseScriptComponent {
   ...
   */
  private getCurrentViewCenterGeoPosition(currentOffset: vec2, zoomLevel: number): GeoPosition {
    ...
    const relativeOffset = centerOffset.sub(this.offsetForLocation);
    
    // Convertir cet offset en coordonnées géographiques
    // en utilisant la même logique que fromLocalPositionToLongLat mais adapté
    ...
    log.i(`Current view center: lat=${latitude.toFixed(6)}, lon=${longitude.toFixed(6)}`);
    return geoPos;
  }
  ...
  handleZoomIn(): void {
    ...
    // 5. Appliquer le nouvel offset (en ajoutant le mapFocusPosition pour le centrage)
    this.updateLocationOffset();
    this.gridView.setOffset(newCenterOffset.add(this.mapParameters.mapFocusPosition));
    
    // 6. Recréer le PinOffsetter avec les nouveaux paramètres
    this.pinOffsetter = PinOffsetter.makeMapLocationOffsetter(
      this.mapModule,
      this.referencePositionLocationAsset
    );
    ...
    // 8. Repositionner tous les pins avec le nouveau zoom
    this.pinSet.forEach((pin: MapPin) => {
      this.pinOffsetter.bindScreenTransformToLocation(
        pin.screenTransform,
        pin.location.longitude,
