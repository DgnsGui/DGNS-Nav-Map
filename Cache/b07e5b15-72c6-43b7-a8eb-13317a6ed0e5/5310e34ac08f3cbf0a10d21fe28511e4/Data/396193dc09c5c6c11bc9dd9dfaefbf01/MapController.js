...
let MapController = (() => {
    ...
    var MapController = _classThis = class extends _classSuper {
         ...
         * Approche simplifiée : on calcule le ratio de changement d'échelle entre les niveaux de zoom.
         */
        handleZoomIn() {
            if (!this.isInitialized) {
            ...
            this.referencePositionLocationAsset = LocationAsset.getGeoAnchoredPosition(this.mapLocation.longitude, this.mapLocation.latitude).location.adjacentTile(0, 0, this.mapParameters.zoomOffet);
            this.northwestLocationAsset = LocationAsset.getGeoAnchoredPosition(MAX_LONGITUDE, MAX_LATITUDE).location.adjacentTile(0, 0, this.mapParameters.zoomOffet);
            // 4. Calculer l'ancien offsetForLocation
            const oldOffsetForLocation = (0, MapUtils_1.getOffsetForLocation)(this.mapModule, this.referencePositionLocationAsset, this.mapLocation.longitude, this.mapLocation.latitude);
            ...
            // 6. Calculer le delta de l'offsetForLocation
            const deltaOffsetForLocation = this.offsetForLocation.sub(oldOffsetForLocation);
            log.i(`Delta offsetForLocation: (${deltaOffsetForLocation.x.toFixed(4)}, ${deltaOffsetForLocation.y.toFixed(4)})`);
            // 7. Le ratio d'échelle entre les niveaux de zoom est 2 (chaque niveau double la résolution)
            const zoomScale = 2;
            // 8. Calculer la position relative du centre par rapport à la position de référence
            ...
            // 9. Appliquer le facteur d'échelle à la position relative
            ...
            // 10. Calculer le nouvel offset
            const newOffset = this.offsetForLocation.add(scaledRelativeOffset).add(this.mapParameters.mapFocusPosition);
            log.i(`New offset to apply: (${newOffset.x.toFixed(4)}, ${newOffset.y.toFixed(4)})`);
            this.gridView.setOffset(newOffset);
            // 11. Recréer le PinOffsetter avec les nouveaux paramètres
            this.pinOffsetter = PinOffsetter_1.PinOffsetter.makeMapLocationOffsetter(this.mapModule, this.referencePositionLocationAsset);
            // 12. Mettre à jour la grille et les pins
            this.gridView.handleUpdateConfig(this.config);
            this.gridView.layoutCells(true);
            // 13. Repositionner tous les pins avec le nouveau zoom
            this.pinSet.forEach((pin) => {
                this.pinOffsetter.bindScreenTransformToLocation(pin.screenTransform, pin.location.longitude, pin.location.latitude);
            });
            // 14. Repositionner le pin utilisateur si présent
            if (this.userPin) {
        ...
        handleZoomOut() {
            ...
            const deltaOffsetForLocation = this.offsetForLocation.sub(oldOffsetForLocation);
            log.i(`Delta offsetForLocation: (${deltaOffsetForLocation.x.toFixed(4)}, ${deltaOffsetForLocation.y.toFixed(4)})`);
            // 7. Le ratio d'échelle entre les niveaux de zoom est 0.5 (on divise par 2)
            const zoomScale = 0.5;
            // 8. Calculer la position relative du centre par rapport à la position de référence
            const relativeOffset = oldOffset.sub(oldOffsetForLocation).sub(this.mapParameters.mapFocusPosition);
            log.i(`Relative offset: (${relativeOffset.x.toFixed(4)}, ${relativeOffset.y.toFixed(4)})`);
            ...
            log.i(`New offset to apply: (${newOffset.x.toFixed(4)}, ${newOffset.y.toFixed(4)})`);
            this.gridView.setOffset(newOffset);
            // 11. Recréer le PinOffsetter
            this.pinOffsetter = PinOffsetter_1.PinOffsetter.makeMapLocationOffsetter(this.mapModule, this.referencePositionLocationAsset);
            // 12. Mettre à jour la grille et les pins
            this.gridView.handleUpdateConfig(this.config);
            this.gridView.layoutCells(true);
            // 13. Repositionner tous les pins
            this.pinSet.forEach((pin) => {
                this.pinOffsetter.bindScreenTransformToLocation(pin.screenTransform, pin.location.longitude, pin.location.latitude);
            });
            // 14. Repositionner le pin utilisateur si présent
