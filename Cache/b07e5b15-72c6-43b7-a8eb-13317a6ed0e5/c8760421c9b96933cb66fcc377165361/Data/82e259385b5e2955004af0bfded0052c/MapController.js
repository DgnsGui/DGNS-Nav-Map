...
let MapController = (() => {
    ...
    var MapController = _classThis = class extends _classSuper {
        ...
        // ===== ZOOM HANDLERS (CORRIGÉS) =====
        updateZoomLevel(newZoomLevel) {
            // 1. Sauvegarder l'état actuel
            const currentOffset = this.gridView.getOffset();
            const currentZoomLevel = this.mapParameters.zoomLevel;
            log.i(`=== ZOOM START ===`);
            log.i(`Current zoom: ${currentZoomLevel}, New zoom: ${newZoomLevel}`);
            log.i(`Current offset: (${currentOffset.x.toFixed(3)}, ${currentOffset.y.toFixed(3)})`);
            // 2. Calculer la position géographique du centre actuel de la vue
            // currentOffset représente où se trouve le centre de l'écran dans l'espace des tuiles
            // On doit soustraire (0.5, 0.5) car offsetForLocation est déjà centré
            const relativeOffset = currentOffset.sub(this.offsetForLocation).sub(new vec2(0.5, 0.5));
            log.i(`Relative offset: (${relativeOffset.x.toFixed(3)}, ${relativeOffset.y.toFixed(3)})`);
            log.i(`Map location: lat=${this.mapLocation.latitude}, lon=${this.mapLocation.longitude}`);
            // Utiliser la fonction existante pour convertir en position géographique
            const centerGeoPosition = this.fromLocalPositionToLongLat(relativeOffset, currentZoomLevel);
            log.i(`Center geo: lat=${centerGeoPosition.latitude.toFixed(6)}, lon=${centerGeoPosition.longitude.toFixed(6)}`);
            ...
            // 4. Recalculer les assets de location pour le nouveau niveau de zoom
            this.referencePositionLocationAsset = LocationAsset.getGeoAnchoredPosition(this.mapLocation.longitude, this.mapLocation.latitude).location.adjacentTile(0, 0, this.mapParameters.zoomOffet);
            this.northwestLocationAsset = LocationAsset.getGeoAnchoredPosition(MAX_LONGITUDE, MAX_LATITUDE).location.adjacentTile(0, 0, this.mapParameters.zoomOffet);
            // 5. Mettre à jour l'offset pour la location de base au nouveau zoom
            this.updateLocationOffset();
            log.i(`New offsetForLocation: (${this.offsetForLocation.x.toFixed(3)}, ${this.offsetForLocation.y.toFixed(3)})`);
            // 6. Recréer le pinOffsetter avec les nouveaux assets
            this.pinOffsetter = PinOffsetter_1.PinOffsetter.makeMapLocationOffsetter(this.mapModule, this.referencePositionLocationAsset);
            // 7. Calculer le nouvel offset pour garder le même centre géographique
            const newCenterOffsetFromMapLocation = (0, MapUtils_1.getOffsetForLocation)(this.mapModule, this.referencePositionLocationAsset, centerGeoPosition.longitude, centerGeoPosition.latitude);
            log.i(`New center offset from map: (${newCenterOffsetFromMapLocation.x.toFixed(3)}, ${newCenterOffsetFromMapLocation.y.toFixed(3)})`);
            // Le nouvel offset absolu = offset du centre + (0.5, 0.5) pour recentrer à l'écran
            const newOffset = newCenterOffsetFromMapLocation.add(this.offsetForLocation).add(new vec2(0.5, 0.5));
            log.i(`Final new offset: (${newOffset.x.toFixed(3)}, ${newOffset.y.toFixed(3)})`);
            log.i(`=== ZOOM END ===`);
            // 8. Appliquer le nouvel offset
            this.gridView.setOffset(newOffset);
            // 9. Mettre à jour la config
            this.gridView.handleUpdateConfig(this.config);
            // 10. Relayout des cellules
            this.gridView.layoutCells(true);
            // 11. Rebind tous les pins à leurs nouvelles positions
            this.pinSet.forEach((pin) => {
                this.pinOffsetter.bindScreenTransformToLocation(pin.screenTransform, pin.location.longitude, pin.location.latitude);
            });
            // 12. Rebind le user pin si présent
            if (this.userPin) {
                this.pinOffsetter.bindScreenTransformToLocation(this.userPin.screenTransform, this.userLocation.longitude, this.userLocation.latitude);
            }
        }
        handleZoomIn() {
            this.updateZoomLevel(this.mapParameters.zoomLevel + 1);
            log.i(`Zoomed IN to level ${this.mapParameters.zoomLevel}`);
        }
        handleZoomOut() {
            this.updateZoomLevel(this.mapParameters.zoomLevel - 1);
            log.i(`Zoomed OUT to level ${this.mapParameters.zoomLevel}`);
        }
        toggleMiniMap(isOn) {
        ...
        showNearbyPlaces(category) {
            ...
            this.placesProvider
                ...
                // Obtenir les infos détaillées des lieux
                this.placesProvider
