...
export class MapController extends BaseScriptComponent {
  ...
  @input
  @hint("Scroll sensitivity multiplier (default: 1.0)")
  @widget(new SliderWidget(0.1, 5.0, 0.1))
  scrollSensitivity: number = 1.0;

  @input
  @hint("Zoom sensitivity multiplier (default: 1.0)")
  @widget(new SliderWidget(0.1, 5.0, 0.1))
  ...
  handleTouchUpdate(localPosition: vec2): void {
    ...
    if (this.isDraggingPin) {
      ...
      // Appliquer la sensibilité du scroll
      const adjustedPosition = new vec2(
        localPosition.x * this.scrollSensitivity,
        localPosition.y * this.scrollSensitivity
      );
      this.gridView.handleScrollUpdate(adjustedPosition);
    }
  }
  ...
  handleZoomIn(): void {
    // Appliquer la sensibilité du zoom
    const zoomSteps = Math.max(1, Math.round(this.zoomSensitivity));
    this.mapParameters.zoomLevel += zoomSteps;
