// --- START OF FILE MapController.ts (FINAL MERGED & CORRECTED VERSION) ---
...
export class MapController extends BaseScriptComponent {
  ...
  handleZoomIn(): void { if (!this.isInitialized) return; const oldOffset = this.gridView.getOffset(); const oldOffsetForLocation = new vec2(this.offsetForLocation.x, this.offsetForLocation.y); this.mapParameters.zoomLevel++; this.mapParameters.zoomOffet = calculateZoomOffset(this.mapParameters.zoomLevel); this.referencePositionLocationAsset = LocationAsset.getGeoAnchoredPosition(this.mapLocation.longitude, this.mapLocation.latitude).location.adjacentTile(0, 0, this.mapParameters.zoomOffet); this.northwestLocationAsset = LocationAsset.getGeoAnchoredPosition(MAX_LONGITUDE, MAX_LATITUDE).location.adjacentTile(0, 0, this.mapParameters.zoomOffet); this.updateLocationOffset(); const relativeOffset = oldOffset.sub(oldOffsetForLocation).sub(this.mapParameters.mapFocusPosition); const scaledRelativeOffset = relativeOffset.uniformScale(2); const newOffset = this.offsetForLocation.add(scaledRelativeOffset).add(this.mapParameters.mapFocusPosition); this.gridView.setOffset(newOffset); this.pinOffsetter = PinOffsetter.makeMapLocationOffsetter(this.mapModule, this.referencePositionLocationAsset); this.gridView.handleUpdateConfig(this.config); this.gridView.layoutCells(true); this.pinSet.forEach((pin: MapPin) => this.pinOffsetter.bindScreenTransformToLocation(pin.screenTransform, pin.location.longitude, pin.location.latitude)); if (this.userPin) this.pinOffsetter.bindScreenTransformToLocation(this.userPin.screenTransform, this.userPin.location.longitude, this.userPin.location.latitude); this.pinOffsetter.layoutScreenTransforms(this.gridView); }
  handleZoomOut(): void { if (!this.isInitialized || this.mapParameters.zoomLevel <= 0) return; const oldOffset = this.gridView.getOffset(); const oldOffsetForLocation = new vec2(this.offsetForLocation.x, this.offsetForLocation.y); this.mapParameters.zoomLevel--; this.mapParameters.zoomOffet = calculateZoomOffset(this.mapParameters.zoomLevel); this.referencePositionLocationAsset = LocationAsset.getGeoAnchoredPosition(this.mapLocation.longitude, this.mapLocation.latitude).location.adjacentTile(0, 0, this.mapParameters.zoomOffet); this.northwestLocationAsset = LocationAsset.getGeoAnchoredPosition(MAX_LONGITUDE, MAX_LATITUDE).location.adjacentTile(0, 0, this.mapParameters.zoomOffet); this.updateLocationOffset(); const relativeOffset = oldOffset.sub(oldOffsetForLocation).sub(this.mapParameters.mapFocusPosition); const scaledRelativeOffset = relativeOffset.uniformScale(0.5); const newOffset = this.offsetForLocation.add(scaledRelativeOffset).add(this.mapParameters.mapFocusPosition); this.gridView.setOffset(newOffset); this.pinOffsetter = PinOffsetter.makeMapLocationOffsetter(this.mapModule, this.referencePositionLocationAsset); this.gridView.handleUpdateConfig(this.config); this.gridView.layoutCells(true); this.pinSet.forEach((pin: MapPin) => this.pinOffsetter.bindScreenTransformToLocation(pin.screenTransform, pin.location.longitude, pin.location.latitude)); if (this.userPin) this.pinOffsetter.bindScreenTransformToLocation(this.userPin.screenTransform, this.userPin.location.longitude, this.userPin.location.latitude); this.pinOffsetter.layoutScreenTransforms(this.gridView); }
  toggleMiniMap(isOn: boolean): void { if (!this.gridView) return; this.config.gridScreenTransform.rotation = quat.quatIdentity(); this.gridView.toggleMiniMap(isOn, this.pinSet, this.userPin); if (!isOn) this.pinSet.forEach((pin: MapPin) => pin.screenTransform.rotation = quat.quatIdentity()); this.onMiniMapToggledEvent.invoke(isOn); }
  ...
  showNearbyPlaces(category: string[] | null): void {
    ...
    if (!this.placesProvider) { log.e("PlacesProvider not assigned!"); this.onNearbyPlacesFailedEvent.invoke(); return; }
    
    // CORRECTION FINALE : Utiliser this.userLocation comme le fait le sample.
    const searchLocation = this.userLocation;
    
    if (!searchLocation) { 
        log.e("User location not available yet for search. Trying again shortly...");
        this.onNearbyPlacesFailedEvent.invoke(); // Déclenche le fallback immédiatement pour ne pas bloquer l'utilisateur
        return; 
    }
    ...
    this.placesProvider.getNearbyPlaces(searchLocation, NEARBY_PLACES_RANGE, category)
      .then((places) => {
        ...
        this.placesProvider.getPlacesInfo(places)
          ...
          .catch((error) => { log.e(`Failed to get places info: ${error}`); this.onNearbyPlacesFailedEvent.invoke(); });
      })
      .catch((error) => { log.e(`Failed to get nearby places: ${error}`); this.onNearbyPlacesFailedEvent.invoke(); });
  }

  drawGeometryPoint(geometryPoint: vec2, radius: number = 0.1) { /* ... code original ... */ }
  drawGeometryLine(geometryLine: vec2[], thickness: number = 0.2) { /* ... code original ... */ }
  drawGeometryMultiline(geometryMultiline: any, thickness: number = 0.2) { /* ... code original ... */ }
  clearGeometry(): void { /* ... code original ... */ }
  getWorldPositionForGeometryPoint(geometryPoint: vec2) { /* ... code original ... */ return vec3.zero(); }
  onContentMaskRenderLayer(renderLayer: any) { /* ... code original ... */ }
  onScrollingStarted() { /* ... code original ... */ }
  onLayout() { /* ... code original ... */ }
}