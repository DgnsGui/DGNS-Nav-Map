...
let MapController = (() => {
    ...
    var MapController = _classThis = class extends _classSuper {
        ...
        handleNorthAlignedOrientationUpdate(orientation) { this.orientation = orientation; this.heading = (0, MapUtils_1.normalizeAngle)((0, MapUtils_1.customGetEuler)(orientation).y); }
        updateRotations() {
        ...
        updateMapPinRotations(pinRotation) {
            if (this.mapParameters.mapPinsRotated) {
                ...
                if (this.mapParameters.enableMapSmoothing) {
                    this.currentPinRotation = (0, MapUtils_1.interpolate)(this.currentPinRotation, targetRotation, 4);
                    this.pinSet.forEach((pin) => { if (pin.screenTransform)
                        pin.screenTransform.rotation = this.currentPinRotation; });
                }
                else {
                    this.pinSet.forEach((pin) => { if (pin.screenTransform)
                        pin.screenTransform.rotation = targetRotation; });
                }
            }
        }
        updateMapRotation() {
            ...
            if (this.mapParameters.enableMapSmoothing) {
                this.currentMapRotation = (0, MapUtils_1.interpolate)(this.currentMapRotation, targetRotation, 4);
                if (this.config)
                    this.config.gridScreenTransform.rotation = this.currentMapRotation;
            }
            else {
                if (this.config)
                    this.config.gridScreenTransform.rotation = targetRotation;
            }
        }
        updateUserPinRotation(pinRotation) {
            if (this.userPin && this.userPin.screenTransform && this.mapParameters.userPinAlignedWithOrientation) {
                const targetRotation = quat.fromEulerAngles(0, 0, pinRotation);
                if (this.mapParameters.enableMapSmoothing) {
                    this.targetUserRotation = (0, MapUtils_1.interpolate)(this.targetUserRotation, targetRotation, 4);
        ...
        handleZoomIn() { if (!this.isInitialized)
            return; const oldOffset = this.gridView.getOffset(); const oldOffsetForLocation = new vec2(this.offsetForLocation.x, this.offsetForLocation.y); this.mapParameters.zoomLevel++; this.mapParameters.zoomOffet = (0, MapUtils_1.calculateZoomOffset)(this.mapParameters.zoomLevel); this.referencePositionLocationAsset = LocationAsset.getGeoAnchoredPosition(this.mapLocation.longitude, this.mapLocation.latitude).location.adjacentTile(0, 0, this.mapParameters.zoomOffet); this.northwestLocationAsset = LocationAsset.getGeoAnchoredPosition(MAX_LONGITUDE, MAX_LATITUDE).location.adjacentTile(0, 0, this.mapParameters.zoomOffet); this.updateLocationOffset(); const relativeOffset = oldOffset.sub(oldOffsetForLocation).sub(this.mapParameters.mapFocusPosition); const scaledRelativeOffset = relativeOffset.uniformScale(2); const newOffset = this.offsetForLocation.add(scaledRelativeOffset).add(this.mapParameters.mapFocusPosition); this.gridView.setOffset(newOffset); this.pinOffsetter = PinOffsetter_1.PinOffsetter.makeMapLocationOffsetter(this.mapModule, this.referencePositionLocationAsset); this.gridView.handleUpdateConfig(this.config); this.gridView.layoutCells(true); this.pinSet.forEach((pin) => this.pinOffsetter.bindScreenTransformToLocation(pin.screenTransform, pin.location.longitude, pin.location.latitude)); if (this.userPin)
            this.pinOffsetter.bindScreenTransformToLocation(this.userPin.screenTransform, this.userPin.location.longitude, this.userPin.location.latitude); this.pinOffsetter.layoutScreenTransforms(this.gridView); }
        handleZoomOut() { if (!this.isInitialized || this.mapParameters.zoomLevel <= 0)
            return; const oldOffset = this.gridView.getOffset(); const oldOffsetForLocation = new vec2(this.offsetForLocation.x, this.offsetForLocation.y); this.mapParameters.zoomLevel--; this.mapParameters.zoomOffet = (0, MapUtils_1.calculateZoomOffset)(this.mapParameters.zoomLevel); this.referencePositionLocationAsset = LocationAsset.getGeoAnchoredPosition(this.mapLocation.longitude, this.mapLocation.latitude).location.adjacentTile(0, 0, this.mapParameters.zoomOffet); this.northwestLocationAsset = LocationAsset.getGeoAnchoredPosition(MAX_LONGITUDE, MAX_LATITUDE).location.adjacentTile(0, 0, this.mapParameters.zoomOffet); this.updateLocationOffset(); const relativeOffset = oldOffset.sub(oldOffsetForLocation).sub(this.mapParameters.mapFocusPosition); const scaledRelativeOffset = relativeOffset.uniformScale(0.5); const newOffset = this.offsetForLocation.add(scaledRelativeOffset).add(this.mapParameters.mapFocusPosition); this.gridView.setOffset(newOffset); this.pinOffsetter = PinOffsetter_1.PinOffsetter.makeMapLocationOffsetter(this.mapModule, this.referencePositionLocationAsset); this.gridView.handleUpdateConfig(this.config); this.gridView.layoutCells(true); this.pinSet.forEach((pin) => this.pinOffsetter.bindScreenTransformToLocation(pin.screenTransform, pin.location.longitude, pin.location.latitude)); if (this.userPin)
            this.pinOffsetter.bindScreenTransformToLocation(this.userPin.screenTransform, this.userPin.location.longitude, this.userPin.location.latitude); this.pinOffsetter.layoutScreenTransforms(this.gridView); }
        toggleMiniMap(isOn) { if (!this.gridView)
            return; this.config.gridScreenTransform.rotation = quat.quatIdentity(); this.gridView.toggleMiniMap(isOn, this.pinSet, this.userPin); if (!isOn)
            this.pinSet.forEach((pin) => pin.screenTransform.rotation = quat.quatIdentity()); this.onMiniMapToggledEvent.invoke(isOn); }
        ...
        clearGeometry() { }
        getWorldPositionForGeometryPoint(geometryPoint) { /* ... code original ... */ return vec3.zero(); }
