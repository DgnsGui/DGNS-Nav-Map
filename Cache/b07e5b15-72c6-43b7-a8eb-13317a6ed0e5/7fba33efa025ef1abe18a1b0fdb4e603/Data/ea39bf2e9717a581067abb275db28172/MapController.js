...
let MapController = (() => {
    ...
    var MapController = _classThis = class extends _classSuper {
         ...
         * IMPORTANT: Doit être appelée AVANT toute modification du zoom avec les valeurs sauvegardées.
         *
         * @param currentOffset - L'offset actuel de la grille (vec2)
         * @param zoomLevel - Le niveau de zoom actuel (AVANT modification)
         * @param mapLocation - La position de référence de la carte (AVANT modification)
         * @param northwestAsset - L'asset northwest (AVANT modification)
         * @returns GeoPosition - Les coordonnées géographiques correspondant au centre de la vue
         */
        getCurrentViewCenterGeoPosition(currentOffset, zoomLevel, mapLocation, northwestAsset) {
            // Le centre de la vue correspond à l'offset - mapFocusPosition (qui est normalement 0.5, 0.5)
            const centerLocalPosition = currentOffset.sub(this.mapParameters.mapFocusPosition);
            log.i(`Center local position: (${centerLocalPosition.x.toFixed(4)}, ${centerLocalPosition.y.toFixed(4)})`);
            // Calculer manuellement avec les valeurs sauvegardées
            const pixelOffsetFromMapLocationX = centerLocalPosition.x * TEXTURE_SIZE;
            const pixelOffsetFromMapLocationY = -centerLocalPosition.y * TEXTURE_SIZE;
            const mapImageOffset = this.mapModule.longLatToImageRatio(mapLocation.longitude, mapLocation.latitude, northwestAsset);
            const pixelX = mapImageOffset.x * TEXTURE_SIZE + pixelOffsetFromMapLocationX;
            const pixelY = mapImageOffset.y * TEXTURE_SIZE + pixelOffsetFromMapLocationY;
            const mapSize = TEXTURE_SIZE << zoomLevel;
            const x = (0, MapUtils_1.clip)(pixelX, 0, mapSize - 1) / mapSize - 0.5;
            ...
            geoPos.altitude = mapLocation.altitude || 0;
            log.i(`View center at zoom ${zoomLevel}: lat=${latitude.toFixed(6)}, lon=${longitude.toFixed(6)}`);
            return geoPos;
        }
        /**
         * Zoom In - Conserve le centre de la vue actuelle lors du zoom avec animation fluide.
         */
        handleZoomIn() {
            if (!this.isInitialized) {
            ...
            // 1. Capturer TOUTES les valeurs AVANT toute modification
            const oldOffset = this.gridView.getOffset();
            ...
            log.i(`Old offset: (${oldOffset.x.toFixed(4)}, ${oldOffset.y.toFixed(4)})`);
            log.i(`Old offsetForLocation: (${oldOffsetForLocation.x.toFixed(4)}, ${oldOffsetForLocation.y.toFixed(4)})`);
            ...
            log.i(`New zoom level: ${this.mapParameters.zoomLevel}`);
            // 3. Recalculer les assets de référence avec le nouveau zoom
            this.referencePositionLocationAsset = LocationAsset.getGeoAnchoredPosition(this.mapLocation.longitude, this.mapLocation.latitude).location.adjacentTile(0, 0, this.mapParameters.zoomOffet);
            this.northwestLocationAsset = LocationAsset.getGeoAnchoredPosition(MAX_LONGITUDE, MAX_LATITUDE).location.adjacentTile(0, 0, this.mapParameters.zoomOffet);
            // 4. Mettre à jour offsetForLocation avec le nouveau zoom
            this.updateLocationOffset();
            log.i(`New offsetForLocation: (${this.offsetForLocation.x.toFixed(4)}, ${this.offsetForLocation.y.toFixed(4)})`);
            // 5. Le ratio d'échelle entre les niveaux de zoom est 2
            const zoomScale = 2;
            // 6. Calculer la position relative du centre par rapport à la position de référence
            const relativeOffset = oldOffset.sub(oldOffsetForLocation).sub(this.mapParameters.mapFocusPosition);
            log.i(`Relative offset: (${relativeOffset.x.toFixed(4)}, ${relativeOffset.y.toFixed(4)})`);
            // 7. Appliquer le facteur d'échelle à la position relative
            const scaledRelativeOffset = relativeOffset.uniformScale(zoomScale);
            log.i(`Scaled relative offset: (${scaledRelativeOffset.x.toFixed(4)}, ${scaledRelativeOffset.y.toFixed(4)})`);
            // 8. Calculer le nouvel offset cible
            const targetOffset = this.offsetForLocation.add(scaledRelativeOffset).add(this.mapParameters.mapFocusPosition);
            log.i(`Target offset: (${targetOffset.x.toFixed(4)}, ${targetOffset.y.toFixed(4)})`);
            // 9. Recréer le PinOffsetter avec les nouveaux paramètres
            this.pinOffsetter = PinOffsetter_1.PinOffsetter.makeMapLocationOffsetter(this.mapModule, this.referencePositionLocationAsset);
            // 10. Mettre à jour la grille
            this.gridView.handleUpdateConfig(this.config);
            this.gridView.layoutCells(true);
            // 11. Repositionner tous les pins avec le nouveau zoom
            this.pinSet.forEach((pin) => {
                this.pinOffsetter.bindScreenTransformToLocation(pin.screenTransform, pin.location.longitude, pin.location.latitude);
            });
            // 12. Repositionner le pin utilisateur si présent
            if (this.userPin) {
            ...
            // 13. Animer le zoom
            this.isZooming = true;
            if (this.zoomTweenCancelFunction) {
                this.zoomTweenCancelFunction();
            }
            this.zoomTweenCancelFunction = (0, MapUtils_1.makeTween)((t) => {
                // Interpolation avec easing (ease-out pour un effet plus naturel)
                const easedT = 1 - Math.pow(1 - t, 3); // cubic ease-out
                // Interpoler l'offset
                const currentOffset = vec2.lerp(oldOffset, targetOffset, easedT);
                this.gridView.setOffset(currentOffset);
                if (t === 1) {
                    this.isZooming = false;
                    log.i(`=== ZOOM IN COMPLETE ===`);
                }
            }, 0.3); // 300ms d'animation
        }
        /**
         * Zoom Out - Conserve le centre de la vue actuelle lors du dézoom avec animation fluide.
         */
        handleZoomOut() {
            if (!this.isInitialized) {
                log.w("Cannot zoom: map not initialized");
            ...
            const oldZoomLevel = this.mapParameters.zoomLevel;
            const oldOffsetForLocation = new vec2(this.offsetForLocation.x, this.offsetForLocation.y);
            const oldScale = this.config.gridScreenTransform.scale;
            log.i(`Old offset: (${oldOffset.x.toFixed(4)}, ${oldOffset.y.toFixed(4)})`);
            log.i(`Old offsetForLocation: (${oldOffsetForLocation.x.toFixed(4)}, ${oldOffsetForLocation.y.toFixed(4)})`);
            // 2. Changer le niveau de zoom
            this.mapParameters.zoomLevel--;
            ...
            this.updateLocationOffset();
            log.i(`New offsetForLocation: (${this.offsetForLocation.x.toFixed(4)}, ${this.offsetForLocation.y.toFixed(4)})`);
            // 5. Le ratio d'échelle entre les niveaux de zoom est 0.5 (on divise par 2)
            const zoomScale = 0.5;
            // 6. Calculer la position relative du centre par rapport à la position de référence
            const relativeOffset = oldOffset.sub(oldOffsetForLocation).sub(this.mapParameters.mapFocusPosition);
            log.i(`Relative offset: (${relativeOffset.x.toFixed(4)}, ${relativeOffset.y.toFixed(4)})`);
            ...
            const targetOffset = this.offsetForLocation.add(scaledRelativeOffset).add(this.mapParameters.mapFocusPosition);
            log.i(`Target offset: (${targetOffset.x.toFixed(4)}, ${targetOffset.y.toFixed(4)})`);
            // 9. Recréer le PinOffsetter
            this.pinOffsetter = PinOffsetter_1.PinOffsetter.makeMapLocationOffsetter(this.mapModule, this.referencePositionLocationAsset);
            // 10. Mettre à jour la grille et les pins
            this.gridView.handleUpdateConfig(this.config);
            this.gridView.layoutCells(true);
            // 11. Repositionner tous les pins
            this.pinSet.forEach((pin) => {
                this.pinOffsetter.bindScreenTransformToLocation(pin.screenTransform, pin.location.longitude, pin.location.latitude);
            });
            // 12. Repositionner le pin utilisateur si présent
            ...
            this.pinOffsetter.layoutScreenTransforms(this.gridView);
            // 13. Animer le zoom avec scale et offset
            this.isZooming = true;
            if (this.zoomTweenCancelFunction) {
                this.zoomTweenCancelFunction();
            }
            // Commencer avec l'ancienne échelle (2x) pour simuler le zoom out
            this.config.gridScreenTransform.scale = oldScale.uniformScale(2.0);
            this.zoomTweenCancelFunction = (0, MapUtils_1.makeTween)((t) => {
                // Interpolation avec easing (ease-out pour un effet plus naturel)
                const easedT = 1 - Math.pow(1 - t, 3); // cubic ease-out
                // Animer l'échelle de 2.0 à 1.0
                const currentScale = (0, MapUtils_1.lerp)(2.0, 1.0, easedT);
                this.config.gridScreenTransform.scale = oldScale.uniformScale(currentScale);
                // Interpoler l'offset
                const currentOffset = vec2.lerp(oldOffset, targetOffset, easedT);
                this.gridView.setOffset(currentOffset);
                if (t === 1) {
                    this.config.gridScreenTransform.scale = oldScale;
                    this.isZooming = false;
