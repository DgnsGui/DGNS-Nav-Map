// --- START OF FILE MapController.ts (FINAL API FIX VERSION) ---
...
export class MapController extends BaseScriptComponent {
  ...
  private updateMapPinRotations(pinRotation: number) {
    if (this.mapParameters.mapPinsRotated) {
      ...
      if (this.mapParameters.enableMapSmoothing) {
        ...
        this.pinSet.forEach((pin: MapPin) => pin.screenTransform.rotation = targetRotation);
      }
    }
  }
  ...
  handleZoomIn(): void { /* ... code original ... */ }
  handleZoomOut(): void { /* ... code original ... */ }
  toggleMiniMap(isOn: boolean): void { /* ... code original ... */ }
  ...
  centerMap(): void { /* ... code original ... */ }
  isMapCentered(): boolean { /* ... code original ... */ return false; }
  getPositionWithMapRotationOffset(localPosition: vec2): vec2 { /* ... code original ... */ return vec2.zero(); }
  ...
  showNearbyPlaces(category: string[] | null): void {
    ...
    // Modification cruciale : on passe 'null' comme argument de localisation
    // pour laisser le système natif de Snap gérer la géolocalisation.
    log.i(`Searching near device's automatic location...`);
