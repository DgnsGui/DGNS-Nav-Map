...
// --- START OF FILE MapController.ts (FINAL RESTORED & CORRECTED VERSION) ---
...
let MapController = (() => {
    ...
    var MapController = _classThis = class extends _classSuper {
        ...
        handleZoomOut() {
            if (!this.isInitialized || this.mapParameters.zoomLevel <= 0)
                return;
            const oldOffset = this.gridView.getOffset();
            const oldOffsetForLocation = new vec2(this.offsetForLocation.x, this.offsetForLocation.y);
            this.mapParameters.zoomLevel--;
        ...
        onContentMaskRenderLayer(renderLayer) {
            (0, MapUtils_1.forEachSceneObjectInSubHierarchy)(this.mapPinsAnchor, (sceneObject) => { sceneObject.layer = renderLayer; });
        }
        onScrollingStarted() {
            this.shouldFollowMapLocation = false;
