...
export class MapComponent extends BaseScriptComponent {
  ...
  private cleanupOldPlacesData(): void {
    ...
    // Supprimer les anciens pins de la carte
    if (this.placesData.length > 0) {
      this.placesData.forEach(place => {
        if (place.pin) {
          try {
            this.mapController.removeMapPin(place.pin);
          } catch (e) {
  ...
  // La signature de cette fonction a été changée pour ne plus inclure 'color'
  drawGeometryPoint(geometry: any, radius: number): any {
    // CORRECTION: L'API attend 'geometry' et 'radius' comme deux arguments séparés.
    return this.mapController.drawGeometryPoint(geometry, radius);
  }

  // La signature de cette fonction a été changée pour ne plus inclure 'color'
  drawGeometryLine(geometry: any, width: number): any {
    // CORRECTION: L'API attend 'geometry' et 'width' comme deux arguments séparés.
    return this.mapController.drawGeometryLine(geometry, width);
  }

  drawGeometryPolygon(geometry: any, color: vec4, outlineWidth: number, outlineColor: vec4): any {
    // Cette fonction ne causait pas d'erreur, on garde la structure précédente avec les options
    return (this.mapController as any).drawGeometryPolygon(geometry, { color: color, outlineWidth: outlineWidth, outlineColor: outlineColor });
  }
  
  removeGeometry(geometry: any): void {
    // On garde le contournement de l'erreur de type
    (this.mapController as any).removeGeometry(geometry);
  }
}