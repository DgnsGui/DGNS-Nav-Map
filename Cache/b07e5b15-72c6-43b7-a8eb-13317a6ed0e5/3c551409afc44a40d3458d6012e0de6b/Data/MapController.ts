...
export class MapController extends BaseScriptComponent {
  ...
  handleZoomOut(): void {
    ...
    const scaledRelativeOffset = relativeOffset.
    uniformScale(zoomScale);
    log.i(`Scaled relative offset: (${scaledRelativeOffset.x.toFixed(4)}, ${scaledRelativeOffset.y.toFixed(4)})`);
  ...
  log.i(`=== showNearbyPlaces FIXED VERSION ===`);
  log.i(`Categories: ${category ? category.join(", ") : "ALL"}`);
...
log.i(`🔍 USER LOCATION: ${this.userLocation ? this.userLocation.latitude + ',' + this.userLocation.longitude : 'NULL'}`);
log.i(`🗺️ MAP LOCATION: ${this.mapLocation ? this.mapLocation.latitude + ',' + this.mapLocation.longitude : 'NULL'}`);
log.i(`📍 FINAL SEARCH LOCATION: ${searchLocation.latitude},${searchLocation.longitude}`);

log.i(`Searching near: lat=${searchLocation.latitude}, lon=${searchLocation.longitude}`);
  // CHANGEMENT CRITIQUE: Utiliser getNearbyPlacesInfo au lieu de getNearbyPlaces
  this.placesProvider
    .getNearbyPlacesInfo(searchLocation, NEARBY_PLACES_RANGE, 1000, category)
    .then((placesInfo: PlaceInfo[]) => {
      log.i(`✅ Got ${placesInfo.length} places with full info`);
      ...
      for (let i = 0; i < placesInfo.length; i++) {
        if (!this.pinnedPlaceSet.has(placesInfo[i].placeId)) {
          log.i(`📍 Creating pin ${i}: ${placesInfo[i].name} at ${placesInfo[i].centroid.latitude},${placesInfo[i].centroid.longitude}`);
          this.createMapPin(placesInfo[i].centroid, placesInfo[i]);
          this.pinnedPlaceSet.add(placesInfo[i].placeId);
          addedCount++;
        } else {
          log.i(`⏭️ Place already pinned: ${placesInfo[i].name}`);
        }
      }
    ...
    .catch((error) => {
      log.e(`❌ Failed to get places: ${error}`);
      this.onNearbyPlacesFailedEvent.invoke();
    });
}
  ...
  /**
   * Ask AI about the current map view
   */
  askAIAboutCurrentView(): void {
    if (!this.aiAssistant) {
      log.e("AI Assistant is not assigned!");
      return;
    }

    if (!this.isInitialized) {
      log.w("Map not initialized yet");
      return;
    }

    log.i("Requesting AI analysis of current map view...");
   ...
   */
  askAIAboutLocation(latitude: number, longitude: number): void {
    if (!this.aiAssistant) {
      log.e("AI Assistant is not assigned!");
      return;
    }

    log.i(`Requesting AI analysis of location: ${latitude}, ${longitude}`);
    this.aiAssistant.askAboutLocation(latitude, longitude);
  }

  /**
   * Ask AI about current pins
   */
  askAIAboutPins(): void {
    if (!this.aiAssistant) {
      log.e("AI Assistant is not assigned!");
    ...
    if (this.pinSet.size === 0) {
      log.w("No pins on the map to analyze");
      return;
    }

    // Convert pins to PlaceInfo array
    const placesInfo: PlaceInfo[] = [];
    this.pinSet.forEach((pin: MapPin) => {
      if (pin.placeInfo) {
        placesInfo.push(pin.placeInfo);
      }
    });

    if (placesInfo.length === 0) {
      log.w("No pins with place info to analyze");
      return;
    }

    log.i(`Requesting AI analysis of ${placesInfo.length} pins`);
    this.aiAssistant.askAboutPlaces(placesInfo);
  }

  /**
   * Get AI Assistant reference
   */
  getAIAssistant(): AIMapAssistant {
