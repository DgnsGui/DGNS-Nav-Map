...
let MapController = class MapController extends BaseScriptComponent {
    ...
    /**
     * Applique la transformation inverse de la rotation de la carte aux coordonnées
     */
    applyInverseMapRotation(localPosition) {
        if (!this.mapParameters.isMinimapAutoRotate || this.viewScrolled || !this.config.isMiniMap) {
            return localPosition;
        }
        const mapRotationAngle = (0, MapUtils_1.customGetEuler)(this.config.gridScreenTransform.rotation).z;
        const cos = Math.cos(-mapRotationAngle);
        const sin = Math.sin(-mapRotationAngle);
        return new vec2(localPosition.x * cos - localPosition.y * sin, localPosition.x * sin + localPosition.y * cos);
    }
    handleHoverUpdate(localPosition) {
        ...
        // Appliquer la correction de rotation pour le hover
        const correctedPosition = this.applyInverseMapRotation(localPosition.uniformScale(0.5));
        const adjustedAnchoredPosition = this.getPositionWithMapRotationOffset(correctedPosition);
        this.pinSet.forEach((pin) => {
    ...
    handleTouchStart(localPosition) {
        ...
        if (this.hoveringPinSet.size > 0) {
            ...
            // Appliquer la correction de rotation pour le scroll
            const correctedPosition = this.applyInverseMapRotation(localPosition);
            this.gridView.handleScrollStart(correctedPosition);
        }
    }
    handleTouchUpdate(localPosition) {
        ...
        if (this.isDraggingPin) {
            ...
            // Appliquer la correction de rotation pour le scroll
            const correctedPosition = this.applyInverseMapRotation(localPosition);
            this.gridView.handleScrollUpdate(correctedPosition);
        }
    }
