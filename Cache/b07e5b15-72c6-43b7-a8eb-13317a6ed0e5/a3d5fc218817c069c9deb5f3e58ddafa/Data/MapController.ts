// --- START OF FILE MapController.ts (FINAL with Correct Pin Dragging Logic) ---
...
export class MapController extends BaseScriptComponent {
  ...
  pinDragSensitivity: number = 1.0; // Remis à 1.0 pour une manipulation plus directe
  ...
  // NOUVEAU : Variables pour un déplacement de pin plus stable
  ...
  public onInitialLocationSetEvent = new Event<GeoPosition>();
  public onInitialLocationSet = this.onInitialLocationSetEvent.publicApi();
  public onMapTilesLoadedEvent = new Event();
  public onMapTilesLoaded = this.onMapTilesLoadedEvent.publicApi();
  // ... (autres déclarations d'événements)
  public onMapPinAddedEvent = new Event<MapPin>();
  public onMapPinAdded = this.onMapPinAddedEvent.publicApi();
  public onMapPinRemovedEvent = new Event<MapPin>();
  public onMapPinRemoved = this.onMapPinRemovedEvent.publicApi();
  // ...

  // (Le début du script reste inchangé jusqu'à handleHoverUpdate)
  // ...
  ...
  // --- LOGIQUE DE MANIPULATION ENTIÈREMENT REVUE ---

  handleTouchStart(localPosition: vec2): void {
    if (!this.isInitialized) return;

    if (this.hoveringPinSet.size > 0) {
        // Le clic est sur un pin. On NE commence PAS un scroll de la carte.
        ...
        if (this.lastClickedPin === hoveredPin && (currentTime - this.lastPinClickTime) < DOUBLE_CLICK_THRESHOLD_MS) {
            ...
            // C'est un simple clic. On prépare le potentiel double-clic.
            this.lastClickedPin = hoveredPin;
            this.lastPinClickTime = currentTime;
        }
    } else {
        // Le clic est sur la carte vide. ON COMMENCE un scroll.
  ...
  handleTouchEnd(localPosition: vec2): void {
    ...
    if (this.isDraggingPin && this.draggingPin) {
      // On finalise la position du pin.
      ...
      // On finalise le scroll de la carte.
  ...
  private updateMapPinRotations(pinRotation: number) {
    if (this.mapParameters.mapPinsRotated) {
      ...
      this.pinSet.forEach((pin: MapPin) => {
          pin.screenTransform.rotation = this.mapParameters.enableMapSmoothing ?
              quat.slerp(pin.screenTransform.rotation, targetRot, 4 * getDeltaTime()) :
              targetRot;
      });
    }
  }
  private updateMapRotation() {
    ...
    this.config.gridScreenTransform.rotation = this.mapParameters.enableMapSmoothing ?
        quat.slerp(this.config.gridScreenTransform.rotation, targetRot, 4 * getDeltaTime()) :
        targetRot;
  }
  private updateUserPinRotation(pinRotation: number) {
    if (this.userPin && this.userPin.screenTransform && this.mapParameters.userPinAlignedWithOrientation) {
        const targetRot = quat.fromEulerAngles(0, 0, pinRotation);
        this.userPin.screenTransform.rotation = this.mapParameters.enableMapSmoothing ?
            quat.slerp(this.userPin.screenTransform.rotation, targetRot, 4 * getDeltaTime()) :
            targetRot;
    }
  }
