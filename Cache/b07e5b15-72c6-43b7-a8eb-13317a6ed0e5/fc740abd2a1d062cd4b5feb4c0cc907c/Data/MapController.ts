...
export class MapController extends BaseScriptComponent {
   ...
   * Utilise la fonction fromLocalPositionToLongLat existante pour garantir la cohérence des calculs.
   * 
  ...
  private getCurrentViewCenterGeoPosition(
    ...
    // Utiliser la fonction existante pour garantir la cohérence des calculs
    const geoPos = this.fromLocalPositionToLongLat(centerLocalPosition, zoomLevel);
    
    log.i(`View center at zoom ${zoomLevel}: lat=${geoPos.latitude.toFixed(6)}, lon=${geoPos.longitude.toFixed(6)}`);
  ...
  handleZoomIn(): void {
    ...
    // Utilise la fonction fromLocalPositionToLongLat pour garantir la cohérence
