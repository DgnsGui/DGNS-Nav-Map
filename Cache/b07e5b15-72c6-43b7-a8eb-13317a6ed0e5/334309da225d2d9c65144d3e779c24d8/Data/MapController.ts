...
export class MapController extends BaseScriptComponent {
  ...
  handleHoverUpdate(localPosition: vec2): void {
    ...
    // Créer un set temporaire pour les pins actuellement survolés
    const currentlyHoveringPins = new Set<MapPin>();
    ...
    this.pinSet.forEach((pin: MapPin) => {
      ...
      if (isHoveringPin) {
        currentlyHoveringPins.add(pin);
        // Activer l'outline seulement si ce n'était pas déjà survolé
        if (!this.hoveringPinSet.has(pin)) {
          log.i("Pin hovered: " + pin.sceneObject.name);
          pin.enableOutline(true);
        }
      }
    });

    // Désactiver l'outline des pins qui ne sont plus survolés
    this.hoveringPinSet.forEach((pin: MapPin) => {
      if (!currentlyHoveringPins.has(pin)) {
        log.i("Pin exit hover: " + pin.sceneObject.name);
        pin.enableOutline(false);
      }
    });

    // Mettre à jour le set des pins survolés
    this.hoveringPinSet = currentlyHoveringPins;
  }
  ...
  handleTouchStart(localPosition: vec2): void {
    ...
    if (this.hoveringPinSet.size > 0) {
      log.i("Touch start on pin - starting drag");
      // Prendre le premier pin du set
      for (let pin of this.hoveringPinSet.values()) {
        this.draggingPin = pin;
        break;
      }
      this.isDraggingPin = true;
      // Désactiver l'outline pendant le drag
  ...
  handleTouchEnd(localPosition: vec2): void {
    ...
    if (this.isDraggingPin && this.draggingPin) {
      localPosition = localPosition.uniformScale(0.5);
      const adjustedAnchoredPosition = this.getPositionWithMapRotationOffset(localPosition);
      log.i(`Touch end - dropping pin at: ${adjustedAnchoredPosition}`);

      // Repositionner le pin à la nouvelle location
      this.setPinLocation(this.draggingPin, adjustedAnchoredPosition.uniformScale(0.5));

      // S'assurer que le child est visible
      if (this.draggingPin.sceneObject.getChildrenCount() > 0) {
        this.draggingPin.sceneObject.getChild(0).enabled = true;
      }

      // IMPORTANT: Nettoyer l'état AVANT de réactiver le hover
      const releasedPin = this.draggingPin;
      this.draggingPin = null;
      this.isDraggingPin = false;

      // Vider le set de hover pour forcer une nouvelle détection
      this.hoveringPinSet.clear();

      // Réactiver l'outline si le curseur est toujours sur le pin
      const pinCenter = releasedPin.screenTransform.anchors.getCenter();
      if (adjustedAnchoredPosition.distance(pinCenter) < this.mapParameters.mapPinCursorDetectorSize) {
        this.hoveringPinSet.add(releasedPin);
        releasedPin.enableOutline(true);
        log.i("Pin still hovered after drop");
      }
