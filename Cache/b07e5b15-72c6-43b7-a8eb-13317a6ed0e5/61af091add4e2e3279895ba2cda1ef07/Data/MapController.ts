...
export class MapController extends BaseScriptComponent {
  ...
  @input
  @hint("Active le suivi automatique de la position utilisateur")
  enableAutoFollowLocation: boolean = true;

  @input
  @hint("Active le recentrage automatique de la carte")
  enableAutoRecenter: boolean = false;

  @input
  @hint("Active la mise à jour automatique de la position")
  enableLocationUpdates: boolean = true;

  @input
  @hint("Active les rotations de la carte")
  enableMapRotation: boolean = true;

  @input
  @hint("Active les logs détaillés pour debug")
  enableDetailedLogs: boolean = false;

  @input
  @hint("Force le flag viewScrolled à false (pour debug)")
  forceResetViewScrolled: boolean = false;

  @input
  @hint("Bloque complètement le recentrage automatique")
  blockAutoRecenter: boolean = true;
  ...
  private viewScrolled: boolean = false;
  ...
  private onUpdate() {
    ...
    // Debug force reset
    if (this.forceResetViewScrolled && this.viewScrolled) {
      this.debugLog("Force resetting viewScrolled flag");
      this.viewScrolled = false;
    }

    if (this.enableLocationUpdates && getTime() - this.lastMapUpdate > this.mapParameters.mapUpdateThreshold) {
      this.fetchLocation((location: GeoPosition) => {
        if (!this.mapParameters.setMapToCustomLocation) {
  ...
  private createMapGrid() {
    ...
    this.shouldFollowMapLocation = this.enableAutoFollowLocation;
    this.setUpZoom();
  }
  ...
  handleTouchStart(localPosition: vec2): void {
    ...
    this.debugLog(`Touch start - viewScrolled: ${this.viewScrolled}, shouldFollow: ${this.shouldFollowMapLocation}`);
    
    if (this.hoveringPinSet.size > 0) {
      this.debugLog("Starting pin drag");
  ...
  handleTouchEnd(localPosition: vec2): void {
    ...
    this.debugLog(`Touch end - viewScrolled: ${this.viewScrolled}, shouldFollow: ${this.shouldFollowMapLocation}`);
    
    if (this.isDraggingPin) {
      localPosition = localPosition.uniformScale(0.5);
      const adjustedAnchoredPosition = this.getPositionWithMapRotationOffset(localPosition);
      this.debugLog(`Pin dropped at: ${adjustedAnchoredPosition}`);
  ...
  private setNewMapLocation(location: GeoPosition): void {
    ...
    // Système de recentrage avec contrôles debug
    const shouldRecenter = this.shouldFollowMapLocation && 
                          !this.viewScrolled && 
                          this.enableAutoRecenter &&
                          !this.blockAutoRecenter;
    
    this.debugLog(`setNewMapLocation - shouldFollow: ${this.shouldFollowMapLocation}, viewScrolled: ${this.viewScrolled}, shouldRecenter: ${shouldRecenter}`);
    
    if (shouldRecenter) {
      this.debugLog("Auto-recentering map");
      this.offsetForLocation = getOffsetForLocation(
        this.mapModule,
        this.referencePositionLocationAsset,
        location.longitude,
  ...
  centerMap(): void {
    ...
    this.debugLog("centerMap called - resetting flags");
    ...
    this.tweenCancelFunction = makeTween((t) => {
      ...
      if (t === 1) {
        this.shouldFollowMapLocation = this.enableAutoFollowLocation;
        this.viewScrolled = false;
        this.debugLog(`Map centered - shouldFollow: ${this.shouldFollowMapLocation}, viewScrolled: ${this.viewScrolled}`);
  ...
  onScrollingStarted() {
    this.debugLog("onScrollingStarted - setting viewScrolled to TRUE");
