...
export class MapController extends BaseScriptComponent {
  ...
  showNearbyPlaces(category: string[] | null): void {
    log.i(`MapController.showNearbyPlaces called with category: ${category ? category.join(", ") : "null"}`);
    
    if (!this.placesProvider) {
      log.e("Places provider is not assigned! Please assign SnapPlacesProvider in the Inspector.");
      this.onNearbyPlacesFailedEvent.invoke();
      return;
    }

    if (!this.userLocation) {
      log.e("User location is not available yet!");
      this.onNearbyPlacesFailedEvent.invoke();
      return;
    }

    log.i(`User location: lat=${this.userLocation.latitude}, lng=${this.userLocation.longitude}`);
    log.i(`Searching within ${NEARBY_PLACES_RANGE}m radius`);

    this.placesProvider
      .getNearbyPlaces(this.userLocation, NEARBY_PLACES_RANGE, category)
      .then((places) => {
        log.i(`getNearbyPlaces returned ${places.length} places`);
        ...
        const placeString: string = places
          ...
          .join(", ");
        log.i(`Found places: ${placeString}`);

        this.placesProvider
          .getPlacesInfo(places)
          .then((placesInfo: PlaceInfo[]) => {
            log.i(`getPlacesInfo returned ${placesInfo.length} place infos`);
            
            for (let i = 0; i < placesInfo.length; i++) {
              if (!this.pinnedPlaceSet.has(placesInfo[i].placeId)) {
                log.i(`Creating pin for: ${placesInfo[i].name} at lat=${placesInfo[i].centroid.latitude}, lng=${placesInfo[i].centroid.longitude}`);
            ...
            log.i(`Total pins on map: ${this.pinSet.size}`);
          })
          .catch((error) => {
            log.e(`getPlacesInfo failed: ${error}`);
          });
      })
      .catch((error) => {
        log.e(`getNearbyPlaces failed: ${error}`);
        this.onNearbyPlacesFailedEvent.invoke();
      });
  }
}