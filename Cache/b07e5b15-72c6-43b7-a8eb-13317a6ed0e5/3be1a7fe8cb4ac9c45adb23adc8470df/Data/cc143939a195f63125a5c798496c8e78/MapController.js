...
let MapController = (() => {
    ...
    var MapController = _classThis = class extends _classSuper {
        ...
        updateZoomLevel(newZoomLevel) {
            ...
            log.i(`OffsetForLocation: (${this.offsetForLocation.x.toFixed(3)}, ${this.offsetForLocation.y.toFixed(3)})`);
            // 2. Calculer la position géographique du centre actuel de la vue
            // L'offset actuel contient déjà offsetForLocation + mapFocusPosition (0.5, 0.5)
            // Pour obtenir la position relative à mapLocation, on soustrait tout
            const offset = this.gridView.getOffset().sub(this.offsetForLocation).sub(new vec2(0.5, 0.5));
            log.i(`Offset from map center: (${offset.x.toFixed(3)}, ${offset.y.toFixed(3)})`);
            // Utiliser la fonction existante pour convertir en position géographique
            const centerGeoPosition = this.fromLocalPositionToLongLat(offset, currentZoomLevel);
            log.i(`Center geo: lat=${centerGeoPosition.latitude.toFixed(6)}, lon=${centerGeoPosition.longitude.toFixed(6)}`);
            // 3. Changer le niveau de zoom et recalculer les assets
            ...
            this.pinOffsetter = PinOffsetter_1.PinOffsetter.makeMapLocationOffsetter(this.mapModule, this.referencePositionLocationAsset);
            // 4. Calculer où se trouve ce point géographique au nouveau zoom
            const centerOffsetAtNewZoom = (0, MapUtils_1.getOffsetForLocation)(this.mapModule, this.referencePositionLocationAsset, centerGeoPosition.longitude, centerGeoPosition.latitude);
            log.i(`Center offset at new zoom: (${centerOffsetAtNewZoom.x.toFixed(3)}, ${centerOffsetAtNewZoom.y.toFixed(3)})`);
            // 5. Construire le nouvel offset de la vue
            // newOffset = offsetForLocation + centerOffset + (0.5, 0.5)
            const newOffset = this.offsetForLocation.add(centerOffsetAtNewZoom).add(new vec2(0.5, 0.5));
            log.i(`Final new offset: (${newOffset.x.toFixed(3)}, ${newOffset.y.toFixed(3)})`);
            log.i(`=== ZOOM END ===`);
            // 6. Appliquer le nouvel offset
            this.gridView.setOffset(newOffset);
            // 7. Mettre à jour la config et relayout
            this.gridView.handleUpdateConfig(this.config);
            this.gridView.layoutCells(true);
            // 8. Rebind tous les pins
            this.pinSet.forEach((pin) => {
                this.pinOffsetter.bindScreenTransformToLocation(pin.screenTransform, pin.location.longitude, pin.location.latitude);
            });
            if (this.userPin) {
