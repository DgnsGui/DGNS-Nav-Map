...
SC_DECLARE_TEXTURE(glowTexture); // Title: Glow Texture
uniform NF_PRECISION                float4 pokeGlowColor; // Title: Poke Glow Color
uniform NF_PRECISION                float4 pinchGlowColor; // Title: Pinch Glow Color
uniform NF_PRECISION                float  pokePinchBlend; // Title: pokePinchBlend
uniform NF_PRECISION                float  pokeGlowBrightness; // Title: Poke Glow Brightness
uniform NF_PRECISION                float  pinchGlowBrightness; // Title: Pinch Glow Brightness	
...
#define Node18_Texture_2D_Sample( UVCoord, Color, Globals ) Color = SC_SAMPLE_TEX_R(glowTexture, UVCoord, 0.0)
#define Node21_Swizzle( Input, Output, Globals ) Output = float3( Input.r, Input.g, Input.b )
void Node20_Color_Parameter( out float4 Output, ssGlobals Globals ) { Output = pokeGlowColor; }
void Node19_Color_Parameter( out float4 Output, ssGlobals Globals ) { Output = pinchGlowColor; }
void Node59_Float_Parameter( out float Output, ssGlobals Globals ) { Output = pokePinchBlend; }
#define Node60_Mix( Input0, Input1, Input2, Output, Globals ) Output = mix( Input0, Input1, float4(Input2) )
#define Node26_Swizzle( Input, Output, Globals ) Output = float3( Input.r, Input.g, Input.b )
#define Node25_Multiply( Input0, Input1, Output, Globals ) Output = Input0 * Input1
#define Node27_Swizzle( Input, Output, Globals ) Output = Input.a
#define Node22_Swizzle( Input, Output, Globals ) Output = Input.a
void Node14_Float_Parameter( out float Output, ssGlobals Globals ) { Output = pokeGlowBrightness; }
void Node13_Float_Parameter( out float Output, ssGlobals Globals ) { Output = pinchGlowBrightness; }
#define Node1_Mix( Input0, Input1, Input2, Output, Globals ) Output = mix( Input0, Input1, Input2 )
#define Node23_Multiply( Input0, Input1, Input2, Output, Globals ) Output = Input0 * Input1 * Input2
#define Node24_Construct_Vector( Value1, Value2, Value, Globals ) Value.xyz = Value1; Value.w = Value2
//-----------------------------------------------------------------------------
...
void main() 
{
	...
	
	{
		...
		float4 Color_N18 = float4(0.0); Node18_Texture_2D_Sample( UVCoord_N17, Color_N18, Globals );
		float3 Output_N21 = float3(0.0); Node21_Swizzle( Color_N18.xyz, Output_N21, Globals );
		float4 Output_N20 = float4(0.0); Node20_Color_Parameter( Output_N20, Globals );
		float4 Output_N19 = float4(0.0); Node19_Color_Parameter( Output_N19, Globals );
		float Output_N59 = 0.0; Node59_Float_Parameter( Output_N59, Globals );
		float4 Output_N60 = float4(0.0); Node60_Mix( Output_N20, Output_N19, Output_N59, Output_N60, Globals );
		float3 Output_N26 = float3(0.0); Node26_Swizzle( Output_N60.xyz, Output_N26, Globals );
		float3 Output_N25 = float3(0.0); Node25_Multiply( Output_N21, Output_N26, Output_N25, Globals );
		float Output_N27 = 0.0; Node27_Swizzle( Output_N60, Output_N27, Globals );
		float Output_N22 = 0.0; Node22_Swizzle( Color_N18, Output_N22, Globals );
		float Output_N14 = 0.0; Node14_Float_Parameter( Output_N14, Globals );
		float Output_N13 = 0.0; Node13_Float_Parameter( Output_N13, Globals );
		float Output_N1 = 0.0; Node1_Mix( Output_N14, Output_N13, Output_N59, Output_N1, Globals );
		float Output_N23 = 0.0; Node23_Multiply( Output_N27, Output_N22, Output_N1, Output_N23, Globals );
		float4 Value_N24 = float4(0.0); Node24_Construct_Vector( Output_N25, Output_N23, Value_N24, Globals );
		
		FinalColor = Value_N24;
	}
