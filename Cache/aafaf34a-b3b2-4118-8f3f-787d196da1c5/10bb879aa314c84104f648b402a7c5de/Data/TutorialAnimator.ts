...
export class YZToggleMover extends BaseScriptComponent {
  ...
  private initialize(): void {
    ...
    // IMPORTANT: Commencer dans l'état rentré
    this.isOut = false
    this.moveToInitial(true)
    this.targetObject.enabled = false

    if (this.enableDebugLogs) {
      log.i("Initial state: isOut = " + this.isOut + ", enabled = " + this.targetObject.enabled)
    }
  ...
  private togglePosition(): void {
    if (this.animating) {
      if (this.enableDebugLogs) log.i("Animation en cours, ignoring toggle")
      return // Ignore si déjà en animation
    }

    if (this.enableDebugLogs) {
      log.i("=== TOGGLE TRIGGERED ===")
      log.i("Current state: isOut = " + this.isOut)
    }
    ...
    if (this.isOut) {
      // Actuellement sorti → Rentrer
      this.animationEndY = this.initialY
      this.animationEndZ = this.initialZ
      this.shouldHideAtEnd = true // Masquer à la fin
      
      if (this.enableDebugLogs) log.i("Action: RENTRER (hide at end)")
    } else {
      // Actuellement rentré → Sortir
      ...
      if (this.enableDebugLogs) log.i("Action: SORTIR (show immediately)")
    }

    this.animationElapsed = 0
    this.animating = true
    this.isOut = !this.isOut // Inverser l'état pour le prochain toggle

    if (this.enableDebugLogs) {
      log.i("Animation start: Y " + this.animationStartY + " → " + this.animationEndY)
      log.i("Animation start: Z " + this.animationStartZ + " → " + this.animationEndZ)
      log.i("Next state will be: isOut = " + this.isOut)
    }
  }
