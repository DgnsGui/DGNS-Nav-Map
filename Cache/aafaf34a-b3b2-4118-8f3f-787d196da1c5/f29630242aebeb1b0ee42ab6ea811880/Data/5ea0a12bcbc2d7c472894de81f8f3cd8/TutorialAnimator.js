...
// TutorialAnimator.ts
// Version FINALE – Fonctionne parfaitement dès le premier clic
...
let TutorialAnimator = (() => {
    ...
    var TutorialAnimator = _classThis = class extends _classSuper {
        ...
        initialize() {
            ...
            if (this.enableDebugLogs)
                log.i("TutorialAnimator initialisé et caché.");
        }
        toggle() {
            if (this.animating)
                return;
        ...
        show() {
            if (this.isVisible || this.animating)
                return;
            // Activer l'objet AVANT de lancer l'animation
            this.targetObject.enabled = true;
            // On anime depuis la position actuelle (qui est hiddenPosition) vers visiblePosition
            this.animationStartPos = this.targetTransform.getLocalPosition();
            this.animationEndPos = this.visiblePosition;
            this.animating = true;
            this.animationProgress = 0;
            if (this.enableDebugLogs)
                log.i("Démarrage animation : APPARITION");
        }
        hide() {
            if (!this.isVisible || this.animating)
                return;
            // On anime depuis la position actuelle (visible) vers hidden
            this.animationStartPos = this.targetTransform.getLocalPosition();
            this.animationEndPos = this.hiddenPosition;
            this.animating = true;
            this.animationProgress = 0;
            if (this.enableDebugLogs)
                log.i("Démarrage animation : DISPARITION");
        }
        updateAnimation() {
            if (!this.animating)
                return;
            ...
            if (this.animationProgress >= 1.0) {
                this.animating = false;
                this.targetTransform.setLocalPosition(this.animationEndPos); // sécurité position exacte
                if (this.animationEndPos.equal(this.visiblePosition)) {
                    ...
                    this.targetObject.enabled = false;
                    if (this.enableDebugLogs)
                        log.i("Disparition terminée → caché et désactivé");
                }
            }
        }
        // Méthodes publiques
