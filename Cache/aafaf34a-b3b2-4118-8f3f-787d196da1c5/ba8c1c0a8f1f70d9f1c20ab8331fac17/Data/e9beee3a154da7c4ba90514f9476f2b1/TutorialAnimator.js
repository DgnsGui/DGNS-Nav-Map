...
let TutorialAnimator = (() => {
    ...
    var TutorialAnimator = _classThis = class extends _classSuper {
        constructor() {
            ...
            this.initialPosition = new vec3(0, 0, 0);
            this.isVisible = false; // false = caché (rentré), true = visible (sorti)
            this.animating = false;
            this.animationProgress = 0;
            this.animationFrom = new vec3(0, 0, 0);
            this.animationTo = new vec3(0, 0, 0);
            // ====================== Easing functions ======================
        ...
        initialize() {
            ...
            if (this.enableDebugLogs)
                log.i("TutorialAnimator prêt et en position cachée.");
        }
        // ====================== Public Methods ======================
        show() {
        ...
        startAnimation(show) {
            ...
            if (show) {
                this.animationTo = new vec3(this.initialPosition.x, this.initialPosition.y + this.moveDistanceY, this.initialPosition.z + this.moveDistanceZ);
                this.isVisible = true;
            }
            else {
        ...
        updateAnimation(deltaTime) {
            ...
            const newPos = vec3.lerp(this.animationFrom, this.animationTo, t);
            ...
            if (this.animationProgress >= 1.0) {
                ...
                if (!this.isVisible) {
                    this.targetObject.enabled = false;
                    if (this.enableDebugLogs)
                        log.i("Container caché et désactivé");
                }
                else {
                    if (this.enableDebugLogs)
                        log.i("Container visible et animation terminée");
                }
            }
        }
        setPosition(pos) {
