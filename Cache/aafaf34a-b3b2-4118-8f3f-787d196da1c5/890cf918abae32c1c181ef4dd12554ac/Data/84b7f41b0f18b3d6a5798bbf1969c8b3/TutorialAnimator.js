...
let TutorialAnimator = (() => {
    ...
    var TutorialAnimator = _classThis = class extends _classSuper {
        constructor() {
            ...
            this.isFirstShow = true; // Flag pour gérer le premier show avec délai supplémentaire
            // Easing functions
        ...
        initialize() {
            ...
            if (this.enableDebugLogs)
                log.i("TutorialAnimator initialisé (caché).");
        }
        toggle() {
            if (this.animating)
                return;
        ...
        show() {
            ...
            delayEvent.bind(() => {
                ...
                if (this.enableDebugLogs)
                    log.i("Animation APPARITION lancée après délai");
            });
            delayEvent.reset(delayTime);
            if (this.enableDebugLogs)
                log.i("Show déclenché – objet activé, animation après délai");
        }
        hide() {
            if (!this.isVisible || this.animating)
                return;
            ...
            this.animationProgress = 0;
            if (this.enableDebugLogs)
                log.i("Animation DISPARITION lancée");
        }
        updateAnimation() {
            if (!this.animating)
                return;
            ...
            if (this.animationProgress >= 1.0) {
                ...
                if (this.animationEndPos.equal(this.visiblePosition)) {
                    ...
                    this.targetObject.enabled = false;
                    if (this.enableDebugLogs)
                        log.i("Disparition terminée → caché");
                }
            }
        }
        reset() {
