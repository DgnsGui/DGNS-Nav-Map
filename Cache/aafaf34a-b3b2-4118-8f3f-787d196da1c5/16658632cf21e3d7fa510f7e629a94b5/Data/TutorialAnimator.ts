...
export class YZToggleMover extends BaseScriptComponent {
  ...
  private isOut: boolean = false // false = rentré (invisible), true = sorti (visible)
  ...
  private initialize(): void {
    ...
    // Position initiale = rentrée et invisible
  ...
  private togglePosition(): void {
    ...
    // === CORRECTIF VISIBILITÉ : on active l'objet AVANT l'animation si on va le faire sortir ===
    if (!this.isOut) {
    ...
    if (this.isOut) {
      // On va RENTRER → retour à position initiale + masquage à la fin
      this.animationEndY = this.initialY
      this.animationEndZ = this.initialZ
      this.shouldHideAtEnd = true
    } else {
      // On va SORTIR → déplacement vers le bas et l'avant
    ...
    this.animating = true
    this.isOut = !this.isOut // On inverse l'état tout de suite

    if (this.enableDebugLogs) {
      log.i(`Animation ${this.isOut ? 'sortie' : 'rentrée'} vers Y=${this.animationEndY}, Z=${this.animationEndZ}`)
    }
  }
  ...
  private updateAnimation(eventData): void {
    ...
    if (t >= 1.0) {
      ...
      // Masquer uniquement si on vient de rentrer
      if (this.shouldHideAtEnd) {
        this.targetObject.enabled = false
        if (this.enableDebugLogs) log.i("Container masqué à la fin de l'animation de rentrée")
      }
  ...
  private moveToInitial(logIt = false): void {
    ...
    if (logIt && this.enableDebugLogs) {
      log.i(`Position forcée rentrée : Y=${this.initialY}, Z=${this.initialZ}`)
    }
  }
  ...
  private moveToTarget(logIt = false): void {
    ...
    this.targetTransform.setLocalPosition(pos)
    if (logIt && this.enableDebugLogs) {
      log.i(`Position forcée sortie : Y=${pos.y}, Z=${pos.z}`)
    }
  }
}