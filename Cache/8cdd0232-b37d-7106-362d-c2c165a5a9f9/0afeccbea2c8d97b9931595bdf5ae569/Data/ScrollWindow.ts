...
export class ScrollWindow extends BaseScriptComponent {
  ...
  vertical: boolean = false        // ← Désactivé par défaut (scroll horizontal activé)
  @input
  @hint("Enable Horizontal Scrolling")
  horizontal: boolean = true        // ← Activé par défaut
  @input("vec2", "{100,32}")
  @hint(
    "Size of masked window viewport in local space. <br><br>Note: to set dynamically, use <code>setWindowSize</code>"
  )
  private windowSize: vec2 = new vec2(100, 32)  // Largeur > hauteur pour horizontal
  @input("vec2", "{200,32}")
  @hint("Size of total scrollable area. <br><br>Note: to set dynamically, use <code>setScrollDimensions</code>")
  private scrollDimensions: vec2 = new vec2(200, 32)  // Contenu plus large
  ...
  private colliderShape = Shape.createSphereShape()
  ...
  public set scrollSnapping(scrollSnapping: boolean) {
    if (scrollSnapping === undefined) return
  ...
  public set snapRegion(snapRegion: vec2) {
    if (snapRegion === undefined) return
  ...
  onAwake() {
    ...
    this.createEvent("OnDestroyEvent").bind(() => {
      this.scrollTweenCancel()
      this.scroller.children.forEach((child) => child.setParent(this.sceneObject))
      this.managedComponents.forEach((component) => {
        if (!isNull(component) && component) component.destroy()
      })
      this.managedComponents = []
      this.managedSceneObjects.forEach((sceneObject) => {
        if (!isNull(sceneObject) && sceneObject) sceneObject.destroy()
      })
  ...
  setWindowSize = (size: vec2) => {
    ...
    const radiusX = this.windowSize.x * 0.5
    const radiusY = this.windowSize.y * 0.5
    const maxRadius = Math.max(radiusX, radiusY)
    
    this.colliderShape.radius = maxRadius
    this.collider.shape = this.colliderShape
    
    const colliderTransform = this.collider.getSceneObject().getTransform()
    const scaleX = radiusX / maxRadius
    const scaleY = radiusY / maxRadius
    colliderTransform.setLocalScale(new vec3(scaleX, scaleY, 1))
    
    this.maskingComponent.cornerRadius = Math.min(radiusX, radiusY)
  }
  ...
  enableEdgeFade = (enable: boolean) => {
    ...
    if (enable && !this.rmv) this.createEdgeFade()
  ...
  initialize = () => {
    ...
    this.screenTransform = this.sceneObject.getComponent("ScreenTransform") || this.sceneObject.createComponent("ScreenTransform")
    ...
    this.maskingComponent = this.sceneObject.getComponent("MaskingComponent") || this.sceneObject.createComponent("MaskingComponent")
    this.managedComponents.push(this.maskingComponent)
    this._interactable = this.sceneObject.getComponent(Interactable.getTypeName()) || this.sceneObject.createComponent(Interactable.getTypeName())
    this.managedComponents.push(this._interactable)
    this._interactable.isScrollable = true

    if (this.edgeFade) this.createEdgeFade()
    ...
    for (let i = 0; i < currentChildren.length; i++) {
      currentChildren[i].setParent(this.scroller)
    }
    ...
    // Position initiale à gauche pour scroll horizontal
    if (this.horizontal) {
      const leftPosition = new vec2(
        this.scrollDimensions.x * -0.5 + this.windowSize.x * 0.5,
        0
      )
      this.scrollPosition = leftPosition
    }
  ...
  private subscribeToEvents = (subscribe: boolean) => {
    ...
    const onDragUpdate = (event: InteractorEvent) => {
      ...
      if (event.interactor) {
        ...
        if (interactedElement && interactedElement.isDraggable && !interactedElement.inactive) return
        ...
        if (cursor) cursor.cursorPosition = event.interactor.planecastPoint
      }
    }
    ...
    const onDragEnd = (event: InteractorEvent) => {
      ...
      if (event.target.sceneObject !== this.sceneObject) {
        ...
        if (!interactedElement?.isDraggable) event.stopPropagation()
      }
      ...
      if (timeSinceLastGesture <= GESTURE_ACCUMULATION_TIME_MS) {
        const currentDirection = { x: Math.sign(this.velocity.x), y: Math.sign(this.velocity.y) }
        const accumulatedDirection = { x: Math.sign(this.accumulatedVelocity.x), y: Math.sign(this.accumulatedVelocity.y) }
      ...
      const cursor = SIK.CursorController.getCursorByInteractor(event.interactor)
      if (cursor) cursor.cursorPosition = null
    }
  ...
  getVisibleWindow(): VisibleWindow {
    const visibleWindow: VisibleWindow = { bottomLeft: vec2.zero(), topRight: vec2.zero() }
    const scrollerLocalPosition = this.scrollerTransform.getLocalPosition()
  ...
  setVelocity = (velocity: vec2): void => {
    this.spring.velocity = new vec3(velocity.x, velocity.y, this.spring.velocity.z)
  }

  private enableChildColliders = (enable: boolean): void => {
    const childColliders: ColliderComponent[] = findAllChildComponents(this.sceneObject, "ColliderComponent") as ColliderComponent[]
  ...
  private update = () => {
    ...
    if (this.debugRender) this.frustum.render()
        ...
        if (isOutOfBounds) {
          ...
          if (this.horizontal) {
            if (this.scrollSnapping && this.snapRegion.x > 0) {
              ...
              if (this.spring.velocityX > 0) currentPageIndex = Math.floor(currentPageFloat)
              else if (this.spring.velocityX < 0) currentPageIndex = Math.ceil(currentPageFloat)
              ...
              if (Math.abs(this.spring.velocityX) > MINIMUM_SCROLLING_VELOCITY) {
                ...
                if (this.spring.velocityX > 0) targetPageIndex = Math.ceil(currentPageFloat)
                else if (this.spring.velocityX < 0) targetPageIndex = Math.floor(currentPageFloat)
              }
              ...
              if (Math.abs(this.spring.velocityX) > MINIMUM_SCROLLING_VELOCITY && absDistanceToTarget > this.snapRegion.x) {
          ...
          if (this.vertical) {
            if (this.scrollSnapping && this.snapRegion.y > 0) {
              ...
              let currentPageIndex = Math.round(currentPageFloat)
              if (this.spring.velocityY > 0) currentPageIndex = Math.floor(currentPageFloat)
              else if (this.spring.velocityY < 0) currentPageIndex = Math.ceil(currentPageFloat)
              ...
              if (Math.abs(this.spring.velocityY) > MINIMUM_SCROLLING_VELOCITY) {
                ...
                if (this.spring.velocityY > 0) targetPageIndex = Math.ceil(currentPageFloat)
                else if (this.spring.velocityY < 0) targetPageIndex = Math.floor(currentPageFloat)
              }
              ...
              if (Math.abs(this.spring.velocityY) > MINIMUM_SCROLLING_VELOCITY && absDistanceToTarget > this.snapRegion.y) {
