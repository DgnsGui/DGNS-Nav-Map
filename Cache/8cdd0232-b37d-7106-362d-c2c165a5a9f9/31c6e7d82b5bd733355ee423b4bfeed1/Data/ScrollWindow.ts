...
export class ScrollWindow extends BaseScriptComponent {
  ...
  vertical: boolean = true          // ← Réactivé : scroll vertical par défaut
  @input
  @hint("Enable Horizontal Scrolling")
  horizontal: boolean = false       // ← Désactivé par défaut
  @input("vec2", "{32,32}")
  @hint(
    "Size of masked window viewport in local space. <br><br>Note: to set dynamically, use <code>setWindowSize</code>"
  )
  private windowSize: vec2 = new vec2(32, 32)     // Format carré par défaut (adapté vertical)
  @input("vec2", "{32,100}")
  @hint("Size of total scrollable area. <br><br>Note: to set dynamically, use <code>setScrollDimensions</code>")
  private scrollDimensions: vec2 = new vec2(32, 100)  // Hauteur > largeur = scroll vertical
  ...
  initialize = () => {
    ...
    // Position initiale en haut pour scroll vertical
    if (this.vertical) {
      const topPosition = new vec2(
        0,
