...
export class AIMapAssistant extends BaseScriptComponent {
  ...
  private playAudio(audioTrack: AudioTrackAsset): void {
    ...
    // Utilise la durée de la piste audio pour synchroniser le rangement
    let duration = 3.0; // fallback par défaut
    if (audioTrack && typeof audioTrack.duration === "number" && audioTrack.duration > 0) {
      duration = audioTrack.duration;
    }

    // Ajoute un petit buffer (ex: 0.2s)
