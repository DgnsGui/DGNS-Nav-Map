...
export class AIMapAssistant extends BaseScriptComponent {
  ...
  private collectMapContext(): string {
    ...
    parts.push(`Heading: ${this.mapComponent.getUserHeading().toFixed(2)}°`);
...
Give 2-3 brief, interesting facts. Max 100 words.`;
  ...
  private playAudio(audioTrack: AudioTrackAsset): void {
    ...
    // Utilisation d'un DelayedCallbackEvent basé sur la durée du track
    const duration = audioTrack.duration; // en secondes
    const delayEvent = this.createEvent("DelayedCallbackEvent");
    delayEvent.bind(() => {
      if (this.enableDebugLogs) log.i("Audio finished → hiding response");
      this.hideResponse();
      delayEvent.enabled = false; // Nettoyage
    });
    delayEvent.delay = duration;
