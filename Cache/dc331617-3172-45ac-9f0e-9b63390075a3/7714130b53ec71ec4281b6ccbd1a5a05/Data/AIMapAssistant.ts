...
export class AIMapAssistant extends BaseScriptComponent {
  ...
  // Estimation de la durée du TTS à partir du texte (2.5 mots/seconde + 0.5s de buffer, clampée entre 2 et 10s)
  private estimateTTSDuration(text: string): number {
    const words = text.trim().split(/\s+/).length;
    const seconds = words / 2.5 + 0.5;
    return Math.max(2, Math.min(seconds, 10));
  }
  ...
  // Timer manuel pour la fin du TTS
