...
import { AIResponseAnimator } from "../../AIFrameAnimator"; // <-- CHEMIN D'IMPORTATION CORRIGÃ‰
...
export class AIMapAssistant extends BaseScriptComponent {
  ...
  public askAboutCurrentView(): void {
    ...
    if (!this.mapComponent) {
      ...
      this.displayResponse("âŒ Error: Map not configured", true);
      return;
    }
    ...
    try {
      ...
      log.e(`âŒ Error in askAboutCurrentView: ${error}`);
      this.displayResponse(`âŒ Error: ${error}`, true);
...
Provide 2-3 brief, interesting insights about this geographical area. Keep your response under 100 words and conversational.`;
  ...
  private callOpenAI(prompt: string): void {
    ...
    OpenAI.chatCompletions(request)
      ...
      .catch((error) => {
        log.e(`âŒ OpenAI API error: ${error}`);
  ...
  private playAudio(audioTrack: AudioTrackAsset): void {
    try {
      ...
      log.e(`âŒ Error playing audio: ${error}`);
      this.isProcessing = false;
    }
  }

  private handleError(errorMessage: string): void {
    log.e(`âŒ ${errorMessage}`);
    this.displayResponse(`âŒ Error: ${errorMessage}`, true);
    this.isProcessing = false;
  }
  ...
  public askAboutPlaces(places: PlaceInfo[]): void {
    ...
    this.isProcessing = true;
    this.displayResponse("ðŸª Analyzing nearby places...", true);
