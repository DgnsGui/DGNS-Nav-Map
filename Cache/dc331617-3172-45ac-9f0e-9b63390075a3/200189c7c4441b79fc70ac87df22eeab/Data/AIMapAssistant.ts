...
export class AIMapAssistant extends BaseScriptComponent {
  ...
  // === TIMER POUR TTS ===
  private ttsEndTime: number | null = null;
  private ttsTimerActive: boolean = false;
  private ttsWasPlaying: boolean = false;
  private ttsFinished: boolean = false;
  ...
  private playAudio(audioTrack: AudioTrackAsset, ttsText?: string): void {
    ...
    if (this.enableDebugLogs) log.i("TTS estimated duration: " + duration + "s");
  ...
  private updateTTSTimer(): void {
    ...
    if (this.ttsWasPlaying && this.audioComponent && !this.audioComponent.isPlaying()) {
      if (this.enableDebugLogs) log.i("TTS finished (isPlaying) → restoring music & hiding response");
      this.finishTTS();
      return;
    }
  }
