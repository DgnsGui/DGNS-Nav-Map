...
export class AIMapAssistant extends BaseScriptComponent {
  ...
  // Pour le timer Spectacles
  private ttsTimerEvent: DelayedCallbackEvent = null;
  ...
  // Ajout du paramètre text pour fallback durée
  private playAudio(audioTrack: AudioTrackAsset, ttsText: string): void {
    ...
    // --- Correction ici ---
    if (globalThis.DeviceInfo && globalThis.DeviceInfo.isSpectacles) {
      // Sur Spectacles : timer basé sur la durée du TTS
      let duration = audioTrack.duration;
      if (!duration || duration <= 0) {
        // Fallback : estimation à partir du texte (moyenne 2.5 mots/sec)
        const wordCount = ttsText.split(/\s+/).length;
        duration = Math.max(2.0, wordCount / 2.5);
      }
      if (this.enableDebugLogs) log.i(`TTS duration (Spectacles): ${duration}s`);
      // Annule timer précédent si besoin
      if (this.ttsTimerEvent) this.ttsTimerEvent.enabled = false;
      this.ttsTimerEvent = script.createEvent("DelayedCallbackEvent");
      this.ttsTimerEvent.bind(() => {
        if (this.enableDebugLogs) log.i("TTS finished (timer) → restoring music & hiding response");
        this.restoreMusicVolume();
        this.hideResponse();
        this.ttsTimerEvent = null;
      });
      this.ttsTimerEvent.reset(duration + 0.2); // petit buffer
    } else {
      // Preview : logique classique
  ...
  // Sur Spectacles, on ne fait rien ici (timer gère la fin)
  private checkAudioCompletion(): void {
    if (globalThis.DeviceInfo && globalThis.DeviceInfo.isSpectacles) return;
  ...
  public cancelRequest(): void {
    if (this.isProcessing) {
      ...
      this.restoreMusicVolume();
      this.hideResponse();
      if (this.ttsTimerEvent) {
        this.ttsTimerEvent.enabled = false;
        this.ttsTimerEvent = null;
      }
    }
  }
