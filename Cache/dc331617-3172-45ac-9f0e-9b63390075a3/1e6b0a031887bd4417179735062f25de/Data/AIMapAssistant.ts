...
Provide 2-3 brief, interesting insights about this geographical area. Keep your response under 100 words and conversational.`;
  ...
  private playAudio(audioTrack: AudioTrackAsset): void {
    try {
      ...
      // Add onCompletion handler to auto-hide after audio finishes
      const completionBinding = this.audioComponent.onCompletion.add(() => {
        if (this.enableDebugLogs) log.i("🎵 Audio playback completed, hiding response...");
        this.hideResponse();
        completionBinding.remove();
      });
  ...
  public hideResponse(): void {
    ...
    if (this.aiResponseAnimator) {
      ...
      if (this.enableDebugLogs) log.i("Forcing animateOut via AIResponseAnimator");
      
      // Si une animation est en cours, l'annuler d'abord
      if (this.aiResponseAnimator.getIsAnimating()) {
        if (this.enableDebugLogs) log.i("Animation in progress, will be cancelled by animateOut");
      }
