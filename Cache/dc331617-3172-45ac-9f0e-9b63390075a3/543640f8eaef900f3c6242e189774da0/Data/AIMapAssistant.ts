...
export class AIMapAssistant extends BaseScriptComponent {
  ...
  private playAudio(audioTrack: AudioTrackAsset, ttsText: string): void {
    ...
    if (globalThis.DeviceInfo && globalThis.DeviceInfo.isSpectacles) {
      // Estimation de la durée du TTS (2.5 mots/sec)
      const wordCount = ttsText.split(/\s+/).length;
      const duration = Math.max(2.0, wordCount / 2.5);
      if (this.enableDebugLogs) log.i(`TTS estimated duration (Spectacles): ${duration}s`);
      // Annule timer précédent si besoin
      if (this.ttsTimerEvent) this.ttsTimerEvent.enabled = false;
      this.ttsTimerEvent = this.createEvent("DelayedCallbackEvent");
