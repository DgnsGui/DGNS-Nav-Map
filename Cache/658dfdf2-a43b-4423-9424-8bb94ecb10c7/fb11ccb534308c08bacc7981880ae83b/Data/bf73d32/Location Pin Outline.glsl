...
uniform NF_PRECISION            float3 circleBoundCentre; // Title: CircleBoundCentre
...
uniform NF_PRECISION float Port_Value1_N089;
uniform NF_PRECISION float Port_Value2_N089;
uniform NF_PRECISION float Port_Value3_N089;
uniform NF_PRECISION float2 Port_Input1_N097;
...
#define Node5_Surface_Position( Position, Globals ) Position = Globals.SurfacePosition_ObjectSpace
void Node11_Float_Parameter( out float Output, ssGlobals Globals ) { Output = lineWeight; }
#define Node3_Add_One( Input0, Output, Globals ) Output = Input0 + 1.0
#define Node8_Scale_and_Offset( Input0, Input1, Input2, Output, Globals ) Output = Input0 * float3(Input1) + float3(Input2)
#define Node1_Transform_Vector( VectorIn, VectorOut, Globals ) VectorOut = ( ngsModelMatrix * float4( VectorIn.xyz, 1.0 ) ).xyz
...
void main() 
{
	...
	{
		float3 Position_N5 = float3(0.0); Node5_Surface_Position( Position_N5, Globals );
		float Output_N11 = 0.0; Node11_Float_Parameter( Output_N11, Globals );
		float Output_N3 = 0.0; Node3_Add_One( Output_N11, Output_N3, Globals );
		float3 Output_N8 = float3(0.0); Node8_Scale_and_Offset( Position_N5, Output_N3, NF_PORT_CONSTANT( float( 0.0 ), Port_Input2_N008 ), Output_N8, Globals );
		float3 VectorOut_N1 = float3(0.0); Node1_Transform_Vector( Output_N8, VectorOut_N1, Globals );
		
		WorldPosition = VectorOut_N1;
	}
...
void Node12_Color_Parameter( out float4 Output, ssGlobals Globals ) { Output = lineColor; }
#define Node41_Surface_Position( Position, Globals ) Position = Globals.SurfacePosition_WorldSpace
void Node74_Float_Parameter( out float3 Output, ssGlobals Globals ) { Output = circleBoundCentre; }
#define Node22_Subtract( Input0, Input1, Output, Globals ) Output = Input0 - Input1
#define Node23_Surface_Normal( Normal, Globals ) Normal = Globals.VertexNormal_WorldSpace
#define Node89_Construct_Vector( Value1, Value2, Value3, Value, Globals ) Value.x = Value1; Value.y = Value2; Value.z = Value3
#define Node90_Cross( A, B, Result, Globals ) Result = cross( A, B )
...
#define Node92_Dot_Product( Input0, Input1, Output, Globals ) Output = dot( Input0, Input1 )
#define Node93_Cross( A, B, Result, Globals ) Result = cross( A, B )
void Node94_Normalize( in float3 Input0, out float3 Output, ssGlobals Globals )
{ 
	float lengthSquared = dot( Input0, Input0 );
	float l = ( lengthSquared > 0.0 ) ? 1.0 / sqrt( lengthSquared  ) : 0.0;
	Output = Input0 * l;
}
#define Node95_Dot_Product( Input0, Input1, Output, Globals ) Output = dot( Input0, Input1 )
#define Node96_Construct_Vector( Value1, Value2, Value, Globals ) Value.x = Value1; Value.y = Value2
...
#define Node7_Surface_UV_Coord( UVCoord, Globals ) UVCoord = Globals.Surface_UVCoord0
#define Node2_Texture_2D_Sample( UVCoord, Color, Globals ) Color = SC_SAMPLE_TEX_R(baseTex, UVCoord, 0.0)
#define Node0_Swizzle( Input, Output, Globals ) Output = Input.a
#define Node9_Min( Input0, Input1, Output, Globals ) Output = min( Input0, Input1 )
...
void main() 
{
	...
	
	{
		...
		float3 Position_N41 = float3(0.0); Node41_Surface_Position( Position_N41, Globals );
		float3 Output_N74 = float3(0.0); Node74_Float_Parameter( Output_N74, Globals );
		float3 Output_N22 = float3(0.0); Node22_Subtract( Position_N41, Output_N74, Output_N22, Globals );
		float3 Normal_N23 = float3(0.0); Node23_Surface_Normal( Normal_N23, Globals );
		float3 Value_N89 = float3(0.0); Node89_Construct_Vector( NF_PORT_CONSTANT( float( 0.0 ), Port_Value1_N089 ), NF_PORT_CONSTANT( float( 1.0 ), Port_Value2_N089 ), NF_PORT_CONSTANT( float( 0.0 ), Port_Value3_N089 ), Value_N89, Globals );
		float3 Result_N90 = float3(0.0); Node90_Cross( Normal_N23, Value_N89, Result_N90, Globals );
		float3 Output_N91 = float3(0.0); Node91_Normalize( Result_N90, Output_N91, Globals );
		float Output_N92 = 0.0; Node92_Dot_Product( Output_N22, Output_N91, Output_N92, Globals );
		float3 Result_N93 = float3(0.0); Node93_Cross( Normal_N23, Output_N91, Result_N93, Globals );
		float3 Output_N94 = float3(0.0); Node94_Normalize( Result_N93, Output_N94, Globals );
		float Output_N95 = 0.0; Node95_Dot_Product( Output_N22, Output_N94, Output_N95, Globals );
		float2 Value_N96 = float2(0.0); Node96_Construct_Vector( Output_N92, Output_N95, Value_N96, Globals );
		float2 Output_N97 = float2(0.0); Node97_Divide( Value_N96, NF_PORT_CONSTANT( float2( 1.88, 0.95 ), Port_Input1_N097 ), Output_N97, Globals );
		float Output_N98 = 0.0; Node98_Length( Output_N97, Output_N98, Globals );
		float Output_N99 = 0.0; Node99_Step( Output_N98, NF_PORT_CONSTANT( float( 10.0 ), Port_Input1_N099 ), Output_N99, Globals );
		Node28_Texture_2D_Object_Parameter( Globals );
		float2 UVCoord_N7 = float2(0.0); Node7_Surface_UV_Coord( UVCoord_N7, Globals );
		float4 Color_N2 = float4(0.0); Node2_Texture_2D_Sample( UVCoord_N7, Color_N2, Globals );
		float Output_N0 = 0.0; Node0_Swizzle( Color_N2, Output_N0, Globals );
		float Output_N9 = 0.0; Node9_Min( Output_N99, Output_N0, Output_N9, Globals );
		float4 Value_N6 = float4(0.0); Node6_Construct_Vector( Output_N12.xyz, Output_N9, Value_N6, Globals );
