...
export class MapComponent extends BaseScriptComponent {
  ...
  placesClamAnimator: ScriptComponent;
  ...
  onStart() {
    ...
    this.placesAnimator = this.placesClamAnimator as unknown as PlacesClamAnimator;
  ...
  private loadNearbyPlaces(categoryName: string[]): void {
    ...
    const pinAddedCallback = (pin: MapPin) => {
      ...
      // Remets ici ta logique originale de traitement des pins si nécessaire
    };
  ...
  // === NOUVELLE MÉTHODE : centre de la carte affichée ===
  public getMapCenterLocation(): GeoPosition | null {
    if (!this.mapController) {
      return null;
    }

    // Position locale du centre de l'écran (0.5, 0.5)
    const centerLocalPos = new vec2(0.5, 0.5);

    // Conversion en position géographique (doit exister dans ton MapController)
    // Si la méthode s'appelle autrement, adapte le nom
    const geoPos = this.mapController.localToGeoPosition?.(centerLocalPos) 
                || this.mapController.screenToGeo?.(centerLocalPos)
                || null;
