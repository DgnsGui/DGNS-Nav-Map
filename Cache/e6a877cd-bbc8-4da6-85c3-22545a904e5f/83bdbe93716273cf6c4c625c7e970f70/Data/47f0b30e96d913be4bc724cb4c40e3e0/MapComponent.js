                ...
                if (child.name === "Switch" || child.name.includes("Switch")) {
                    ...
                    for (let j = 0; j < childComponents.length; j++) {
                        ...
                        if (componentName === "Switch" || (switchComponent.typeName && switchComponent.typeName === "Switch")) {
                            print("✓ SWITCH COMPONENT FOUND");
                            // Lister TOUTES les propriétés du composant Switch (pas l'API)
                            print("Switch component properties:");
                            for (const prop in switchComponent) {
                                if (switchComponent.hasOwnProperty(prop)) {
                                    const propType = typeof switchComponent[prop];
                                    print("  - " + prop + " (" + propType + ")");
                                }
                            }
                            // Essayer d'accéder directement aux propriétés
                            let stateSet = false;
                            // Essayer différentes propriétés
                            if (switchComponent.isOn !== undefined) {
                                print("Found isOn property, current value: " + switchComponent.isOn);
                                switchComponent.isOn = place.isActive;
                                print("✓ Set isOn to: " + place.isActive);
                                stateSet = true;
                            }
                            else if (switchComponent._isOn !== undefined) {
                                print("Found _isOn property");
                                switchComponent._isOn = place.isActive;
                                print("✓ Set _isOn to: " + place.isActive);
                                stateSet = true;
                            }
                            else if (switchComponent.state !== undefined) {
                                print("Found state property");
                                switchComponent.state = place.isActive;
                                print("✓ Set state to: " + place.isActive);
                                stateSet = true;
                            }
                            // Essayer de trouver les événements
                            let subscribed = false;
                            if (switchComponent.onStateChanged) {
                                print("Found onStateChanged");
                                if (typeof switchComponent.onStateChanged.add === "function") {
                                    switchComponent.onStateChanged.add((newState) => {
                                        print(">>> SWITCH TOGGLED: " + place.name + " = " + newState);
                            ...
                            if (!subscribed && switchComponent._onStateChanged) {
                                print("Found _onStateChanged");
                                if (typeof switchComponent._onStateChanged.add === "function") {
                                    switchComponent._onStateChanged.add((newState) => {
                                        print(">>> SWITCH TOGGLED: " + place.name + " = " + newState);
                                        this.togglePlace(index, newState);
                                    });
                                    print("✓ Subscribed to _onStateChanged");
                                    subscribed = true;
                                }
                            }
                            print("Setup result - State set: " + stateSet + ", Subscribed: " + subscribed);
                            if (stateSet || subscribed) {
                                return; // Au moins une chose fonctionne
                            }
                        }
                    }
                }
            }
            print("✗ Could not setup Switch for " + place.name);
        }
        /**
