...
export class MapComponent extends BaseScriptComponent {
  ...
  @ui.separator
  @ui.label("Pinch Button for Auto Rotation")
  @input
  @hint("Button that toggles minimap auto rotation on pinch gesture")
  pinchButton: SceneObject;
  @ui.separator
  @ui.label(
  ...
  private pinchButtonInteractable: Interactable;
  ...
  onStart() {
    ...
    // Setup pinch button interaction
    this.setupPinchButton();
  }

  private setupPinchButton(): void {
    if (this.pinchButton) {
      this.pinchButtonInteractable = this.pinchButton.getComponent("Component.Interactable");
      if (this.pinchButtonInteractable) {
        this.pinchButtonInteractable.onTriggerStart.add(this.onPinchButtonPressed.bind(this));
      }
    }
  }

  private onPinchButtonPressed(): void {
    // Toggle the minimap auto rotate setting
    this.isMinimapAutoRotate = !this.isMinimapAutoRotate;
    
    // Update the map controller with the new setting
  ...
  /**
   * Toggle minimap auto rotation (for external script calls)
   */
  toggleMinimapAutoRotate(): void {
    this.onPinchButtonPressed();
  }

  /**
   * Get current state of minimap auto rotation
   */
  getMinimapAutoRotateState(): boolean {
