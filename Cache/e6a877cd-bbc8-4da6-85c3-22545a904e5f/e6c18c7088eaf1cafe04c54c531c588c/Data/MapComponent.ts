...
export class MapComponent extends BaseScriptComponent {
  ...
  private loadNearbyPlaces(categoryName: string[]): void {
    ...
    const pinAddedCallback = (pin: MapPin) => {
      print("==========================================");
      print("Pin #" + pinsLoaded + " added, extracting info...");
      ...
      if (location) {
        ...
        // Debug: afficher toutes les propriétés du pin
        print("Pin sceneObject.name: " + (pin.sceneObject ? pin.sceneObject.name : "null"));
        print("Pin placeInfo exists: " + (pin.placeInfo !== undefined && pin.placeInfo !== null));
        
        if (pin.placeInfo) {
          print("PlaceInfo.name: " + (pin.placeInfo.name || "undefined"));
          print("PlaceInfo type: " + typeof pin.placeInfo);
          
          if (pin.placeInfo.name) {
            placeName = pin.placeInfo.name;
            print("✓ Found place name from placeInfo: " + placeName);
          }
        }
        
        if (placeName.startsWith("Place ") && pin.sceneObject && pin.sceneObject.name) {
          placeName = pin.sceneObject.name;
          print("✓ Found place name from sceneObject: " + placeName);
        }
        ...
        const placeItem: PlaceItem = {
          ...
          pin: pin,
          questMarker: undefined, // Sera assigné plus tard
          isActive: false
        };
        ...
        print("Place added to list. Total places: " + this.placesData.length);
