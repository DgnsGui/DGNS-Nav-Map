...
export class MapComponent extends BaseScriptComponent {
  ...
  onStart() {
    ...
    const mapParameters: MapParameter = {
      ...
      setMapToCustomLocation: false, // Forcé à 'false'
      mapLocation: null,             // Ignoré
      mapFocusPosition: new vec2(0.5, 0.5),
    ...
    // S'abonner aux événements du contrôleur pour gérer les résultats
  ...
  private findScriptInChildren(parent: SceneObject, scriptName: string): any {
    ...
    while (queue.length > 0) {
      ...
      for (let i = 0; i < scripts.length; i++) {
        ...
        if (script.typeName === scriptName || script.constructor.name === scriptName) {
          return script;
        }
      }
  ...
  togglePlace(index: number, isActive: boolean): void {
    ...
    if (place.pin) {
      if (isActive) this.showPlacePin(place.pin); else this.hidePlacePin(place.pin);
    }
    if (place.questMarker) {
      if (isActive) this.showQuestMarker(place.questMarker); else this.hideQuestMarker(place.questMarker);
    }
  }
  ...
  createMapPin(longitude: number, latitude: number): MapPin {
    const location = GeoPosition.create(); location.longitude = longitude; location.latitude = latitude;
  ...
  showNearbyPlaces(categoryName: string[]): void { this.openPlacesClam(categoryName); }
