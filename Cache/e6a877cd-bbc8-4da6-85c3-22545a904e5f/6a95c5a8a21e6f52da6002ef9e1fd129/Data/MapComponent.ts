...
export class MapComponent extends BaseScriptComponent {
  ...
  @ui.separator
  @ui.label("Places Search Configuration")
  @ui.label("Search radius in meters")
  @input
  @widget(new SliderWidget(50, 1000, 50))
  searchRadius: number = 100;

  @ui.separator
  @ui.label("Category Configurations")
  @ui.group_start("Shops Category")
  @input
  shopsEnabled: boolean = true;
  @input
  @hint("Comma-separated tags for shops (e.g., 'retail_store,shopping_mall,supermarket')")
  shopsTags: string = "retail_store,shopping_mall,supermarket,clothing_store,electronics_store";
  @ui.group_end

  @ui.group_start("Snacks Category") 
  @input
  snacksEnabled: boolean = true;
  @input
  @hint("Comma-separated tags for snacks/food (e.g., 'restaurant,cafe,fast_food')")
  snacksTags: string = "restaurant,cafe,fast_food,bakery,food_truck";
  @ui.group_end

  @ui.group_start("Scenic Places (Camera)")
  @input
  scenicEnabled: boolean = true;
  @input
  @hint("Comma-separated tags for scenic places")
  scenicTags: string = "tourist_attraction,park,museum,monument,viewpoint";
  @ui.group_end

  @ui.group_start("Bars/Nightlife (Cocktail)")
  @input
  nightlifeEnabled: boolean = true;
  @input
  @hint("Comma-separated tags for bars and nightlife")
  nightlifeTags: string = "bar,nightclub,brewery,pub,wine_bar";
  @ui.group_end

  @ui.group_start("Beauty/Barber (Barber)")
  @input
  beautyEnabled: boolean = true;
  @input
  @hint("Comma-separated tags for beauty and barber services")
  beautyTags: string = "hair_salon,barber_shop,beauty_salon,spa,nail_salon";
  @ui.group_end

  @ui.group_start("Skate (Skateboard)")
  @input
  skateEnabled: boolean = true;
  @input
  @hint("Comma-separated tags for skate-related places")
  skateTags: string = "skate_park,sporting_goods_store,sports_complex";
  @ui.group_end

  @ui.group_start("Travel (Plane)")
  @input
  travelEnabled: boolean = true;
  @input
  @hint("Comma-separated tags for travel-related places")
  travelTags: string = "airport,travel_agency,hotel,subway_station,bus_station";
  @ui.group_end

  @ui.group_start("Books (Book)")
  @input
  booksEnabled: boolean = true;
  @input
  @hint("Comma-separated tags for books and media")
  booksTags: string = "book_store,library,news_stand,stationery_store";
  @ui.group_end

  @ui.group_start("Outdoor/Camping (Camping)")
  @input
  outdoorEnabled: boolean = true;
  @input
  @hint("Comma-separated tags for outdoor and camping")
  outdoorTags: string = "campground,park,beach,hiking_trail,outdoor_gear_store";
  @ui.group_end
  ...
  onStart() {
    ...
    this.mapController.setSearchRadius(this.searchRadius);
  }
  ...
  showShops(): void {
    if (this.shopsEnabled) {
      const tags = this.parseTagsString(this.shopsTags);
      this.mapController.showNearbyPlaces(tags);
    }
  }

  showSnacks(): void {
    if (this.snacksEnabled) {
      const tags = this.parseTagsString(this.snacksTags);
      this.mapController.showNearbyPlaces(tags);
    }
  }

  showScenic(): void {
    if (this.scenicEnabled) {
      const tags = this.parseTagsString(this.scenicTags);
      this.mapController.showNearbyPlaces(tags);
    }
  }

  showNightlife(): void {
    if (this.nightlifeEnabled) {
      const tags = this.parseTagsString(this.nightlifeTags);
      this.mapController.showNearbyPlaces(tags);
    }
  }
  ...
  showBeauty(): void {
    if (this.beautyEnabled) {
      const tags = this.parseTagsString(this.beautyTags);
      this.mapController.showNearbyPlaces(tags);
    }
  }

  showSkate(): void {
    if (this.skateEnabled) {
      const tags = this.parseTagsString(this.skateTags);
      this.mapController.showNearbyPlaces(tags);
    }
  }
  ...
  showTravel(): void {
    if (this.travelEnabled) {
      const tags = this.parseTagsString(this.travelTags);
      this.mapController.showNearbyPlaces(tags);
    }
  }
  ...
  showBooks(): void {
    if (this.booksEnabled) {
      const tags = this.parseTagsString(this.booksTags);
      this.mapController.showNearbyPlaces(tags);
    }
  }

  showOutdoor(): void {
    if (this.outdoorEnabled) {
      const tags = this.parseTagsString(this.outdoorTags);
      this.mapController.showNearbyPlaces(tags);
    }
  }
  ...
  showAllPlaces(): void {
    ...
    if (this.shopsEnabled) allTags.push(...this.parseTagsString(this.shopsTags));
    if (this.snacksEnabled) allTags.push(...this.parseTagsString(this.snacksTags));
    if (this.scenicEnabled) allTags.push(...this.parseTagsString(this.scenicTags));
    if (this.nightlifeEnabled) allTags.push(...this.parseTagsString(this.nightlifeTags));
    if (this.beautyEnabled) allTags.push(...this.parseTagsString(this.beautyTags));
    if (this.skateEnabled) allTags.push(...this.parseTagsString(this.skateTags));
    if (this.travelEnabled) allTags.push(...this.parseTagsString(this.travelTags));
    if (this.booksEnabled) allTags.push(...this.parseTagsString(this.booksTags));
    if (this.outdoorEnabled) allTags.push(...this.parseTagsString(this.outdoorTags));

    // Remove duplicates
    const uniqueTags = [...new Set(allTags)];
    this.mapController.showNearbyPlaces(uniqueTags);
  }
   ...
   * Uses the configured search radius
   */
