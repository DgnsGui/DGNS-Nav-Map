...
class PlaceListItem {
  ...
  public setPlaceData(placeInfo: PlaceInfo, mapPin: MapPin, questMarker: SceneObject): void {
  ...
  private updatePinAndMarker(): void {
    if (this.mapPin && this.mapPin.getSceneObject()) {
      this.mapPin.getSceneObject().enabled = this.isActive;
    }
    if (this.questMarker) {
...
export class PlacesListController extends BaseScriptComponent {
  ...
  private findPinchButton(parent: SceneObject, name: string): PinchButton | null {
    ...
    const component = obj.getComponent("Component.PinchButton");
    return component as PinchButton;
  }
