// --- START OF FILE MapController.ts (FINAL COMPLETE VERSION) ---
...
export class MapController extends BaseScriptComponent {
  ...
  removeMapPin(mapPin: MapPin) {
    ...
    if (mapPin.placeInfo !== undefined) this.pinnedPlaceSet.delete(mapPin.placeInfo.placeId);
    if (mapPin.sceneObject && mapPin.sceneObject.isValid()) {
  ...
  handleZoomIn(): void {
    ...
    const scaledRelativeOffset = relativeOffset.uniformScale(2);
  ...
  handleZoomOut(): void {
    if (!this.isInitialized || this.mapParameters.zoomLevel <= 0) return;
    const oldOffset = this.gridView.getOffset();
    const oldOffsetForLocation = new vec2(this.offsetForLocation.x, this.offsetForLocation.y);
    this.mapParameters.zoomLevel--;
    this.mapParameters.zoomOffet = calculateZoomOffset(this.mapParameters.zoomLevel);
    ...
    this.updateLocationOffset();
    const relativeOffset = oldOffset.sub(oldOffsetForLocation).sub(this.mapParameters.mapFocusPosition);
    const scaledRelativeOffset = relativeOffset.uniformScale(0.5);
    const newOffset = this.offsetForLocation.add(scaledRelativeOffset).add(this.mapParameters.mapFocusPosition);
    this.gridView.setOffset(newOffset);
    this.pinOffsetter = PinOffsetter.makeMapLocationOffsetter(this.mapModule, this.referencePositionLocationAsset);
    this.gridView.handleUpdateConfig(this.config);
  ...
  showNearbyPlaces(category: string[]): void {
    ...
    if (!searchLocation) { log.e("Real user location not available for search"); this.onNearbyPlacesFailedEvent.invoke(); return; }
    log.i(`Searching near REAL user location: lat=${searchLocation.latitude}, lon=${searchLocation.longitude}`);
    this.placesProvider.getNearbyPlaces(searchLocation, NEARBY_PLACES_RANGE, category)
      .then((places) => {
        ...
        this.placesProvider.getPlacesInfo(places)
          .then((placesInfo: PlaceInfo[]) => { this.onNearbyPlacesSuccessEvent.invoke(placesInfo); })
  ...
  drawGeometryPoint(geometryPoint: vec2, radius: number = 0.1) {
    ...
    addRenderMeshVisual(renderMeshSceneObject, makeCircle2DMesh(position, radius), this.lineMaterial, this.mapRenderOrder + 1);
  ...
  drawGeometryLine(geometryLine: vec2[], thickness: number = 0.2) {
    ...
    renderMeshSceneObject.setParent(sceneObject);
    renderMeshSceneObject.layer = this.getSceneObject().layer;
    addRenderMeshVisual(renderMeshSceneObject, makeLineStrip2DMeshWithJoints([start, end], thickness), this.lineMaterial, this.mapRenderOrder + 1);
    this.pinOffsetter.bindScreenTransformToLocation(screenTransform, this.mapLocation.longitude, this.mapLocation.latitude);
    this.geometryObjects.push(sceneObject);
  }
  drawGeometryMultiline(geometryMultiline: any, thickness: number = 0.2) {
    ...
    renderMeshSceneObject.setParent(sceneObject);
    renderMeshSceneObject.layer = this.getSceneObject().layer;
    const positions: vec3[] = geometryMultiline.map((point: any) => this.getWorldPositionForGeometryPoint(point));
    addRenderMeshVisual(renderMeshSceneObject, makeLineStrip2DMeshWithJoints(positions, thickness), this.lineMaterial, this.mapRenderOrder + 1);
    this.pinOffsetter.bindScreenTransformToLocation(screenTransform, this.mapLocation.longitude, this.mapLocation.latitude);
    this.geometryObjects.push(sceneObject);
  }
  clearGeometry(): void {
  ...
  onContentMaskRenderLayer(renderLayer: any) {
    forEachSceneObjectInSubHierarchy(this.mapPinsAnchor, (sceneObject) => { sceneObject.layer = renderLayer; });
  }
  onScrollingStarted() {
