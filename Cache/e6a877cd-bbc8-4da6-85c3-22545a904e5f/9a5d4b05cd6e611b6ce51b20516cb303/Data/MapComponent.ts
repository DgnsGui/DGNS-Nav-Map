...
export class MapComponent extends BaseScriptComponent {
  ...
  // NEW: Storage for all retrieved places and current filter
  private allRetrievedPlaces: any[] = [];
  private currentFilterType: string = "all";
  private isPlacesLoaded: boolean = false;
   ...
   * MODIFIED: Now supports filtering
   */
  showNeaybyPlaces(categoryName: string[]): void {
    log.i("showNeaybyPlaces called with: " + (categoryName ? categoryName.join(", ") : "null"));
    
    if (!this.isPlacesLoaded) {
      log.i("Places not loaded yet, loading all places first...");
      this.currentFilterType = "all"; // Will be updated by the button handler
      this.mapController.showNearbyPlaces(categoryName);
      this.isPlacesLoaded = true;
    } else {
      log.i("Places already loaded, applying current filter: " + this.currentFilterType);
      this.applyCurrentFilter();
    }
  }

  /**
   * NEW: Show filtered places by type
   */
  showFilteredPlaces(filterType: string): void {
    log.i("showFilteredPlaces called with filter: " + filterType);
    this.currentFilterType = filterType;
    
    if (!this.isPlacesLoaded) {
      log.i("Places not loaded yet, loading all places first...");
      this.showNeaybyPlaces(null);
    } else {
      log.i("Applying filter to existing places...");
      this.applyCurrentFilter();
    }
  }

  /**
   * NEW: Apply current filter to loaded places
   */
  private applyCurrentFilter(): void {
    log.i("Applying filter: " + this.currentFilterType);
    
    // For now, we'll delegate to MapController
    // This is where we'll implement the actual filtering logic
    // once we understand the place data structure
    
    if (this.currentFilterType === "all") {
      log.i("Showing all places");
      // Show all places - we might need to call a specific method in MapController
      this.mapController.showNearbyPlaces(null);
    } else {
      log.i("Filtering places for: " + this.currentFilterType);
      // Apply specific filter - this will need to be implemented in MapController
      this.applySpecificFilter(this.currentFilterType);
    }
  }

  /**
   * NEW: Apply specific filter based on keywords
   */
  private applySpecificFilter(filterType: string): void {
    const keywords = this.getKeywordsForFilter(filterType);
    log.i("Filter keywords for '" + filterType + "': " + keywords.join(", "));
    
    // This method will need to work with MapController to filter places
    // For now, we'll just log and delegate to MapController
    // The actual filtering will be implemented once we see MapController code
    
    this.mapController.filterPlacesByKeywords(keywords);
  }

  /**
   * NEW: Get keywords for each filter type
   */
  private getKeywordsForFilter(filterType: string): string[] {
    const filterKeywords: { [key: string]: string[] } = {
      "restaurants": [
        "restaurant", "food", "dining", "eatery", "bistro", "grill", 
        "pizza", "burger", "chicken", "mexican", "italian", "chinese",
        "thai", "indian", "sushi", "steakhouse", "diner"
      ],
      "cafes": [
        "cafe", "coffee", "espresso", "starbucks", "costa", "dunkin",
        "coffeehouse", "bakery", "tea", "latte", "cappuccino"
      ],
      "bars": [
        "bar", "pub", "brewery", "tavern", "nightclub", "lounge",
        "wine", "cocktail", "beer", "spirits", "club"
      ],
      "snacks": [
        "convenience", "snack", "fast food", "takeaway", "bakery",
        "donut", "sandwich", "wrap", "smoothie", "juice", "ice cream",
        "frozen yogurt", "7-eleven", "gas station"
      ],
      "shops": [
        "shop", "store", "mall", "boutique", "market", "retail",
        "clothing", "electronics", "grocery", "supermarket", "pharmacy",
        "department store", "hardware", "walmart", "target"
      ],
      "scenic": [
        "tourist attraction", "museum", "park", "monument", "landmark",
        "gallery", "zoo", "aquarium", "garden", "viewpoint", "scenic",
        "historic", "cultural", "art"
      ],
      "barbers": [
        "hair", "salon", "barber", "beauty", "spa", "nail", "massage",
        "hairdresser", "stylist", "grooming"
      ],
      "recreation": [
        "gym", "fitness", "stadium", "sports", "recreation", "bowling",
        "pool", "tennis", "golf", "skate", "park", "playground"
      ],
      "transportation": [
        "airport", "train", "subway", "bus", "station", "terminal",
        "transport", "travel", "taxi", "uber", "lyft"
      ],
      "libraries": [
        "library", "book", "university", "school", "education", "study",
        "college", "bookstore", "academic"
      ],
      "parks": [
        "park", "nature", "outdoor", "trail", "camping", "recreation",
        "forest", "beach", "lake", "river", "hiking"
      ]
    };

    return filterKeywords[filterType.toLowerCase()] || [];
  }

  /**
   * NEW: Reset places loading state (useful for debugging)
   */
  resetPlacesLoadingState(): void {
    this.isPlacesLoaded = false;
    this.allRetrievedPlaces = [];
    this.currentFilterType = "all";
    log.i("Places loading state reset");
  }
