...
export class MapComponent extends BaseScriptComponent {
  ...
  drawGeometryPoint(geometry: any, radius: number, color: vec4): any {
    // CORRECTION: Passage des options dans un seul objet
    return this.mapController.drawGeometryPoint(geometry, { radius: radius, color: color });
  }

  drawGeometryLine(geometry: any, width: number, color: vec4): any {
    // CORRECTION: Passage des options dans un seul objet
    return this.mapController.drawGeometryLine(geometry, { width: width, color: color });
  }

  drawGeometryPolygon(geometry: any, color: vec4, outlineWidth: number, outlineColor: vec4): any {
    // CORRECTION: Contournement de l'erreur de type et passage des options dans un seul objet
  ...
  removeGeometry(geometry: any): void {
    // CORRECTION: Contournement de l'erreur de type
