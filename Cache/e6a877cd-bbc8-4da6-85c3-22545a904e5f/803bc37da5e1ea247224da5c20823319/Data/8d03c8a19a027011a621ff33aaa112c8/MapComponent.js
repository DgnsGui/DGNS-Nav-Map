...
let MapComponent = (() => {
    ...
    var MapComponent = _classThis = class extends _classSuper {
        ...
        loadNearbyPlaces(categoryName) {
            ...
            // Timer de timeout pour détecter si l'API ne répond pas
            let timeoutTriggered = false;
            const timeoutEvent = this.createEvent("DelayedCallbackEvent");
            timeoutEvent.bind(() => {
                if (!this.placesUIShown && this.placesData.length === 0) {
                    print("API timeout - no places loaded after 3 seconds");
                    timeoutTriggered = true;
                    this.placesUIShown = true;
                    this.delayedShowPlacesUI(); // Affichera les placeholders
                }
            });
            timeoutEvent.reset(3.0); // Timeout après 3 secondes
            // S'abonner aux événements du MapController pour récupérer les places
            const pinAddedCallback = (pin) => {
                if (timeoutTriggered) {
                    print("Pin received after timeout, ignoring");
                    return;
                }
        ...
        delayedShowPlacesUI() {
            ...
            if (this.placesClamContainer && !this.placesClamContainer.enabled) {
                ...
                delayEvent.bind(() => {
                    ...
                    // Si aucune place n'a été chargée, créer des placeholders
                    if (this.placesData.length === 0) {
                        print("No places loaded from API, using fallback placeholders");
        ...
        /**
         * Crée des places fictives pour les tests (quand l'API échoue)
         */
        createFallbackPlaces() {
            print("Creating " + this.fallbackPlacesCount + " fallback places...");
        ...
        setupPlaceToggle(itemObject, place, index) {
            print("=== SETUP TOGGLE for " + place.name + " (should be: " + place.isActive + ") ===");
                ...
                if (child.name === "Switch" || child.name.includes("Switch")) {
                    ...
                    for (let j = 0; j < childComponents.length; j++) {
                        ...
                        if (componentName === "Switch" || (switchComponent.typeName && switchComponent.typeName === "Switch")) {
                            print("✓ SWITCH FOUND");
                            // Définir l'état initial via la propriété isOn
                            try {
                                switchComponent.isOn = place.isActive;
                                print("✓ Set isOn to: " + place.isActive);
                                print("  Current isOn value: " + switchComponent.isOn);
                            }
                            catch (e) {
                                print("✗ Failed to set isOn: " + e);
                            }
                            // S'abonner à onStateChanged qui donne le boolean isOn
                            if (switchComponent.onStateChanged && typeof switchComponent.onStateChanged.add === "function") {
                                switchComponent.onStateChanged.add(() => {
                                    ...
                                    print(">>> SWITCH STATE CHANGED: " + place.name + " = " + newState);
                                ...
                                print("✗ onStateChanged not found");
                            }
                        }
                    }
                }
        ...
        togglePlace(index, isActive) {
            ...
            // Ne rien faire si l'état est déjà correct
            if (place.isActive === isActive) {
                print("Place " + place.name + " already in state: " + isActive + ", skipping");
                return;
            }
            place.isActive = isActive;
            print("Toggling place " + place.name + " to " + (isActive ? "ON" : "OFF"));
            // Si c'est une place placeholder (sans pin), juste logger
            if (!place.pin) {
                print("Placeholder place toggled (no pin to show/hide)");
                return;
            }
            if (isActive) {
