...
let MapComponent = (() => {
    ...
    var MapComponent = _classThis = class extends _classSuper {
        ...
        setupPlaceToggle(itemObject, place, index) {
            ...
            // Chercher tous les composants Switch directement
            const allComponents = itemObject.getComponents("Component.ScriptComponent");
            for (let i = 0; i < allComponents.length; i++) {
                const component = allComponents[i];
                const componentName = component.constructor ? component.constructor.name : "unknown";
                print("Checking component: " + componentName);
                // Chercher le Switch directement
                if (componentName === "Switch" || (component.typeName && component.typeName === "Switch")) {
                    print("✓ SWITCH FOUND for " + place.name);
                    // Accéder à l'API du Switch
                    if (component.api) {
                        print("Switch API available");
                        // Définir l'état initial
                        if (component.api.setIsOn) {
                            component.api.setIsOn(place.isActive);
                            print("✓ Switch state SET to: " + place.isActive);
                        }
                        else if (component.api.setState) {
                            component.api.setState(place.isActive);
                            print("✓ Switch state SET to: " + place.isActive);
                        }
                        else {
                            print("✗ No setIsOn or setState method found");
                        }
                        // S'abonner aux changements
                        if (component.api.onStateChanged) {
                            component.api.onStateChanged.add((newState) => {
                                print(">>> Switch toggled for " + place.name + ": " + newState);
                                this.togglePlace(index, newState);
                            });
                            print("✓ Subscribed to onStateChanged");
                        }
                        else {
                            print("✗ No onStateChanged event found");
                        }
                        return; // Switch trouvé et configuré
                    }
                    else {
            ...
            for (let i = 0; i < itemObject.getChildrenCount(); i++) {
                ...
                for (let j = 0; j < childComponents.length; j++) {
                    ...
                    if (childComponentName === "Switch" || (childComponent.typeName && childComponent.typeName === "Switch")) {
                        print("✓ SWITCH FOUND in child for " + place.name);
                        if (childComponent.api) {
                            if (childComponent.api.setIsOn) {
                                childComponent.api.setIsOn(place.isActive);
                                print("✓ Switch state SET to: " + place.isActive);
                            }
                            else if (childComponent.api.setState) {
                                childComponent.api.setState(place.isActive);
                                print("✓ Switch state SET to: " + place.isActive);
                            }
                            if (childComponent.api.onStateChanged) {
                            ...
                            return; // Switch trouvé
                        }
                    }
                }
            }
            print("✗ NO SWITCH FOUND for " + place.name);
        }
        /**
