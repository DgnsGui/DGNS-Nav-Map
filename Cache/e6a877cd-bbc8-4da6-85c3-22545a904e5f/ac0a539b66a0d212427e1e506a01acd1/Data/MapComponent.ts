...
export class MapComponent extends BaseScriptComponent {
  ...
  @input
  @hint("Position Y quand l'ovale est visible (valeur de base)")
  ...
  public adjustPlacesForAI(forAI: boolean, callback?: () => void): void {
    ...
    if (newVisibleY === this.placesClamActualVisibleY) {
    ...
    if (this.isPlacesClamVisible) {
      ...
      // Easing dynamique : rebond à la montée, douceur à la descente
      const easing = forAI ? "ease-out-back" : "ease-in-quad";
  ...
  private loadNearbyPlaces(categoryName: string[]): void {
    ...
    const pinAddedCallback = (pin: MapPin) => {
      ...
      // Remets ici ton code original pour traiter chaque pin ajouté
    };
