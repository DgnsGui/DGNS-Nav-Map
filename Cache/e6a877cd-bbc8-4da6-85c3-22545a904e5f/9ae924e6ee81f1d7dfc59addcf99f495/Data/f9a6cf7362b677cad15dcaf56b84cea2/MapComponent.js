    ...
    var MapComponent = _classThis = class extends _classSuper {
        ...
        setupPinCollection() {
            ...
            this.subscribeOnMapAddPin((pin) => {
                ...
                // NOUVEAU: Masquer le quest marker par défaut s'il existe déjà
                // Il sera affiché plus tard par linkQuestMarkerToPlace si isActive est true
                const delayEvent = this.createEvent("DelayedCallbackEvent");
                delayEvent.bind(() => {
                    if (placeItem.questMarker) {
                        if (isActive) {
                            this.showQuestMarker(placeItem.questMarker);
                        }
                        else {
                            this.hideQuestMarker(placeItem.questMarker);
                        }
                    }
                });
        ...
        delayedShowPlacesUI() {
            ...
            if (this.placesClamContainer) {
                ...
                delayEvent.bind(() => {
                    ...
                    this.populatePlacesList();
                    // NOUVEAU: Plusieurs tentatives de synchronisation des quest markers
                    // car ils peuvent être créés à différents moments
        ...
        populatePlacesList() {
            ...
            for (let i = 0; i < this.placesData.length; i++) {
                ...
                // Apply initial state pour les pins ET les quest markers
                if (place.pin) {
                    if (place.isActive) {
                ...
                // NOUVEAU: Appliquer aussi l'état initial pour les quest markers
        ...
        linkQuestMarkerToPlace(pin, questMarker) {
            ...
            // CRITIQUE: D'abord CACHER le quest marker par défaut
            if (questMarker && questMarker.transform) {
                const sceneObject = questMarker.transform.getSceneObject();
                if (sceneObject) {
                    sceneObject.enabled = false;
            ...
            if (place) {
                ...
                // Utiliser un délai pour être sûr que le marker est bien caché avant de décider
                const delayEvent = this.createEvent("DelayedCallbackEvent");
                delayEvent.bind(() => {
                    ...
                    if (place.isActive) {
                        ...
                        // S'assurer qu'il est vraiment caché
