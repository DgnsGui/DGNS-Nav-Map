...
export class MapComponent extends BaseScriptComponent {
  ...
  @ui.separator
  @ui.label("Search Radius for Nearby Places (meters)")
  @input
  ...
  onStart() {
    ...
    // Configure le rayon de recherche
  ...
  /**
   * Centering map to initial location
   */
  centerMap(): void {
  ...
  /**
   * Configure le rayon de recherche pour les lieux à proximité
   */
  setSearchRadius(radius: number): void {
    this.searchRadius = radius;
    if (this.mapController) {
      this.mapController.setSearchRadius(radius);
    }
  }

  /**
   * Retourne le rayon de recherche actuel
   */
  getSearchRadius(): number {
    return this.searchRadius;
  }

  // #region Nouvelles méthodes spécialisées pour chaque catégorie

  /**
   * Affiche tous les lieux à proximité (toutes catégories)
   */
  showAllPlaces(): void {
    this.mapController.showNearbyPlaces([
      "shopping", "food", "lodging", "tourist_attraction", 
      "gas_station", "bank", "pharmacy", "hospital", 
      "restaurant", "cafe", "bar", "nightclub"
    ]);
  }

  /**
   * Affiche uniquement les magasins (Shops button)
   */
  showShops(): void {
    this.mapController.showNearbyPlaces([
      "shopping_mall", "department_store", "clothing_store", 
      "electronics_store", "furniture_store", "grocery_or_supermarket",
      "hardware_store", "jewelry_store", "shoe_store", "store"
    ]);
  }

  /**
   * Affiche uniquement les lieux de restauration (Snacks button)
   */
  showSnacks(): void {
    this.mapController.showNearbyPlaces([
      "restaurant", "food", "meal_takeaway", "meal_delivery",
      "cafe", "bakery", "fast_food", "ice_cream_shop"
    ]);
  }

  /**
   * Affiche les lieux scéniques et attractions (Camera emoji button)
   */
  showScenicPlaces(): void {
    this.mapController.showNearbyPlaces([
      "tourist_attraction", "park", "natural_feature", 
      "point_of_interest", "establishment", "museum",
      "art_gallery", "zoo", "aquarium", "landmark"
    ]);
  }

  /**
   * Affiche les bars et lieux de sortie (Cocktail emoji button)
   */
  showBarsAndNightlife(): void {
    this.mapController.showNearbyPlaces([
      "bar", "night_club", "liquor_store", "brewery",
      "restaurant", "cafe"
    ]);
  }

  /**
   * Affiche les salons de coiffure et esthétique (Barber emoji button)
   */
  showBeautyServices(): void {
    this.mapController.showNearbyPlaces([
  ...
  /**
   * Affiche les skateparks et magasins de skate (Skateboard emoji button)
   */
  showSkateSpots(): void {
    this.mapController.showNearbyPlaces([
      "park", "sporting_goods_store", "gym", 
      "point_of_interest", "establishment"
    ]);
  }

  /**
   * Affiche les aéroports et agences de voyage (Plane emoji button)
   */
  showTravelServices(): void {
    this.mapController.showNearbyPlaces([
      "airport", "travel_agency", "bus_station", 
      "subway_station", "train_station", "taxi_stand"
    ]);
  }

  /**
   * Affiche les librairies et points de presse (Book emoji button)
   */
  showBookstores(): void {
    this.mapController.showNearbyPlaces([
  ...
  /**
   * Affiche les parcs et zones de camping (Camping emoji button)
   */
  showParksAndCamping(): void {
    this.mapController.showNearbyPlaces([
      "park", "campground", "rv_park", "natural_feature",
  ...
  // Méthode générique pour afficher des lieux par catégorie personnalisée
  showNearbyPlaces(categoryName: string[]): void {
