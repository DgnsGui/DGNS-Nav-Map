// --- START OF FILE MapComponent.ts (Corrected Auto-Center Logic) ---
...
export class MapComponent extends BaseScriptComponent {
  ...
  private handleAutoRotateToggle(event: InteractorEvent): void {
    ...
    if (this.mapController) {
      ...
      // Quand l'utilisateur appuie sur le BOUTON, on recentre. C'est le comportement attendu.
  ...
  /**
   * Set the minimap auto-rotation state programmatically
   */
  setMinimapAutoRotate(enabled: boolean): void {
    ...
    if (this.mapController) {
      ...
      // *** MODIFICATION : On supprime le recentrage automatique ici ***
      // Cette fonction est appelée par MapUIController après une interaction,
      // et ne doit PAS recentrer la carte.
      /*
      if (enabled) {
        this.mapController.centerMap();
      }
  ...
  createMapPin(longitude: number, latitude: number): MapPin { const loc = GeoPosition.create(); loc.longitude = longitude; loc.latitude = latitude; return this.mapController.createMapPin(loc); }
  ...
  showNeaybyPlaces(categoryName: string[]): void { this.mapController.showNearbyPlaces(categoryName); }
