...
import { PlaceInfo } from "./SnapPlacesProvider";  // Added import for PlaceInfo
...
export class MapComponent extends BaseScriptComponent {
  ...
  // NEW: Event for when nearby places are fetched (for populating Places Clam list)
  private onNearbyPlacesFetchedEvent = new Event<PlaceInfo[]>();
  onNearbyPlacesFetched: PublicApi<PlaceInfo[]> = this.onNearbyPlacesFetchedEvent.publicApi();
  ...
  onStart() {
    ...
    const mapParameters: MapParameter = {
      ...
      startedAsMiniMap: this.startedAsMiniMap,
      autoRotateToggleButton: this.autoRotateToggleButton
    };

    this.mapController.initialize(mapParameters);
  }
  ...
  // NEW: Subscribe to nearby places fetched (for Places Clam)
  subscribeOnNearbyPlacesFetched(fn: callback<PlaceInfo[]>): void {
    this.onNearbyPlacesFetched.add(fn);
  }
  ...
  // MODIFIED: Now handles population of list when categoryName is null
  showNearbyPlaces(categoryName: string[]): void {
    this.removeMapPins();  // Clear existing pins first

    // Assuming snapPlacesProvider is available via mapController or as a property
    // Adjust based on your full code (from SnapPlacesProvider.ts context)
    const mapLocation = this.setMapToCustomLocation ? this.mapLocation : this.getUserLocation();
    if (mapLocation.latitude === 0 && mapLocation.longitude === 0) {
      this.onNoNearbyPlacesFound.invoke();
      return;
    }

    // Fetch places (adjust numberNearbyPlaces and threshold as needed, e.g., 10 places, 100m threshold)
    this.mapController.snapPlacesProvider.getNearbyPlacesInfo(mapLocation, 10, 100, categoryName)
      .then((places: PlaceInfo[]) => {
        if (places.length === 0) {
          this.onNoNearbyPlacesFound.invoke();
          return;
        }
        ...
        if (categoryName === null) {
          // Populate list instead of adding pins (for Places button)
          this.onNearbyPlacesFetchedEvent.invoke(places);
        } else {
          // Add all pins for specific categories (old behavior)
          places.forEach((place) => {
            const pin = this.createMapPin(place.centroid.longitude, place.centroid.latitude);
            pin.setName(place.name);
            pin.placeInfo = place;
            this.onMapPinAdded.invoke(pin);
            // Assuming quest markers are triggered here or in onMapPinAdded
          });
        }
      })
      .catch((error) => {
        print(error);
        this.onNearbyPlacesFailed.invoke();
      });
  }
