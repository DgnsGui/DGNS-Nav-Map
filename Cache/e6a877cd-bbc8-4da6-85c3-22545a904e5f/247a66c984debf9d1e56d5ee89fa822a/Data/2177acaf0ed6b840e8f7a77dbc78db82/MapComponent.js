...
let MapComponent = (() => {
    ...
    var MapComponent = _classThis = class extends _classSuper {
        constructor() {
            ...
            this.placesUIShown = false; // Flag pour éviter les duplicatas
            this.placesClamAnimation = null;
            this.isPlacesClamAnimating = false;
            this.isPlacesClamVisible = false;
            this.placesClamActualHiddenY = 0; // Calculé dynamiquement
            this.placesClamActualVisibleY = 0; // Position actuelle
        }
        __initialize() {
            super.__initialize();
            this.tileCount = this.tileCount;
        ...
        loadNearbyPlaces(categoryName) {
            ...
            const pinAddedCallback = (pin) => {
                ...
                const alreadyExists = this.placesData.some(p => p.name === placeName &&
        ...
        setupPlaceToggle(itemObject, place, index) {
            print("=== SETUP TOGGLE for " + place.name + " (isActive: " + place.isActive + ") ===");
            // Chercher dans les enfants (le Switch est dans un enfant d'après les logs)
            for (let i = 0; i < itemObject.getChildrenCount(); i++) {
                ...
                for (let j = 0; j < childComponents.length; j++) {
                    const childComponent = childComponents[j];
                    const childComponentName = childComponent.constructor ? childComponent.constructor.name : "unknown";
                    print("  Component: " + childComponentName);
                    if (childComponentName === "Switch" || (childComponent.typeName && childComponent.typeName === "Switch")) {
                        print("✓ SWITCH FOUND in child: " + child.name);
                        if (childComponent.api) {
                            print("API exists, listing available methods:");
                            // Lister toutes les méthodes disponibles
                            const apiProps = Object.keys(childComponent.api);
                            print("API properties: " + apiProps.join(", "));
                            // Essayer différentes méthodes pour définir l'état
                            let stateSet = false;
                            if (typeof childComponent.api.setIsOn === "function") {
                                childComponent.api.setIsOn(place.isActive);
                                print("✓ Used setIsOn(" + place.isActive + ")");
                                stateSet = true;
                            }
                            else if (typeof childComponent.api.setState === "function") {
                                childComponent.api.setState(place.isActive);
                                print("✓ Used setState(" + place.isActive + ")");
                                stateSet = true;
                            }
                            else if (typeof childComponent.api.setSwitchState === "function") {
                                childComponent.api.setSwitchState(place.isActive);
                                print("✓ Used setSwitchState(" + place.isActive + ")");
                                stateSet = true;
                            }
                            else if (childComponent.api.isOn !== undefined) {
                                childComponent.api.isOn = place.isActive;
                                print("✓ Set isOn property to: " + place.isActive);
                                stateSet = true;
                            }
                            else {
                                print("✗ NO METHOD FOUND to set state");
                            }
                            // Essayer de s'abonner aux changements
                            let subscribed = false;
                            if (childComponent.api.onStateChanged && typeof childComponent.api.onStateChanged.add === "function") {
                                childComponent.api.onStateChanged.add((newState) => {
                                    print(">>> Switch toggled (onStateChanged) for " + place.name + ": " + newState);
                                    this.togglePlace(index, newState);
                                });
                                print("✓ Subscribed to onStateChanged");
                                subscribed = true;
                            }
                            else if (childComponent.api.onSwitchToggled && typeof childComponent.api.onSwitchToggled.add === "function") {
                                childComponent.api.onSwitchToggled.add((newState) => {
                                    print(">>> Switch toggled (onSwitchToggled) for " + place.name + ": " + newState);
                                    this.togglePlace(index, newState);
                                });
                                print("✓ Subscribed to onSwitchToggled");
                                subscribed = true;
                            }
                            else if (childComponent.api.onToggle && typeof childComponent.api.onToggle.add === "function") {
                                childComponent.api.onToggle.add((newState) => {
                                    print(">>> Switch toggled (onToggle) for " + place.name + ": " + newState);
                                    this.togglePlace(index, newState);
                                });
                                print("✓ Subscribed to onToggle");
                                subscribed = true;
                            }
                            else {
                                print("✗ NO EVENT FOUND to subscribe");
                            }
                            if (stateSet && subscribed) {
                                print("=== TOGGLE SETUP COMPLETE ===");
                                return;
                            }
                            else {
                                print("⚠ PARTIAL SETUP - State: " + stateSet + ", Subscribed: " + subscribed);
                            }
                        }
                        else {
                            print("✗ Switch found but no API");
                        }
                    }
                }
            }
            print("✗ NO WORKING SWITCH FOUND for " + place.name);
        }
        /**
