...
export class MapComponent extends BaseScriptComponent {
  ...
  @input
  @hint("Offset Y pour cacher le container (ex: -30 pour monter)")
  ...
  private setupPlaceToggle(itemObject: SceneObject, place: PlaceItem, index: number): void {
    ...
    for (let i = 0; i < itemObject.getChildrenCount(); i++) {
      ...
      for (let j = 0; j < childComponents.length; j++) {
        const childComponent = childComponents[j] as any;
        ...
        if (childComponentName === "Switch" || (childComponent.typeName && childComponent.typeName === "Switch")) {
          ...
          if (childComponent.api) {
            ...
            if (childComponent.api.onStateChanged && typeof childComponent.api.onStateChanged.add === "function") {
              childComponent.api.onStateChanged.add((newState: boolean) => {
              ...
              childComponent.api.onSwitchToggled.add((newState: boolean) => {
              ...
              childComponent.api.onToggle.add((newState: boolean) => {
