...
export class SimpleMusicPlayerManager extends BaseScriptComponent {
            ...
            for (let i = 0; i < this.localTracks.length; i++) {
                if (this.localTracks[i] && 
                    ...
                    this.allTracksData.push({
                        ...
                        albumCover: this.localTrackAlbumCovers?.[i] || null,
                        isRemote: false
                    });
                }
            }
            ...
            for (let i = 0; i < this.remoteTracks.length; i++) {
                if (this.remoteTracks[i] && 
                    ...
                    this.allTracksData.push({
                        ...
                        albumCover: this.remoteTrackAlbumCovers?.[i] || null,
                        isRemote: true
                    });
                }
            }
    ...
    private validateInputs(): boolean {
        ...
        if (numLocalTracks > 0) {
            ...
            if (!this.localTrackAlbumCovers || this.localTrackAlbumCovers.length !== numLocalTracks) {
                print(`SimpleMusicPlayerManager Warning: Mismatch local tracks (${numLocalTracks}) vs album covers (${this.localTrackAlbumCovers?.length || 0}). Some covers may be missing.`);
            }
        ...
        if (numRemoteTracks > 0) {
            ...
            if (!this.remoteTrackAlbumCovers || this.remoteTrackAlbumCovers.length !== numRemoteTracks) {
                print(`SimpleMusicPlayerManager Warning: Mismatch remote tracks (${numRemoteTracks}) vs album covers (${this.remoteTrackAlbumCovers?.length || 0}). Some covers may be missing.`);
            }
    ...
    private updateTrackInfoUI(): void {
        try {
            ...
            if (this.albumCoverImage) {
                if (albumCover) {
                    this.albumCoverImage.mainPass.baseTex = albumCover;
                    this.albumCoverImage.enabled = true;
                } else {
                    this.albumCoverImage.enabled = false;
                }
            }
        } catch (error) {
