...
uniform NF_PRECISION            float  Mask_Radius; // Title: Mask_Radius	
...
uniform NF_PRECISION float Port_Value1_N082;
uniform NF_PRECISION float Port_Value2_N082;
uniform NF_PRECISION float Port_Value3_N082;
uniform NF_PRECISION float2 Port_Input1_N088;
uniform NF_PRECISION float Port_Input1_N077;
#endif	
...
#define Node29_Surface_Position( Position, Globals ) Position = Globals.SurfacePosition_WorldSpace
void Node74_Float_Parameter( out float3 Output, ssGlobals Globals ) { Output = circleBoundCentre; }
#define Node76_Subtract( Input0, Input1, Output, Globals ) Output = Input0 - Input1
#define Node79_Surface_Normal( Normal, Globals ) Normal = Globals.VertexNormal_WorldSpace
#define Node82_Construct_Vector( Value1, Value2, Value3, Value, Globals ) Value.x = Value1; Value.y = Value2; Value.z = Value3
#define Node81_Cross( A, B, Result, Globals ) Result = cross( A, B )
void Node83_Normalize( in float3 Input0, out float3 Output, ssGlobals Globals )
{ 
	float lengthSquared = dot( Input0, Input0 );
	float l = ( lengthSquared > 0.0 ) ? 1.0 / sqrt( lengthSquared  ) : 0.0;
	Output = Input0 * l;
}
#define Node75_Dot_Product( Input0, Input1, Output, Globals ) Output = dot( Input0, Input1 )
void Node2_Float_Parameter( out float Output, ssGlobals Globals ) { Output = Mask_Radius; }
#define Node31_Multiply( Input0, Input1, Output, Globals ) Output = Input0 * Input1
#define Node84_Cross( A, B, Result, Globals ) Result = cross( A, B )
void Node85_Normalize( in float3 Input0, out float3 Output, ssGlobals Globals )
{ 
	float lengthSquared = dot( Input0, Input0 );
	float l = ( lengthSquared > 0.0 ) ? 1.0 / sqrt( lengthSquared  ) : 0.0;
	Output = Input0 * l;
}
#define Node86_Dot_Product( Input0, Input1, Output, Globals ) Output = dot( Input0, Input1 )
#define Node25_Multiply( Input0, Input1, Output, Globals ) Output = Input0 * Input1
#define Node87_Construct_Vector( Value1, Value2, Value, Globals ) Value.x = Value1; Value.y = Value2
#define Node88_Divide( Input0, Input1, Output, Globals ) Output = Input0 / (Input1 + 1.234e-6)
#define Node80_Length( Input0, Output, Globals ) Output = length( Input0 )
#define Node77_Step( Input0, Input1, Output, Globals ) Output = step( Input0, Input1 )
...
void main() 
{
	...
	float4 FinalColor = float4( 1.0, 1.0, 1.0, 0.469993 );
	...
	
	{
		...
		float3 Position_N29 = float3(0.0); Node29_Surface_Position( Position_N29, Globals );
		float3 Output_N74 = float3(0.0); Node74_Float_Parameter( Output_N74, Globals );
		float3 Output_N76 = float3(0.0); Node76_Subtract( Position_N29, Output_N74, Output_N76, Globals );
		float3 Normal_N79 = float3(0.0); Node79_Surface_Normal( Normal_N79, Globals );
		float3 Value_N82 = float3(0.0); Node82_Construct_Vector( NF_PORT_CONSTANT( float( 0.0 ), Port_Value1_N082 ), NF_PORT_CONSTANT( float( 1.0 ), Port_Value2_N082 ), NF_PORT_CONSTANT( float( 0.0 ), Port_Value3_N082 ), Value_N82, Globals );
		float3 Result_N81 = float3(0.0); Node81_Cross( Normal_N79, Value_N82, Result_N81, Globals );
		float3 Output_N83 = float3(0.0); Node83_Normalize( Result_N81, Output_N83, Globals );
		float Output_N75 = 0.0; Node75_Dot_Product( Output_N76, Output_N83, Output_N75, Globals );
		float Output_N2 = 0.0; Node2_Float_Parameter( Output_N2, Globals );
		float Output_N31 = 0.0; Node31_Multiply( Output_N75, Output_N2, Output_N31, Globals );
		float3 Result_N84 = float3(0.0); Node84_Cross( Normal_N79, Output_N83, Result_N84, Globals );
		float3 Output_N85 = float3(0.0); Node85_Normalize( Result_N84, Output_N85, Globals );
		float Output_N86 = 0.0; Node86_Dot_Product( Output_N76, Output_N85, Output_N86, Globals );
		float Output_N25 = 0.0; Node25_Multiply( Output_N86, Output_N2, Output_N25, Globals );
		float2 Value_N87 = float2(0.0); Node87_Construct_Vector( Output_N31, Output_N25, Value_N87, Globals );
		float2 Output_N88 = float2(0.0); Node88_Divide( Value_N87, NF_PORT_CONSTANT( float2( 1.9, 0.98 ), Port_Input1_N088 ), Output_N88, Globals );
		float Output_N80 = 0.0; Node80_Length( Output_N88, Output_N80, Globals );
		float Output_N77 = 0.0; Node77_Step( Output_N80, NF_PORT_CONSTANT( float( 10.0 ), Port_Input1_N077 ), Output_N77, Globals );
		float4 Value_N1 = float4(0.0); Node1_Construct_Vector( Export_N385.xyz, Output_N77, Value_N1, Globals );
