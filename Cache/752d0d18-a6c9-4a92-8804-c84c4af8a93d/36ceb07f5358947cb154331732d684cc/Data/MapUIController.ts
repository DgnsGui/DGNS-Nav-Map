// --- START OF FILE MapUIController.ts (CORRECTED) ---
...
export class MapUIController extends BaseScriptComponent {
  ...
  private setupCategoryButtons() {
    if (this.showAllButton) this.showAllButton.onButtonPinched.add(this.handleShowAllButtonPinched.bind(this));
    if (this.showRestaurantsButton) this.showRestaurantsButton.onButtonPinched.add(this.handleShowRestaurantsButtonPinched.bind(this));
    if (this.showCafeButton) this.showCafeButton.onButtonPinched.add(this.handleShowCafeButtonPinched.bind(this));
    if (this.showBarsButton) this.showBarsButton.onButtonPinched.add(this.handleShowBarsButtonPinched.bind(this));
  }
  ...
  private handleSpawnPinButtonPinched(event: InteractorEvent) { this.mapComponent.addPinByLocalPosition(vec2.zero()); }
  private handleClearPinsButtonPinched(event: InteractorEvent) { this.mapComponent.removeMapPins(); }
  private handleZoomInButtonPinched(event: InteractorEvent) { this.startUserInteraction(); this.mapComponent.zoomIn(); this.endUserInteraction(); }
  private handleZoomOutButtonPinched(event: InteractorEvent) { this.startUserInteraction(); this.mapComponent.zoomOut(); this.endUserInteraction(); }
  private handleToggleMiniMapButtonPinched(isOn: boolean) { this.mapComponent.toggleMiniMap(isOn); }
  private handleShowAllButtonPinched(event: InteractorEvent) { this.searchNearbyPlaces(null, "All Places"); }
  private handleShowRestaurantsButtonPinched(event: InteractorEvent) { this.searchNearbyPlaces(null, "Restaurants"); }
  private handleShowCafeButtonPinched(event: InteractorEvent) { this.searchNearbyPlaces(null, "Cafes"); }
  private handleShowBarsButtonPinched(event: InteractorEvent) { this.searchNearbyPlaces(null, "Bars"); }
