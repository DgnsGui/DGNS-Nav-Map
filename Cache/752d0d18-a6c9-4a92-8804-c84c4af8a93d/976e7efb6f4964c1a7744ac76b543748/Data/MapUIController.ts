...
export class MapUIController extends BaseScriptComponent {
  ...
  private onStart() {
    ...
    // Définir la référence au MapUIController dans MapComponent pour le filtrage
    if (this.mapComponent && typeof this.mapComponent.setMapUIController === 'function') {
      this.mapComponent.setMapUIController(this);
      log.i("MapUIController reference set in MapComponent");
    }
  ...
  private searchNearbyPlaces(filterKeywords: string[], searchType: string) {
    try {
      ...
      log.i("  - Search type: " + searchType);
      
      // Appel à l'API Snapchat Places avec filtrage côté client
      log.i("Calling mapComponent.showNeaybyPlaces() with client-side filtering...");
      
      // Passer les mots-clés et le type de recherche au MapComponent
      this.mapComponent.showNeaybyPlaces(filterKeywords, searchType);
