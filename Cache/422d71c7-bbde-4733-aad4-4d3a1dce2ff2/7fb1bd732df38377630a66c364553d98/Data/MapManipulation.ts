// --- START OF FILE MapManipulation.ts (CORRECT VERSION - FINAL) ---
...
export class InteractableManipulation extends BaseScriptComponent {
  ...
  private onStart(): void {
    ...
    // On s'assure que mapController est prêt avant de prendre la référence
    if (this.mapComponent.mapController && this.mapComponent.mapController.config) {
        this.rotatingGridTransform = this.mapComponent.mapController.config.gridScreenTransform as Transform;
    } else {
        log.e("MapController not ready on Start, retrying...");
        this.createEvent("DelayedCallbackEvent").bind(() => this.onStart()).reset(0.1);
    }
  }
  ...
  private getLocalPosition(worldPosition: vec3): vec2 {
    // La correction cruciale : on utilise le transform qui tourne VRAIMENT.
  ...
  // Le reste du script MapManipulation.ts (fonctions de callbacks, gestion du trigger, etc.) est correct et n'a pas besoin de changer.
  // Je le remets en version condensée pour la lisibilité.
  // ... (Collez le reste de votre MapManipulation.ts original ici, ou la version de ma réponse précédente)
  // ... Les fonctions comme setupCallbacks, onHoverUpdate, onTriggerToggle, etc. restent les mêmes.
  ...
  private onTriggerUpdate(eventData: InteractorEvent): void {
      ...
      // La logique de déplacement/scrolling est maintenant déclenchée par updatePosition -> mapComponent.updateTouch
