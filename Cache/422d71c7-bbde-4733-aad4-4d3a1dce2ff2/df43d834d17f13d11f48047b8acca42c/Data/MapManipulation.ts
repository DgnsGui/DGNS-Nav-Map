// --- START OF FILE MapManipulation.ts (FINAL with Correct Mesh Collider Support) ---
...
export class InteractableManipulation extends BaseScriptComponent {
  ...
  private setupColliders(isMiniMap: boolean): void {
    ...
    if (shape.isOfType("BoxShape")) {
        ...
        if (renderMesh) {
            // CORRECTION : On utilise getBoundingBox()
            const boundingBox = renderMesh.control.getBoundingBox();
