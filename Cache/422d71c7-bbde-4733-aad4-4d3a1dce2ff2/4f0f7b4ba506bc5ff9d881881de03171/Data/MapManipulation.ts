// --- START OF FILE MapManipulation.ts (FINAL with Mesh Collider Support) ---
...
export class InteractableManipulation extends BaseScriptComponent {
  ...
  private colliderSizeX: number = 1.0; // Initialiser avec une valeur par défaut
  private colliderSizeY: number = 1.0; // Initialiser avec une valeur par défaut
  ...
  private setupColliders(isMiniMap: boolean): void {
    ...
    if (shape.isOfType("BoxShape")) {
        ...
        // La division par 8 est une "valeur magique" de ce template, on la conserve pour la cohérence.
    ...
    // --- NOUVEAU BLOC DE CODE ---
        ...
        if (mesh) {
            // On calcule la taille à partir de la bounding box du mesh.
            const min = mesh.control.aabbMin;
            const max = mesh.control.aabbMax;
            const size = max.sub(min);

            // Pour normaliser les coordonnées, nous avons besoin de la "demi-largeur" et "demi-hauteur".
            // Le scale du transform de l'objet est déjà pris en compte par le système d'interaction.
