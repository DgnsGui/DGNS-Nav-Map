...
export class InteractableManipulation extends BaseScriptComponent {
  ...
  private scrollSensitivity: number = 1.0;
  ...
  onAwake(): void {
    ...
    // Écoute le toggle minimap (quand l'utilisateur passe en mini/full map)
    this.mapComponent.onMiniMapToggled.add((isMiniMap: boolean) => {
      this.debugLog(`MiniMap toggled: ${isMiniMap ? "ON" : "OFF"}`);
  ...
  private onStart(): void {
    // La carte démarre toujours en mode plein écran (plus de startedAsMiniMap)
    this.setupColliders(false);
  ...
  private setupCallbacks(): void {
    ...
    // Hover exit - termine l'interaction si on sort pendant un drag
  ...
  private setupColliders(isMiniMap: boolean): void {
    ...
    const activeCollider = isMiniMap ? this.miniMapCollider : this.fullMapCollider;
    const shape = activeCollider.shape;
    ...
    this.debugLog(`Colliders updated - MiniMap: ${isMiniMap}, Size: (${this.colliderSizeX.toFixed(3)}, ${this.colliderSizeY.toFixed(3)})`);
  }
}