...
// Padding horizontal et vertical séparés pour le label
const LABEL_BOUNDARY_PADDING_X = 20; // Padding horizontal
const LABEL_BOUNDARY_PADDING_Y = 10; // Padding vertical
const LABEL_ELLIPSE_BOUNDARY_PADDING_X = 20; // Largeur (axe X)
...
class MapPin {
    ...
    static makeMapPin(prefab, parent, layer, renderOrder, location, placeInfo = undefined, isUser = false) {
        ...
        if (!isUser) {
            pin.setName(placeInfo === undefined ? `Map Pin ${++pinAvailableID}` : placeInfo.name);
        }
        return pin;
    }
    updateRenderBound(topLeftCorner, topLeftToBottomLeft, topLeftToTopRight) {
        ...
        if (this.label !== undefined) {
            ...
            const leftPadding = topLeftToTopRight
                .normalize()
                .uniformScale(LABEL_BOUNDARY_PADDING_X);
            const topPadding = topLeftToBottomLeft
                .normalize()
                .uniformScale(LABEL_BOUNDARY_PADDING_Y);
