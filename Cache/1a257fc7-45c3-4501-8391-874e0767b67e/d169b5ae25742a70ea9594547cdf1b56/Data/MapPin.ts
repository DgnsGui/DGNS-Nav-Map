// --- START OF FILE MapPin.ts (Corrected with Text Masking) ---
...
export class MapPin {
  ...
  labelMaterial: Material = undefined; // NOUVEAU : Référence au matériau du label
  placeInfo: PlaceInfo;
  ...
  static makeMapPin(
    ...
    if (pin.sceneObject.getChildrenCount() > 1) {
      ...
      if (pin.label) {
          ...
          // NOUVEAU : Cloner le matériau du label pour le rendre unique
          pin.labelMaterial = pin.label.mainMaterial.clone();
          pin.label.mainMaterial = pin.labelMaterial;
      }
    }
  ...
  updateRenderBound(
    ...
    // Logique pour le masque rectangulaire
    ...
    // NOUVEAU : Appliquer le masque rectangulaire au texte
  ...
  updateCircularRenderBound(center: vec3): void {
    // MODIFICATION : On applique le masque circulaire à tous les éléments
    ...
    // MODIFICATION : On supprime l'ancienne logique qui cachait juste le fond
    /*
    if (this.label !== undefined) {
      if (this.screenTransform.position.add(this.label.getTransform().getLocalPosition()).length > LABEL_CIRCLE_BOUNDARY_PADDING) {
  ...
  toggleMiniMap(isMiniMap: boolean): void {
    ...
    // NOUVEAU : Appliquer aussi au label
    if (this.labelMaterial) {
...
Après avoir appliqué ces trois changements, le texte et son fond violet seront traités exactement comme les tuiles de la carte. Ils seront parfaitement masqués par l'ovale et disparaîtront complètement lorsque tu feras défiler la carte hors de la zone visible.