// --- START OF FILE MapPin.ts (FINAL, COMPLETE, AND CORRECTED) ---
...
export class MapPin {
  ...
  labelMaterial: Material = undefined; // Référence au matériau du label
  placeInfo: PlaceInfo;
  ...
  static makeMapPin(
    ...
    if (pin.imageComponent) {
      ...
      // Cloner le matériau de l'icône pour s'assurer qu'il est unique
    ...
    if (pin.sceneObject.getChildrenCount() > 1) {
      ...
      if (pin.label) {
          ...
          // CORRECTION : On accède au matériau via la police (font)
          if (pin.label.font) {
              pin.labelMaterial = pin.label.font.material.clone();
              pin.label.font.material = pin.labelMaterial;
          }
      }
    }
