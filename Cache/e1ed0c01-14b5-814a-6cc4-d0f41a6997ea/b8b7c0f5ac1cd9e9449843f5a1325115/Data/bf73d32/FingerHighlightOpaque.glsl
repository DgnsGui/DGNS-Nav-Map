...
uniform NF_PRECISION            float  pokeOccludeGradientMaskPosition; // Title: Gradient Position	


// Uniforms ( Ports )

#if defined( STUDIO )
uniform NF_PRECISION float Port_Value1_N000;
uniform NF_PRECISION float Port_Value2_N000;
uniform NF_PRECISION float Port_Value3_N000;
...
void Node21_Float_Parameter( out float Output, ssGlobals Globals ) { Output = pokeOccludeGradientMaskPosition; }
...
#define Node0_Construct_Vector( Value1, Value2, Value3, Value4, Value, Globals ) Value.x = Value1; Value.y = Value2; Value.z = Value3; Value.w = Value4
#define Node8_Multiply( Input0, Input1, Output, Globals ) Output = float4(Input0) * Input1
//-----------------------------------------------------------------------------
...
void main() 
{
	...
	
	{
		...
		float Output_N14 = 0.0; Node14_One_Minus( Output_N59, Output_N14, Globals );
		...
		float2 CoordsOut_N18 = float2(0.0); Node18_Rotate_Coords( UVCoord_N3, NF_PORT_CONSTANT( float( -90.0 ), Port_Rotation_N018 ), NF_PORT_CONSTANT( float2( 0.5, 0.5 ), Port_Center_N018 ), CoordsOut_N18, Globals );
		...
		float Output_N37 = 0.0; Node37_Multiply( Value3_N36, Value_N20, Output_N37, Globals );
		float4 Value_N0 = float4(0.0); Node0_Construct_Vector( NF_PORT_CONSTANT( float( 0.0 ), Port_Value1_N000 ), NF_PORT_CONSTANT( float( 0.0 ), Port_Value2_N000 ), NF_PORT_CONSTANT( float( 0.0 ), Port_Value3_N000 ), Output_N37, Value_N0, Globals );
		float4 Output_N8 = float4(0.0); Node8_Multiply( Output_N14, Value_N0, Output_N8, Globals );
		
		FinalColor = Output_N8;
	}
