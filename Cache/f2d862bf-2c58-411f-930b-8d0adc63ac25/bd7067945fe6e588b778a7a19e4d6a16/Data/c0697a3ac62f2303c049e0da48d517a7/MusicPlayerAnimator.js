...
let ZToggleMover = (() => {
    ...
    var ZToggleMover = _classThis = class extends _classSuper {
        constructor() {
            ...
            this.animationStartRotY = 0;
            this.animationEndRotY = 0;
        ...
        togglePosition() {
            ...
            this.animating = true;
            if (this.enableDebugLogs)
                log.i(`Animation lancée vers Z = ${this.animationEndZ}, Y = ${this.animationEndRotY}`);
        ...
        updateAnimation(eventData) {
            ...
            this.targetTransform.setLocalRotation(quat.fromEulerAngles(currentRot.x, newRotY, currentRot.z));
            if (t >= 1.0) {
                this.animating = false;
                if (this.enableDebugLogs)
                    log.i(`Animation terminée à Z = ${newZ}, Y = ${newRotY}`);
            }
        }
        ...
        moveToInitial(logIt = false) {
            ...
            const currentRot = this.targetTransform.getLocalRotation().toEulerAngles();
            this.targetTransform.setLocalRotation(quat.fromEulerAngles(currentRot.x, 0, currentRot.z));
            if (logIt && this.enableDebugLogs)
                log.i(`Retour à la position initiale : Z = ${this.initialZ}, Y = 0`);
        }
        moveToTarget(logIt = false) {
            ...
            const currentRot = this.targetTransform.getLocalRotation().toEulerAngles();
            this.targetTransform.setLocalRotation(quat.fromEulerAngles(currentRot.x, 180, currentRot.z));
            if (logIt && this.enableDebugLogs)
                log.i(`Déplacé vers l'avant : Z = ${pos.z}, Y = 180`);
        }
    };
