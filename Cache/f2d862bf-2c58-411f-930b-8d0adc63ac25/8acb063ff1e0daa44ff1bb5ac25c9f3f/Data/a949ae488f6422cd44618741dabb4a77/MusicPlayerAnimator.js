...
let MusicPlayerAnimator = (() => {
    ...
    var MusicPlayerAnimator = _classThis = class extends _classSuper {
        constructor() {
            super();
            this.musicObject = this.musicObject;
            this.visibleZ = this.visibleZ;
            this.hiddenZ = this.hiddenZ;
        ...
        recede() {
            if (this.isReceded || this.isAnimating) {
                if (this.enableDebugLogs)
                    log.i("Already receded or animating, skipping.");
                return;
            }
            if (this.currentAnimation)
                this.currentAnimation();
            if (this.enableDebugLogs)
                log.i("Receding MusicPlayer...");
        ...
        restore() {
            if (!this.isReceded || this.isAnimating) {
                if (this.enableDebugLogs)
                    log.i("Already restored or animating, skipping.");
                return;
            }
            if (this.currentAnimation)
                this.currentAnimation();
            if (this.enableDebugLogs)
                log.i("Restoring MusicPlayer...");
            this.isAnimating = true;
            this.isReceded = false;
            const startZ = this.getCurrentZ();
            this.currentAnimation = (0, animate_1.default)({
