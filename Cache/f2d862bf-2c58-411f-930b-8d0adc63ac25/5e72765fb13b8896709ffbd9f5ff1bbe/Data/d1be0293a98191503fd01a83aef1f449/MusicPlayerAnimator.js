...
let ZToggleMover = (() => {
    ...
    var ZToggleMover = _classThis = class extends _classSuper {
        ...
        togglePosition() {
            ...
            if (this.enableDebugLogs)
                log.i(`Animation lancée vers Z = ${this.animationEndZ}, oscillateForward = ${this.oscillateForward}`);
        ...
        updateAnimation(eventData) {
            ...
            if (t >= 1.0) {
                this.animating = false;
                if (this.enableDebugLogs)
                    log.i(`Animation terminée à Z = ${newZ}, X = ${newX}`);
            }
        }
        ...
        moveToInitial(logIt = false) {
            ...
            if (logIt && this.enableDebugLogs)
                log.i(`Retour à la position initiale : Z = ${this.initialZ}, X = ${this.initialX}`);
        }
        moveToTarget(logIt = false) {
            ...
            this.targetTransform.setLocalPosition(pos);
            if (logIt && this.enableDebugLogs)
                log.i(`Déplacé vers l'avant : Z = ${pos.z}, X = ${this.initialX}`);
        }
    };
