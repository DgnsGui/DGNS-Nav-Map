    ...
    var PlacesQuestMarkerLinker = _classThis = class extends _classSuper {
        ...
        attemptLinking() {
            ...
            if (!linked && this.linkingAttempts < this.maxAttempts) {
                ...
                // NOUVEAU: Forcer l'application de l'état après le linking
                const applyDelayEvent = this.createEvent("DelayedCallbackEvent");
                applyDelayEvent.bind(() => {
                    print("Applying quest markers state after linking...");
                    this.mapComponent.forceHideAllQuestMarkersThenShow();
                });
                applyDelayEvent.reset(0.5);
            }
            else {
        ...
        linkExistingQuestMarkers() {
            ...
            if (mapControllerAny.questMarkers && mapControllerAny.questMarkers.length > 0) {
                ...
                let linkedCount = 0;
                // CRITIQUE: D'abord CACHER TOUS les quest markers
                for (let i = 0; i < questMarkers.length; i++) {
                    ...
                    if (questMarker && questMarker.transform) {
                        ...
                        if (sceneObject) {
                            ...
                            print("Pre-hiding quest marker " + i + ": " + sceneObject.name);
                        }
                    }
                }
                // Ensuite linker
        ...
        linkQuestMarker(pin, questMarker) {
            // NOUVEAU: Cacher d'abord le quest marker avant de le linker
            if (questMarker && questMarker.transform) {
                const sceneObject = questMarker.transform.getSceneObject();
                if (sceneObject) {
                    sceneObject.enabled = false;
                    print("Pre-hiding quest marker before linking: " + sceneObject.name);
                }
            }
