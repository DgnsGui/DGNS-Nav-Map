...
uniform NF_PRECISION float3 Port_Red_N029;
uniform NF_PRECISION float3 Port_Green_N029;
uniform NF_PRECISION float3 Port_Blue_N029;
...
void Node29_ReColor( in float4 Input, in float3 Red, in float3 Green, in float3 Blue, out float4 Output, ssGlobals Globals )
{ 
	Output.rgb = Input.r * Red + Input.g * Green + Input.b * Blue;
	Output.a   = Input.a;
}
...
void main() 
{
	...
	
	{
		...
		float4 Output_N29 = float4(0.0); Node29_ReColor( Export_N385, NF_PORT_CONSTANT( float3( 0.0823529, 0.0, 1.0 ), Port_Red_N029 ), NF_PORT_CONSTANT( float3( 0.0, 0.0, 1.0 ), Port_Green_N029 ), NF_PORT_CONSTANT( float3( 1.0, 1.0, 1.0 ), Port_Blue_N029 ), Output_N29, Globals );
		...
		float4 Value_N1 = float4(0.0); Node1_Construct_Vector( Output_N29.xyz, Export_N158, Value_N1, Globals );
