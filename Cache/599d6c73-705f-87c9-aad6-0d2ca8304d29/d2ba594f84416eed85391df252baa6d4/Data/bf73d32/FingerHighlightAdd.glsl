...
uniform NF_PRECISION            float4 pokeColor; // Title: Color
uniform NF_PRECISION            float4 pinchColor; // Title: Color
uniform NF_PRECISION            float  pokePinchBlend; // Title: Poke Pinch Blend
uniform NF_PRECISION            float  pinchBrightness; // Title: Brightness
SC_DECLARE_TEXTURE(maskMap); // Title: Mask Map
uniform NF_PRECISION            float  pinchExponent; // Title: Pinch Exponent
uniform NF_PRECISION            float  pokeBrightness; // Title: Brightness
uniform NF_PRECISION            float  pokeExponent; // Title: Poke Exponent
uniform NF_PRECISION            float  pokeHighlightGradientMaskPosition; // Title: Gradient Position	


// Uniforms ( Ports )

#if defined( STUDIO )
uniform NF_PRECISION float Port_Rotation_N018;
uniform NF_PRECISION float2 Port_Center_N018;
uniform NF_PRECISION float Port_Value0_N020;
uniform NF_PRECISION float Port_Position1_N020;
uniform NF_PRECISION float Port_Value1_N020;
uniform NF_PRECISION float Port_Value2_N020;
uniform NF_PRECISION float Port_Value3_N020;
#endif	
...
void Node30_Color_Parameter( out float4 Output, ssGlobals Globals ) { Output = pokeColor; }
#define Node16_Swizzle( Input, Output, Globals ) Output = float3( Input.r, Input.g, Input.b )
void Node6_Color_Parameter( out float4 Output, ssGlobals Globals ) { Output = pinchColor; }
#define Node15_Swizzle( Input, Output, Globals ) Output = float3( Input.r, Input.g, Input.b )
void Node59_Float_Parameter( out float Output, ssGlobals Globals ) { Output = pokePinchBlend; }
#define Node60_Mix( Input0, Input1, Input2, Output, Globals ) Output = mix( Input0, Input1, Input2 )
void Node28_Float_Parameter( out float Output, ssGlobals Globals ) { Output = pinchBrightness; }
#define Node12_Texture_2D_Object_Parameter( Globals ) /*nothing*/
#define Node34_Surface_UV_Coord( UVCoord, Globals ) UVCoord = Globals.Surface_UVCoord1
#define Node31_Texture_2D_Sample( UVCoord, Color, Globals ) Color = SC_SAMPLE_TEX_R(maskMap, UVCoord, 0.0)
void Node36_Split_Vector( in float4 Value, out float Value1, out float Value2, out float Value3, out float Value4, ssGlobals Globals )
{ 
	Value1 = Value.r;
	Value2 = Value.g;
	Value3 = Value.b;
...
#define Node29_Surface_Normal( Normal, Globals ) Normal = Globals.VertexNormal_WorldSpace
#define Node39_View_Vector( ViewVector, Globals ) ViewVector = Globals.ViewDirWS
#define Node42_Dot_Product( Input0, Input1, Output, Globals ) Output = dot( Input0, Input1 )
#define Node46_One_Minus( Input0, Output, Globals ) Output = 1.0 - Input0
void Node7_Float_Parameter( out float Output, ssGlobals Globals ) { Output = pinchExponent; }
#define Node43_Pow( Input0, Input1, Output, Globals ) Output = ( Input0 <= 0.0 ) ? 0.0 : pow( Input0, Input1 )
...
void Node41_Float_Parameter( out float Output, ssGlobals Globals ) { Output = pokeBrightness; }
void Node0_Float_Parameter( out float Output, ssGlobals Globals ) { Output = pokeExponent; }
#define Node47_Pow( Input0, Input1, Output, Globals ) Output = ( Input0 <= 0.0 ) ? 0.0 : pow( Input0, Input1 )
#define Node24_Multiply( Input0, Input1, Output, Globals ) Output = Input0 * Input1
...
void Node18_Rotate_Coords( in float2 CoordsIn, in float Rotation, in float2 Center, out float2 CoordsOut, ssGlobals Globals )
{ 
...
void Node21_Float_Parameter( out float Output, ssGlobals Globals ) { Output = pokeHighlightGradientMaskPosition; }
void Node20_Gradient( in float Ratio, in float Value0, in float Position1, in float Value1, in float Position2, in float Value2, in float Value3, out float Value, ssGlobals Globals )
{ 
	...
	NF_PREVIEW_SAVE( float4( Value, Value, Value, 1.0 ), 20, false )
}
#define Node37_Multiply( Input0, Input1, Output, Globals ) Output = Input0 * Input1
#define Node51_Max( Input0, Input1, Output, Globals ) Output = max( Input0, Input1 )
#define Node52_Construct_Vector( Value1, Value2, Value, Globals ) Value.xyz = Value1; Value.w = Value2
//-----------------------------------------------------------------------------
...
void main() 
{
	...
	{
		float4 Output_N30 = float4(0.0); Node30_Color_Parameter( Output_N30, Globals );
		float3 Output_N16 = float3(0.0); Node16_Swizzle( Output_N30.xyz, Output_N16, Globals );
		float4 Output_N6 = float4(0.0); Node6_Color_Parameter( Output_N6, Globals );
		float3 Output_N15 = float3(0.0); Node15_Swizzle( Output_N6.xyz, Output_N15, Globals );
		float Output_N59 = 0.0; Node59_Float_Parameter( Output_N59, Globals );
		float3 Output_N60 = float3(0.0); Node60_Mix( Output_N16, Output_N15, float3( Output_N59 ), Output_N60, Globals );
		float Output_N28 = 0.0; Node28_Float_Parameter( Output_N28, Globals );
		Node12_Texture_2D_Object_Parameter( Globals );
		float2 UVCoord_N34 = float2(0.0); Node34_Surface_UV_Coord( UVCoord_N34, Globals );
		float4 Color_N31 = float4(0.0); Node31_Texture_2D_Sample( UVCoord_N34, Color_N31, Globals );
		float Value1_N36 = 0.0; float Value2_N36 = 0.0; float Value3_N36 = 0.0; float Value4_N36 = 0.0; Node36_Split_Vector( Color_N31, Value1_N36, Value2_N36, Value3_N36, Value4_N36, Globals );
		float3 Normal_N29 = float3(0.0); Node29_Surface_Normal( Normal_N29, Globals );
		float3 ViewVector_N39 = float3(0.0); Node39_View_Vector( ViewVector_N39, Globals );
		float Output_N42 = 0.0; Node42_Dot_Product( Normal_N29, ViewVector_N39, Output_N42, Globals );
		float Output_N46 = 0.0; Node46_One_Minus( Output_N42, Output_N46, Globals );
		float Output_N7 = 0.0; Node7_Float_Parameter( Output_N7, Globals );
		float Output_N43 = 0.0; Node43_Pow( Output_N46, Output_N7, Output_N43, Globals );
		float Output_N50 = 0.0; Node50_Multiply( Value1_N36, Output_N43, Output_N50, Globals );
		float Output_N11 = 0.0; Node11_Multiply( Output_N28, Output_N50, Output_N11, Globals );
		...
		float Output_N47 = 0.0; Node47_Pow( Output_N46, Output_N0, Output_N47, Globals );
		float Output_N24 = 0.0; Node24_Multiply( Output_N47, Value2_N36, Output_N24, Globals );
		float Output_N32 = 0.0; Node32_Multiply( Output_N41, Output_N24, Output_N32, Globals );
		float2 UVCoord_N8 = float2(0.0); Node8_Surface_UV_Coord( UVCoord_N8, Globals );
		float2 CoordsOut_N18 = float2(0.0); Node18_Rotate_Coords( UVCoord_N8, NF_PORT_CONSTANT( float( -90.0 ), Port_Rotation_N018 ), NF_PORT_CONSTANT( float2( 0.5, 0.5 ), Port_Center_N018 ), CoordsOut_N18, Globals );
		float Output_N21 = 0.0; Node21_Float_Parameter( Output_N21, Globals );
		float Value_N20 = 0.0; Node20_Gradient( CoordsOut_N18.x, NF_PORT_CONSTANT( float( 1.0 ), Port_Value0_N020 ), NF_PORT_CONSTANT( float( 0.0 ), Port_Position1_N020 ), NF_PORT_CONSTANT( float( 1.0 ), Port_Value1_N020 ), Output_N21, NF_PORT_CONSTANT( float( 0.0 ), Port_Value2_N020 ), NF_PORT_CONSTANT( float( 0.0 ), Port_Value3_N020 ), Value_N20, Globals );
		float Output_N37 = 0.0; Node37_Multiply( Output_N32, Value_N20, Output_N37, Globals );
		float Output_N51 = 0.0; Node51_Max( Output_N11, Output_N37, Output_N51, Globals );
		float4 Value_N52 = float4(0.0); Node52_Construct_Vector( Output_N60, Output_N51, Value_N52, Globals );
		
		FinalColor = Value_N52;
	}
