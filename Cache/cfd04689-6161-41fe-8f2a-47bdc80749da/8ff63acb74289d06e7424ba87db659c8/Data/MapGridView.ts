// --- START OF FILE MapGridView.ts (Corrected for Rotational Scrolling) ---
...
export class MapGridView {
  ...
  // NOUVELLE FONCTION UTILITAIRE pour la rotation
  private rotateVec2(v: vec2, angleRad: number): vec2 {
  ...
  handleScrollStart(localPosition: vec2): void {
    if (
      ...
      // MODIFICATION : On ne transforme plus les coordonnées ici. On stocke la position brute.
  ...
  handleScrollUpdate(localPosition: vec2): void {
    if (
      ...
      if (this.isScrolling) {
        // MODIFICATION : On ne transforme plus les coordonnées ici.
        localPosition = localPosition.uniformScale(0.5);
        ...
        // MODIFICATION : Calcul du déplacement brut (non-rotaté)
        ...
        // MODIFICATION : Récupération de l'angle de rotation de la carte
        const mapRotationRad = customGetEuler(this.config.gridScreenTransform.rotation).z;
        
        // MODIFICATION : Rotation du vecteur de déplacement dans le sens inverse de la carte
        const rotatedOffset = this.rotateVec2(touchOffsetFromInitialTouch, -mapRotationRad);
        const rotatedDelta = this.rotateVec2(touchOffsetFromLastTouch, -mapRotationRad);
        ...
        // MODIFICATION : Application des vecteurs rotatés
        this.draggingOffset = rotatedOffset
        ...
        const acceleration = rotatedDelta
  ...
  // MODIFICATION : Cette fonction n'est plus utilisée pour le scroll, mais reste pour d'autres usages.
