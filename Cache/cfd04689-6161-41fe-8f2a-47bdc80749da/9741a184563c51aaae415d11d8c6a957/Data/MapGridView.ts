// --- START OF FILE MapGridView.ts (Final) ---
...
export class MapGridView {
    ...
    if (neededCellsCount !== this.cells.length) {
      ...
      if (difference < 0) {
        for (let i = 0; i < Math.abs(difference); i++) {
          ...
          if (isFunction(cellToRemove.onShouldDestroy)) cellToRemove.onShouldDestroy();
        }
      } else if (difference > 0) {
        for (let i = 0; i < difference; i++) {
          const cell = Cell.makeCell(this.mapController.referencePositionLocationAsset);
  ...
  updateGridView(mapPins: Set<MapPin>, userPin: MapPin) {
    ...
    if (this.needsLayout) this.layoutCells();
    ...
    mapPins.forEach((pin) => {
      if (this.isMiniMapEnabled) pin.updateCircularRenderBound(center);
      else pin.updateRenderBound(topLeftCorner, topLeftToBottomLeft, topLeftToTopRight);
    });
    if (userPin) {
      if (this.isMiniMapEnabled) userPin.updateCircularRenderBound(center);
      else userPin.updateRenderBound(topLeftCorner, topLeftToBottomLeft, topLeftToTopRight);
    }
    this.cells.forEach((cell) => {
      if (this.isMiniMapEnabled) cell.updateCircularRenderBound(center);
      else cell.updateRenderBound(topLeftCorner, topLeftToBottomLeft, topLeftToTopRight);
      if (this.screenPositionChangedDidChange) cell.onScreenPositionChanged();
    });
    if (this.screenPositionChangedDidChange) this.screenPositionChangedDidChange = false;
  }

  toggleMiniMap(isOn: boolean, mapPins: Set<MapPin>, userPin: MapPin, isAnimated: boolean = true): void {
    ...
    if (userPin) userPin.toggleMiniMap(this.config.isMiniMap);
  ...
  layoutCells(forcedUpdate: boolean = false): void {
    ...
    for (let x = 0; x < this.config.tileCount; x++) {
      for (let y = 0; y < this.config.tileCount; y++) {
        ...
        const wrappedXOffset = mod(targetXOffset + 1.5, this.config.tileCount) - 1.5;
        const wrappedYOffset = mod(targetYOffset + 1.5, this.config.tileCount) - 1.5;
        setScreenTransformRect01(cell.screenTransform, wrappedXOffset, wrappedYOffset, 1, 1);
        ...
        if (indexInHorizontalRange && indexInVerticalRange) {
          if (targetHorizontalIndex !== cell.horizontalIndex) { cell.horizontalIndex = targetHorizontalIndex; indexChanged = true; }
          if (targetVerticalIndex !== cell.verticalIndex) { cell.verticalIndex = targetVerticalIndex; indexChanged = true; }
        }
        if (cell.sceneObject.enabled && (!indexInHorizontalRange || !indexInVerticalRange)) { cell.onDisabled(); cell.sceneObject.enabled = false; }
        if (!cell.sceneObject.enabled && indexInHorizontalRange && indexInVerticalRange) { cell.onEnabled(); cell.sceneObject.enabled = true; }
        if (indexChanged || forcedUpdate) {
          if (forcedUpdate) cell.onZoomChanged(this.mapController.referencePositionLocationAsset);
          cell.onDataChanged();
        }
      }
    }
  ...
  handleScrollStart(localPosition: vec2): void {
    if (!this.config.horizontalScrollingEnabled && !this.config.verticalScrollingEnabled) return;
  ...
  getOffset(): vec2 { return this.currentOffset.add(this.draggingOffset); }
  ...
  getConfig(): MapConfig { return this.config; }
  handleReloadData(): void { this.cells.forEach(cell => cell.onDataChanged()); }
  resetVelocity(): void { this.velocity = vec2.zero(); }
  private clampCurrentOffset(): void {
    this.currentOffset.x = clamp(-this.config.horizontalMaxIndex, -this.config.horizontalMinIndex, this.currentOffset.x);
    this.currentOffset.y = clamp(-this.config.verticalMaxIndex, -this.config.verticalMinIndex, this.currentOffset.y);
  }
}