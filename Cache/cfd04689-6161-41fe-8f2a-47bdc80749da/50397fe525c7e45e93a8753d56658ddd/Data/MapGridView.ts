  ...
  handleScrollUpdate(localPosition: vec2): void {
    if (
      ...
      if (this.isScrolling) {
        ...
        // FIXED: Apply rotation compensation to the touch offset using MapController's method
        const touchOffsetFromInitialTouch = localPosition.sub(this.initialLocalTouchPosition);
        const rotationAdjustedOffset = this.mapController.getPositionWithMapRotationOffset(touchOffsetFromInitialTouch);
        
        const constrainAxis = new vec2(
          this.config.horizontalScrollingEnabled ? 1 : 0,
          this.config.verticalScrollingEnabled ? 1 : 0
        );
        this.draggingOffset = rotationAdjustedOffset
        ...
        // FIXED: Apply rotation compensation to velocity calculation using MapController's method
        const touchOffsetFromLastTouch = localPosition.sub(this.lastTouchPosition);
        const rotationAdjustedVelocityOffset = this.mapController.getPositionWithMapRotationOffset(touchOffsetFromLastTouch);
        
        const acceleration = rotationAdjustedVelocityOffset
