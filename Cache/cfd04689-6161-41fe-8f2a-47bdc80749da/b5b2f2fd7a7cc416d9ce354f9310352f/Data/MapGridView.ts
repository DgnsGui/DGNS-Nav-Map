// --- START OF FILE MapGridView.ts (Final Fix for Rotational Scrolling) ---
...
export class MapGridView {
  ...
  updateGridView(mapPins: Set<MapPin>, userPin: MapPin) {
    ...
    // Ajout d'une vérification pour userPin au cas où il est désactivé
    if (userPin) {
        if (this.isMiniMapEnabled) {
  ...
  handleScrollStart(localPosition: vec2): void {
    if (
      ...
      // *** MODIFICATION : On enlève TOUTE correction de rotation ici ***
  ...
  handleScrollUpdate(localPosition: vec2): void {
    if (
      ...
      if (this.isScrolling) {
        // *** MODIFICATION : On enlève TOUTE correction de rotation ici aussi ***
        localPosition = localPosition.uniformScale(0.5);
        ...
        // Le code suivant est maintenant beaucoup plus simple et correct.
