...
class MapGridView {
    ...
    // CORRECTION PRINCIPALE: Transformation des coordonnées pour gérer la rotation de la carte
    transformTouchPositionForMapRotation(localPosition) {
        // Si la carte n'est pas en mode minimap ou pas de rotation auto, pas de transformation nécessaire
        if (!this.config.isMiniMap || !this.mapController.getMinimapAutoRotate()) {
            return localPosition;
        }
        // Obtenir l'angle de rotation actuel de la carte
        const mapRotation = (0, MapUtils_1.customGetEuler)(this.config.gridScreenTransform.rotation).z;
        // Calculer l'angle et la distance du point de touch
        ...
        // Appliquer la rotation inverse pour compenser la rotation de la carte
        const adjustedAngle = angle + mapRotation; // Rotation inverse
        // Retourner les nouvelles coordonnées
        return new vec2(Math.cos(adjustedAngle) * distance, Math.sin(adjustedAngle) * distance);
    }
    handleScrollStart(localPosition) {
        if (this.config.horizontalScrollingEnabled ||
            this.config.verticalScrollingEnabled) {
            // CORRECTION: Appliquer la transformation de rotation avant toute autre opération
            localPosition = this.transformTouchPositionForMapRotation(localPosition);
            ...
            this.velocity = vec2.zero();
            this.draggingOffset = vec2.zero();
            this.isDragging = false; // CORRECTION: était == au lieu de =
    ...
    handleScrollUpdate(localPosition) {
        if (this.config.horizontalScrollingEnabled ||
            ...
            if (this.isScrolling) {
                ...
                // Calculer l'offset de déplacement depuis la position initiale
    ...
    // DEPRECATED: Cette méthode est remplacée par transformTouchPositionForMapRotation
    getPositionWithMapRotationOffset(localPosition) {
        return this.transformTouchPositionForMapRotation(localPosition);
    }
}
