...
export class MapGridView {
  ...
  private transformTouchPositionForMapRotation(localPosition: vec2): vec2 {
    ...
    const mapRotation = customGetEuler(this.config.gridScreenTransform.rotation).z;
    ...
    return new vec2(
      Math.cos(adjustedAngle) * distance,
      Math.sin(adjustedAngle) * distance
    );
  }
