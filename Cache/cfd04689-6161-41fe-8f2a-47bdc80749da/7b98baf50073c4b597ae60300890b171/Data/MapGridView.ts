  ...
  handleScrollUpdate(localPosition: vec2): void {
    if (
      ...
      if (this.isScrolling) {
        ...
        // FIXED: Apply rotation compensation to the touch offset
        const touchOffsetFromInitialTouch = localPosition.sub(this.initialLocalTouchPosition);
        const rotationAdjustedOffset = this.getPositionWithMapRotationOffset(touchOffsetFromInitialTouch);
        ...
        // FIXED: Apply rotation compensation to velocity calculation
        const touchOffsetFromLastTouch = localPosition.sub(this.lastTouchPosition);
        const rotationAdjustedVelocityOffset = this.getPositionWithMapRotationOffset(touchOffsetFromLastTouch);
