...
uniform NF_PRECISION float Port_Value0_N018;
uniform NF_PRECISION float Port_Position1_N018;
uniform NF_PRECISION float Port_Value1_N018;
uniform NF_PRECISION float Port_Position2_N018;
uniform NF_PRECISION float Port_Value2_N018;
uniform NF_PRECISION float Port_Position3_N018;
uniform NF_PRECISION float Port_Value3_N018;
uniform NF_PRECISION float Port_Value4_N018;
uniform NF_PRECISION float4 Port_Input1_N034;
...
void Node18_Gradient( in float Ratio, in float Value0, in float Position1, in float Value1, in float Position2, in float Value2, in float Position3, in float Value3, in float Value4, out float Value, ssGlobals Globals )
{ 
	Ratio = clamp( Ratio, 0.0, 1.0 );
...
#define Node23_Construct_Vector( Value1, Value2, Value, Globals ) Value.rgb = Value1; Value.a = Value2
...
#define Node34_Mix( Input0, Input1, Input2, Output, Globals ) Output = mix( Input0, Input1, float4(Input2) )
...
void main() 
{
	...
	
	{
		...
		float4 Output_N5 = float4(0.0); Node5_Mix( Export_N59, Export_N76, NF_PORT_CONSTANT( float( 0.2 ), Port_Input2_N005 ), Output_N5, Globals );
		...
		float4 Value_N7 = float4(0.0); Node7_Gradient( Output_N10, NF_PORT_CONSTANT( float4( 0.0, 0.0, 0.0, 1.0 ), Port_Value0_N007 ), NF_PORT_CONSTANT( float( 0.24 ), Port_Position1_N007 ), NF_PORT_CONSTANT( float4( 0.0500038, 0.0500038, 0.0500038, 1.0 ), Port_Value1_N007 ), NF_PORT_CONSTANT( float( 0.7 ), Port_Position2_N007 ), NF_PORT_CONSTANT( float4( 0.504799, 0.520943, 0.545144, 1.0 ), Port_Value2_N007 ), NF_PORT_CONSTANT( float4( 0.212497, 0.220005, 0.250004, 1.0 ), Port_Value3_N007 ), Value_N7, Globals );
		float Value_N18 = 0.0; Node18_Gradient( Value_N7.x, NF_PORT_CONSTANT( float( 1.0 ), Port_Value0_N018 ), NF_PORT_CONSTANT( float( 0.0 ), Port_Position1_N018 ), NF_PORT_CONSTANT( float( 0.0 ), Port_Value1_N018 ), NF_PORT_CONSTANT( float( 0.27 ), Port_Position2_N018 ), NF_PORT_CONSTANT( float( 0.5 ), Port_Value2_N018 ), NF_PORT_CONSTANT( float( 0.34 ), Port_Position3_N018 ), NF_PORT_CONSTANT( float( 1.0 ), Port_Value3_N018 ), NF_PORT_CONSTANT( float( 1.0 ), Port_Value4_N018 ), Value_N18, Globals );
		float4 Value_N23 = float4(0.0); Node23_Construct_Vector( Value_N7.xyz, Value_N18, Value_N23, Globals );
		...
		float4 Output_N34 = float4(0.0); Node34_Mix( Value_N23, NF_PORT_CONSTANT( float4( 1.0, 0.873198, 0.239216, 1.0 ), Port_Input1_N034 ), Rim_N31, Output_N34, Globals );
		float4 Output_N100 = float4(0.0); Node100_Add( Output_N5, Output_N34, Output_N100, Globals );
		Node29_Texture_2D_Object_Parameter( Globals );
		float2 UVCoord_N12 = float2(0.0); Node12_Surface_UV_Coord( UVCoord_N12, Globals );
		float2 CoordsOut_N2 = float2(0.0); Node2_Scale_Coords( UVCoord_N12, NF_PORT_CONSTANT( float2( 1.9, 1.9 ), Port_Scale_N002 ), NF_PORT_CONSTANT( float2( 0.5, 0.5 ), Port_Center_N002 ), CoordsOut_N2, Globals );
		float4 Color_N30 = float4(0.0); Node30_Texture_2D_Sample( CoordsOut_N2, Color_N30, Globals );
