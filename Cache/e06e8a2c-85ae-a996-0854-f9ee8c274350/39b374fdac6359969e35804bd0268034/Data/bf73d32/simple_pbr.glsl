...
#define Node1_Texture_2D_Parameter( Output, Globals ) Output = SC_SAMPLE_TEX_R(baseTex, Globals.Surface_UVCoord0, 0.0)
#define Node3_Multiply( Input0, Input1, Output, Globals ) Output = Input0 * Input1
#define Node7_Swizzle( Input, Output, Globals ) Output = Input.a
void Node2_Float_Parameter( out float Output, ssGlobals Globals ) { Output = metallic; }
void Node0_Float_Parameter( out float Output, ssGlobals Globals ) { Output = roughness; }
...
void main() 
{
	...
	
	{
		...
		float4 Output_N1 = float4(0.0); Node1_Texture_2D_Parameter( Output_N1, Globals );
		float4 Output_N3 = float4(0.0); Node3_Multiply( Output_N5, Output_N1, Output_N3, Globals );
		float Output_N7 = 0.0; Node7_Swizzle( Output_N3, Output_N7, Globals );
		float Output_N2 = 0.0; Node2_Float_Parameter( Output_N2, Globals );
		float Output_N0 = 0.0; Node0_Float_Parameter( Output_N0, Globals );
		float4 Output_N6 = float4(0.0); Node6_PBR_Lighting( Output_N3.xyz, Output_N7, NF_PORT_CONSTANT( float3( 0.0, 1.0, 0.0 ), Port_Normal_N006 ), NF_PORT_CONSTANT( float3( 0.0, 0.0, 0.0 ), Port_Emissive_N006 ), Output_N2, Output_N0, NF_PORT_CONSTANT( float3( 1.0, 1.0, 1.0 ), Port_AO_N006 ), NF_PORT_CONSTANT( float3( 1.0, 1.0, 1.0 ), Port_SpecularAO_N006 ), Output_N6, Globals );
